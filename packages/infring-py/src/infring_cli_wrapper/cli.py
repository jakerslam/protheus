...
def _candidate_repo_roots() -> List[Path]:
    roots: List[Path] = []
    env_root = os.environ.get("INFRING_REPO_ROOT")
    if env_root:
...
def _build_commands(repo_root: Optional[Path]) -> List[List[str]]:
    ...
    env_bin = os.environ.get("INFRING_OPS_BIN", "").strip()
    if env_bin:
        commands.append(shlex.split(env_bin))

    path_bin = shutil.which("infring-ops")
    if path_bin:
        commands.append([path_bin])

    if repo_root is not None:
        release_bin = repo_root / "target" / "release" / "infring-ops"
        debug_bin = repo_root / "target" / "debug" / "infring-ops"
...
def _run_first_available(args: List[str]) -> int:
    ...
    lines = [
        "infring-cli-wrapper: could not locate a runnable infring-ops binary.",
        "Tried:",
    ]
    lines.extend(f"  - {entry}" for entry in attempted if entry)
    lines.append(
        "Set INFRING_OPS_BIN, install infring-ops on PATH, or run from a repo containing core/layer0/ops/Cargo.toml."
    )
    sys.stderr.write("\n".join(lines) + "\n")
    return 127
