...
function exeName() { return process.platform === 'win32' ? 'infring-ops.exe' : 'infring-ops'; }
...
function releaseTag() {
  const override = sanitizeText(process.env.INFRING_NPM_RELEASE_TAG || '', 80);
...
function releaseCandidateUrls() {
  ...
  const stems = ['infring-ops', 'infring-ops'];
  const urls = [];
...
async function tryDownload(outPath) {
  for (const url of releaseCandidateUrls()) {
    try {
      ...
      chmodExec(outPath);
      process.stdout.write('[infring npm] downloaded prebuilt binary: ' + url + '\n');
      return true;
    } catch {}
  }
  return false;
...
function tryBuildLocal(outPath) {
  ...
  const build = spawnSync('cargo', ['build', '--release', '--manifest-path', manifestPath, '--bin', 'infring-ops'], { cwd: workspaceRoot, stdio: 'inherit' });
  ...
  for (const built of candidates) {
    ...
    chmodExec(outPath);
    process.stdout.write('[infring npm] built local binary via cargo\n');
    return true;
  }
  return false;
}
...
async function main() {
  ...
  const forceInstall = String(process.env.INFRING_NPM_FORCE_INSTALL || '').trim() === '1';
  if (fs.existsSync(outPath) && !forceInstall) {
    chmodExec(outPath);
    process.stdout.write('[infring npm] binary already present\n');
    return;
  }

  const skipDownload = String(process.env.INFRING_NPM_SKIP_DOWNLOAD || '').trim() === '1';
  if (!skipDownload) {
    const downloaded = await tryDownload(outPath);
    if (downloaded) return;
  } else {
    process.stdout.write('[infring npm] skipping release download (INFRING_NPM_SKIP_DOWNLOAD=1)\n');
  }

  if (tryBuildLocal(outPath)) return;

  process.stderr.write('[infring npm] failed to provision binary (release download unavailable and local cargo build failed)\n');
  process.exit(1);
}

main().catch((err) => {
  process.stderr.write('[infring npm] install failed: ' + sanitizeText(err && err.message ? err.message : err, 240) + '\n');
  process.exit(1);
});
