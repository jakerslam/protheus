...
pub fn run(root: &Path, argv: &[String]) -> i32 {
    ...
    if global_json {
        env::set_var("INFRING_GLOBAL_JSON", "1");
    }
    if global_quiet {
        env::set_var("INFRING_GLOBAL_QUIET", "1");
    }
    ...
    let mut cmd = if filtered_argv.is_empty() {
        if global_version {
            ...
            let force_repl = bool_env("INFRING_FORCE_REPL", false);
            let repl_disabled = bool_env("INFRING_REPL_DISABLED", false);
                ...
                if should_offer_setup(root, skip_setup_flag) {
                    ...
                    if !setup_gate.ok {
                        eprintln!(
                            ...
                            json!({
                                "ok": false,
                                "type": "infringctl_dispatch_security_gate",
    ...
    if let Err(reason) = enforce_command_center_boundary(&cmd, &route) {
        eprintln!(
            ...
            json!({
                "ok": false,
                "type": "infringctl_boundary_guard",
