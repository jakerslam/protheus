...
use base64::Engine;
use crate::persona_dispatch_security_gate_bridge::{
...
#[path = "../../infringctl_routes.rs"]
mod infringctl_routes;
#[derive(Debug, Clone)]
pub struct Route {
...
const PERSONA_VALID_LENSES_ENV: &str = "INFRING_CTL_PERSONA_VALID_LENSES";
...
fn should_offer_setup(root: &Path, skip_setup: bool) -> bool {
    if skip_setup
        || bool_env("INFRING_SKIP_SETUP", false)
        || bool_env("INFRING_SETUP_DISABLE", false)
    {
        return false;
    }
    if bool_env("INFRING_SETUP_FORCE", false) {
        return true;
    }
    let interactive_session = std::io::stdin().is_terminal() && std::io::stdout().is_terminal();
    if !interactive_session && !bool_env("INFRING_SETUP_ALLOW_NONINTERACTIVE", false) {
        return false;
    }
...
fn runtime_mode_state_path(root: &Path) -> PathBuf {
    if let Some(raw) = env_nonempty("INFRING_RUNTIME_MODE_STATE_PATH") {
