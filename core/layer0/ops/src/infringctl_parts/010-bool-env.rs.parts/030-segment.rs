fn security_request(root: &Path, script_rel: &str, args: &[String]) -> Value {
    ...
    json!({
        "operation_id": clean(format!("infringctl_dispatch_{}_{}", now_ms, &digest[..10]), 160),
        "subsystem": "ops",
        "action": "cli_dispatch",
        "actor": "client/runtime/systems/ops/infringctl",
        "risk_class": if bool_env("INFRING_CTL_SECURITY_HIGH_RISK", false) { "high" } else { "normal" },
        "payload_digest": format!("sha256:{digest}"),
        "tags": ["infringctl", "dispatch", "foundation_lock"],
        "covenant_violation": bool_env("INFRING_CTL_SECURITY_COVENANT_VIOLATION", false),
        "tamper_signal": bool_env("INFRING_CTL_SECURITY_TAMPER_SIGNAL", false),
        "key_age_hours": env::var("INFRING_CTL_SECURITY_KEY_AGE_HOURS").ok().and_then(|v| v.parse::<u64>().ok()).unwrap_or(1),
        "operator_quorum": env::var("INFRING_CTL_SECURITY_OPERATOR_QUORUM").ok().and_then(|v| v.parse::<u8>().ok()).unwrap_or(2),
        "audit_receipt_nonce": clean(format!("nonce-{}-{}", &digest[..12], now_ms), 120),
        "zk_proof": clean(env::var("INFRING_CTL_SECURITY_ZK_PROOF").unwrap_or_else(|_| "zk-infringctl-dispatch".to_string()), 220),
