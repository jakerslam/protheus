...
fn persona_blocked_path_fails_closed_before_security_core() {
    ...
    std::env::set_var("INFRING_CTL_SECURITY_GATE_DISABLED", "0");
    std::env::set_var(
        "INFRING_CTL_PERSONA_BLOCKED_PATHS",
        "client/runtime/systems/ops/infring_control_plane.js",
    );
    ...
    std::env::remove_var("INFRING_CTL_PERSONA_BLOCKED_PATHS");
    std::env::remove_var("INFRING_CTL_SECURITY_GATE_DISABLED");
}
...
fn command_center_boundary_rejects_non_core_session_route() {
    let route = Route {
        script_rel: "client/runtime/systems/ops/infringd.js".to_string(),
...
fn node_missing_fallback_supports_help_surface() {
    let route = Route {
        script_rel: "client/runtime/systems/ops/infring_command_list.js".to_string(),
...
fn node_missing_fallback_supports_version_surface() {
    let nonce = SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let base = std::env::temp_dir().join(format!("infringctl_version_fallback_{nonce}"));
    ...
    let route = Route {
        script_rel: "client/runtime/systems/ops/infring_version_cli.js".to_string(),
...
fn node_missing_fallback_supports_version_surface_ts_route() {
    let nonce = SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let base = std::env::temp_dir().join(format!("infringctl_version_fallback_ts_{nonce}"));
    fs::create_dir_all(&base).expect("mkdir");
    fs::write(base.join("package.json"), r#"{"version":"1.2.3-ts"}"#).expect("package");
    let route = Route {
        script_rel: "client/runtime/systems/ops/infring_version_cli.ts".to_string(),
        args: vec!["version".to_string()],
        forward_stdin: false,
    };
    assert_eq!(node_missing_fallback(&base, &route, true), Some(0));
...
fn node_missing_fallback_supports_update_surface() {
    let nonce = SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let base = std::env::temp_dir().join(format!("infringctl_update_fallback_{nonce}"));
    fs::create_dir_all(&base).expect("mkdir");
...
fn node_missing_fallback_is_none_for_non_fallback_routes() {
    let route = Route {
        script_rel: "client/runtime/systems/ops/infring_diagram.js".to_string(),
        args: vec!["status".to_string()],
        forward_stdin: false,
    };
...
fn run_node_script_falls_back_when_command_list_script_is_missing() {
    let nonce = SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let base = std::env::temp_dir().join(format!("infringctl_missing_script_fallback_{nonce}"));
...
fn run_node_script_dist_mode_fails_closed_on_ts_only_entrypoint() {
    let _guard = env_guard();
    std::env::remove_var("INFRING_RUNTIME_MODE");
    std::env::remove_var("INFRING_RUNTIME_MODE_STATE_PATH");

    let nonce = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .expect("clock")
        .as_nanos();
    let base = std::env::temp_dir().join(format!("infringctl_dist_mode_mismatch_{nonce}"));
    fs::create_dir_all(base.join("client/runtime/systems/ops")).expect("mkdir");
    fs::create_dir_all(base.join("local/state/ops")).expect("state");
    fs::write(
        base.join("client/runtime/systems/ops/infring_command_list.ts"),
        "export {};",
    )
    .expect("ts entry");
    fs::write(
...
fn runtime_missing_entrypoints_dist_mode_requires_js_assets() {
    ...
    let nonce = SystemTime::now()
        ...
        .expect("clock")
        .as_nanos();
    let base = std::env::temp_dir().join(format!("infringctl_runtime_manifest_dist_{nonce}"));
    ...
    assert_eq!(
        dist_missing,
        vec!["client/runtime/systems/ops/infring_command_list.js".to_string()],
...
fn dispatch_security_embedded_engine_passes_without_cargo_manifest() {
    let _guard = env_guard();
    std::env::set_var("INFRING_CTL_SECURITY_GATE_DISABLED", "0");
    std::env::set_var("INFRING_CTL_SECURITY_DISABLE_CARGO_FALLBACK", "1");

    let nonce = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .expect("clock")
        .as_nanos();
    let base = std::env::temp_dir().join(format!("infringctl_embedded_security_{nonce}"));
    fs::create_dir_all(&base).expect("mkdir");
    ...
    let _ = fs::remove_dir_all(base);
    std::env::remove_var("INFRING_CTL_SECURITY_DISABLE_CARGO_FALLBACK");
    std::env::remove_var("INFRING_CTL_SECURITY_GATE_DISABLED");
}
