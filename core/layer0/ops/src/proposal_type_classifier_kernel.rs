...
const PLACEHOLDER_TYPES: &[&str] = &["", "unknown", "new", "queued", "pending", "proposal", "item", "generic"];
...
fn payload_json(argv: &[String]) -> Result<Value, String> {
    if let Some(raw) = lane_utils::parse_flag(argv, "payload", false) {
        return serde_json::from_str::<Value>(&raw).map_err(|err| format!("proposal_type_classifier_payload_decode_failed:{err}"));
    }
    if let Some(raw_b64) = lane_utils::parse_flag(argv, "payload-base64", false) {
        let bytes = BASE64_STANDARD.decode(raw_b64.as_bytes()).map_err(|err| format!("proposal_type_classifier_payload_base64_decode_failed:{err}"))?;
        let text = String::from_utf8(bytes).map_err(|err| format!("proposal_type_classifier_payload_utf8_decode_failed:{err}"))?;
        return serde_json::from_str::<Value>(&text).map_err(|err| format!("proposal_type_classifier_payload_decode_failed:{err}"));
    }
    Ok(json!({}))
}
...
fn normalize_text(value: Option<&Value>) -> String {
    match value { Some(Value::String(v)) => v.trim().to_string(), Some(Value::Null) | None => String::new(), Some(other) => other.to_string().trim_matches('"').trim().to_string() }
}
...
fn extract_source_eye_id(proposal: &Map<String, Value>) -> String {
    let meta = proposal.get("meta").and_then(Value::as_object).cloned().unwrap_or_default();
    let direct = normalize_text(meta.get("source_eye"));
    if !direct.is_empty() { return direct; }
    for row in proposal.get("evidence").and_then(Value::as_array).cloned().unwrap_or_default() {
        let reference = normalize_text(row.get("evidence_ref"));
        if let Some(captures) = regex::Regex::new(r"(?i)\beye:([^\s|]+)").unwrap().captures(&reference) {
...
fn infer_type_from_signals(source_eye: &str, text_blob: &str) -> &'static str {
    ...
    if matches!(eye.as_str(), "directive_pulse" | "directive_compiler") { return "directive_clarification"; }
    if matches!(eye.as_str(), "local_state_fallback" | "local_state_digest" | "tier1_exception") { return "local_state_fallback"; }
    if eye.contains("moltbook") || eye.contains("upwork") || eye.contains("bird_x") || eye.contains("x_") { return "external_intel"; }
    let has_collector = regex::Regex::new(r"\b(collector|eye|sensor|feed|ingest|crawler|scrap|parser)\b").unwrap().is_match(&text);
    ...
    if has_collector && has_repair { return "collector_remediation"; }
    if regex::Regex::new(r"\b(directive|objective|tier|scope|clarif|decompose|lineage)\b").unwrap().is_match(&text) { return "directive_clarification"; }
    if regex::Regex::new(r"\b(campaign|strategy|portfolio|sequenc|roadmap|big[-\s]?bet)\b").unwrap().is_match(&text) { return "strategy"; }
    ...
    if !eye.is_empty() && eye != "unknown_eye" { return "external_intel"; }
...
fn classify(proposal: &Map<String, Value>, opts: &Map<String, Value>) -> Value {
    ...
    let source_eye = {
        ...
        if extracted.is_empty() { normalize_text(opts.get("source_eye")) } else { extracted }
    };
    let text_blob = [
        proposal.get("title"), proposal.get("summary"), proposal.get("notes"), proposal.get("suggested_next_command"), proposal.get("expected_impact"),
        meta.get("trigger"), meta.get("normalized_objective"), meta.get("normalized_expected_outcome"), meta.get("normalized_validation_metric")
    ]
    .into_iter()
    .map(normalize_text)
    ...
    let type_key = {
        ...
        if key.is_empty() { "local_state_fallback".to_string() } else { key }
    };
    json!({
        "type": type_key,
        "inferred": true,
...
pub fn run(_root: &std::path::Path, argv: &[String]) -> i32 {
    ...
    let payload = match payload_json(&argv[1..]) { Ok(payload) => payload, Err(err) => { print_json_line(&cli_error("proposal_type_classifier_kernel_error", &err)); return 1; } };
    let input = payload_obj(&payload);
    let result = match command.as_str() {
        "normalize-type-key" => cli_receipt("proposal_type_classifier_kernel_normalize_type_key", json!({ "ok": true, "type_key": normalize_type_key(&normalize_text(input.get("value").or_else(|| input.get("type")))) })),
        "extract-source-eye-id" => cli_receipt("proposal_type_classifier_kernel_extract_source_eye_id", json!({ "ok": true, "source_eye": extract_source_eye_id(input.get("proposal").and_then(Value::as_object).unwrap_or(input)) })),
        "classify" => cli_receipt("proposal_type_classifier_kernel_classify", json!({ "ok": true, "classification": classify(input.get("proposal").and_then(Value::as_object).unwrap_or(input), input.get("opts").and_then(Value::as_object).unwrap_or(&Map::new())) })),
        _ => cli_error("proposal_type_classifier_kernel_error", &format!("unknown_command:{command}")),
    };
...
mod tests {
    ...
    fn classifies_strategy_text() {
        let result = classify(&serde_json::from_value(json!({"summary":"campaign roadmap sequencing"})).unwrap(), &Map::new());
