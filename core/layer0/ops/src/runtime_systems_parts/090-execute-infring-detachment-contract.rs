fn execute_infring_detachment_contract(
    root: &Path,
    ...
    let assimilation_root = root.join("local/state/assimilations/infring");
    let nursery_root = root.join("local/state/nursery");
    let source_control_root = root.join("config/infring_assimilation");

    if strict && !source_root.exists() {
        return Err(format!(
            "infring_source_root_missing:{}",
    ...
    let copy_plan = vec![
        (
            source_root.join("infring.json"),
            assimilation_root.join("infring.json"),
        ),
        (
    ...
    if apply {
        ...
        let assimilated_artifacts = infring_seed_to_model_artifacts(&seed_manifest);
    ...
    if profile.id == "V6-INFRING-DETACH-001.2" {
        ...
        if apply {
            lane_utils::write_json(
                ...
                &json!({
                    ...
                    "claim_evidence": [{
                        "id": profile.id,
                        "claim": "nursery_specialists_are_assimilated_from_infring_seed_artifacts_with_local_policy_bounds"
                    }]
                }),
            )?;
        }
    ...
    {
        return Err("infring_detach_source_control_mirror_empty".to_string());
    }
    ...
    let claim = if profile.id == "V6-INFRING-DETACH-001.2" {
        "infring_nursery_seed_training_is_materialized_locally_with_specialist_plan_and_receipts"
    } else if profile.id == "V6-INFRING-DETACH-001.3" {
        "infring_cron_and_nursery_contracts_are_mirrored_into_source_controlled_infring_paths"
    } else if profile.id == "V6-INFRING-DETACH-001.4" {
        "llm_runtime_registry_is_bootstrapped_from_assimilated_seed_models_with_deterministic_power_cost_ranking"
    } else {
        "infring_operator_state_and_nursery_artifacts_are_assimilated_into_infring_owned_paths_with_detachment_controls"
    };
...
fn execute_contract_profile(
    ...
    match profile.family {
        ...
        "infring_detachment_stack" => {
            execute_infring_detachment_contract(root, profile, payload, apply, strict)
        }
