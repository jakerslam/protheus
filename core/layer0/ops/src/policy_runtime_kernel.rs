...
fn cli_receipt(kind: &str, payload: Value) -> Value {
    ...
    let mut out = base_receipt(kind, ok, &ts);
    out["payload"] = payload;
...
fn cli_error(kind: &str, error: &str) -> Value {
    let ts = now_iso();
    let mut out = base_receipt(kind, false, &ts);
...
fn payload_json(argv: &[String]) -> Result<Value, String> {
    if let Some(raw) = lane_utils::parse_flag(argv, "payload", false) {
        return serde_json::from_str::<Value>(&raw)
            .map_err(|err| format!("policy_runtime_payload_decode_failed:{err}"));
    }
    if let Some(raw_b64) = lane_utils::parse_flag(argv, "payload-base64", false) {
        let bytes = BASE64_STANDARD
            .decode(raw_b64.as_bytes())
            .map_err(|err| format!("policy_runtime_payload_base64_decode_failed:{err}"))?;
        let text = String::from_utf8(bytes)
            .map_err(|err| format!("policy_runtime_payload_utf8_decode_failed:{err}"))?;
...
fn deep_merge(base_value: &Value, override_value: Option<&Value>) -> Value {
    match base_value {
        Value::Array(base_rows) => {
            if let Some(Value::Array(override_rows)) = override_value {
                Value::Array(override_rows.clone())
            } else {
                Value::Array(base_rows.clone())
            }
        }
...
fn resolve_path(repo_root: &Path, raw: &str, fallback_rel: &str) -> String {
    ...
    let resolved = if candidate.is_absolute() {
        ...
        root_dir(repo_root, &Map::new()).join(candidate)
    };
    resolved.to_string_lossy().to_string()
}
...
fn load_policy_runtime(repo_root: &Path, payload: &Map<String, Value>) -> Value {
    ...
    let raw = if policy_path.is_empty() {
        json!({})
    } else {
        lane_utils::read_json(Path::new(&policy_path)).unwrap_or_else(|| json!({}))
    };
...
pub fn run(root: &Path, argv: &[String]) -> i32 {
    ...
    let payload = match payload_json(&argv[1..]) {
        Ok(payload) => payload,
        Err(err) => {
            print_json_line(&cli_error("policy_runtime_kernel_error", &err));
            return 1;
        }
    };
    let input = payload_obj(&payload);
    let result = match command.as_str() {
        "deep-merge" => cli_receipt(
            "policy_runtime_kernel_deep_merge",
            json!({ "ok": true, "value": deep_merge(input.get("base").unwrap_or(&json!({})), input.get("override")) }),
        ),
        "resolve-policy-path" => cli_receipt(
            "policy_runtime_kernel_resolve_policy_path",
            json!({ "ok": true, "policy_path": resolve_policy_path(root, input) }),
        ),
        "load-policy-runtime" => cli_receipt(
            "policy_runtime_kernel_load_policy_runtime",
            json!({ "ok": true, "runtime": load_policy_runtime(root, input) }),
        ),
        "resolve-policy-value-path" => cli_receipt(
            "policy_runtime_kernel_resolve_policy_value_path",
            json!({ "ok": true, "path": resolve_path(root, input.get("raw").and_then(Value::as_str).unwrap_or(""), input.get("fallback_rel").and_then(Value::as_str).unwrap_or("")) }),
        ),
        _ => cli_error(
            "policy_runtime_kernel_error",
...
mod tests {
    ...
    fn deep_merge_replaces_arrays_and_overrides_scalars() {
        let merged = deep_merge(
            &json!({"a":1,"rows":[1],"obj":{"x":1}}),
            Some(&json!({"rows":[2],"obj":{"y":2}})),
        );
