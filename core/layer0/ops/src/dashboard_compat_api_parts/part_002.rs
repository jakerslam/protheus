...
pub fn handle_with_headers(
    ...
    if method == "POST" && path_only == "/api/agents" {
        ...
        let name = dashboard_compat_api_agent_identity::resolve_agent_name(
            root,
            &requested_name,
            &role,
        );
        ...
        let identity = dashboard_compat_api_agent_identity::resolve_agent_identity(
            root, &request, &role,
        );
    ...
    if let Some((requested_agent_id, segments)) = parse_agent_route(path_only) {
        ...
        if method == "POST" && segments.len() == 1 && segments[0] == "clone" {
            ...
            let source_role = clean_text(
                source.get("role").and_then(Value::as_str).unwrap_or("analyst"),
                60,
            );
...
#[cfg(test)]
mod tests {
    include!("part_002_tests_parts/part_000.rs");
    include!("part_002_tests_parts/part_001.rs");
    include!("part_002_tests_parts/part_002.rs");
    include!("part_002_tests_parts/part_003.rs");
}
