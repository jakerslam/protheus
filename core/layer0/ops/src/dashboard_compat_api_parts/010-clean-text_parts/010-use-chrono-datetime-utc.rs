...
const AGENT_RUNTIME_SYSTEM_PROMPT: &str = "You are the currently selected Infring agent instance. Treat the injected identity profile as authoritative for self-identification. Keep responses natural and user-focused.";
...
fn effective_app_settings(root: &Path, snapshot: &Value) -> (String, String) {
    extract_app_settings(root, snapshot)
}
