...
fn run_multi_agent_debate(root: &Path, argv: &[String]) -> i32 {
    ...
    match action.as_str() {
        "run" => {
            ...
            let out = crate::infring_autonomy_core_v1_bridge::run_multi_agent_debate(
                root,
                &payload,
                policy.as_deref(),
                persist,
        ...
        "status" => {
            ...
            let out =
                crate::infring_autonomy_core_v1_bridge::debate_status(root, policy.as_deref(), key.as_deref());
            let ok = out.get("ok").and_then(Value::as_bool).unwrap_or(false);
            print_json_line(&out);
            if ok {
                0
...
fn run_ethical_reasoning(root: &Path, argv: &[String]) -> i32 {
    ...
    match action.as_str() {
        "run" => {
            ...
            let persist = parse_bool(parse_flag(argv, "persist").as_deref(), true);
            let out = crate::infring_autonomy_core_v1_bridge::run_ethical_reasoning(
                root,
                &payload,
                policy.as_deref(),
                state_dir.as_deref(),
        ...
        "status" => {
            let policy = parse_flag(argv, "policy").map(PathBuf::from);
            let state_dir = parse_flag(argv, "state-dir").map(PathBuf::from);
            let out = crate::infring_autonomy_core_v1_bridge::ethical_reasoning_status(
                root,
                policy.as_deref(),
                state_dir.as_deref(),
            );
...
fn run_simulation_harness(root: &Path, argv: &[String]) -> i32 {
    ...
    match action.as_str() {
        "run" | "status" => {
            let out = crate::infring_autonomy_core_v1_bridge::run_autonomy_simulation(
                root,
                date.as_deref(),
                days,
                write,
