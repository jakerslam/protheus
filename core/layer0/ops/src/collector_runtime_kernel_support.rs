...
pub fn split_error_code(raw: &str) -> String {
    lane_utils::clean_text(Some(raw), 120)
        .split(':')
        .next()
    ...
    payload
        ...
        .map(|headers| {
            headers
                ...
                .map(|(k, v)| {
                    (
                        lane_utils::clean_text(Some(k), 120),
                        lane_utils::clean_text(v.as_str(), 500),
                    )
                })
...
pub fn curl_fetch_with_status(
    ...
    cmd.arg("--silent")
        ...
        .arg("-H")
        .arg("Accept: application/rss+xml,application/atom+xml,application/json,text/xml,text/html;q=0.9,*/*;q=0.8");
    for (k, v) in headers {
