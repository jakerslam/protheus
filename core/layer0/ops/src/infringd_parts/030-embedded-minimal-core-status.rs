...
fn embedded_minimal_core_status() -> Value {
    ...
    let mut out = json!({
        "ok": true,
        "type": "infringd_embedded_minimal_core_status",
        "ts": now_iso(),
...
fn tiny_status() -> Value {
    let profile = crate::infring_tiny_runtime_bridge::tiny_profile();
    let capacity = crate::infring_tiny_runtime_bridge::normalized_capacity_score(
        profile.max_heap_kib,
        profile.max_concurrent_hands,
    );
    let mut out = json!({
        "ok": true,
        "type": "infringd_tiny_status",
        "ts": now_iso(),
...
fn main() {
    ...
    #[cfg(feature = "embedded-max")]
    std::env::set_var("INFRING_EMBEDDED_MAX", "1");
    ...
    if let Some(compat) = installer_compat_command(command.as_str()) {
        let exit = match compat {
            ...
            InstallerCompatCommand::DashboardUi => {
                infring_ops_core::dashboard_ui::run(&cwd, &args[1..])
            }
        };
    ...
    match command.as_str() {
        ...
        "efficiency-status" => {
            let parsed = infring_ops_core::parse_args(&[]);
