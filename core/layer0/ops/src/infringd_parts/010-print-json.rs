...
use infring_ops_core::v8_kernel::{parse_bool_str, parse_u64_str};
...
fn clean_text(raw: Option<&str>, max_len: usize) -> String {
    infring_ops_core::contract_lane_utils::clean_text(raw, max_len)
}
...
fn tiny_max_requested(argv: &[String]) -> bool {
    parse_bool_switch(argv, "tiny-max", false)
        ...
        || parse_bool_str(env::var("INFRING_EMBEDDED_MAX").ok().as_deref(), false)
        || cfg!(feature = "embedded-max")
}
...
fn sensed_memory_mb(argv: &[String]) -> u64 {
    ...
    let parsed_env = parse_u64_str(env::var("INFRING_HW_MEMORY_MB").ok().as_deref(), 0);
    if parsed_env > 0 {
...
fn sensed_cpu_cores(argv: &[String]) -> usize {
    ...
    let parsed_env = parse_u64_str(env::var("INFRING_HW_CPU_CORES").ok().as_deref(), 0);
    if parsed_env > 0 {
...
fn runtime_capability_profile(argv: &[String]) -> RuntimeCapabilityProfile {
    ...
    let override_class = parse_hardware_class(
        parse_flag(argv, "hardware-class")
            ...
            .or_else(|| env::var("INFRING_HW_CLASS").ok())
            .as_deref(),
    );
    let hardware_class =
