...
fn top_holes(feedback_rows: &[Value]) -> Value {
    ...
    json!({
        ...
        "holes": rows.into_iter().take(10).collect::<Vec<_>>()
    })
}
...
fn write_daily_report(path: &Path, report: &Value, trend: &Value, top_holes: &Value) -> Result<(), String> {
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent).map_err(|err| err.to_string())?;
    }
    let mut body = String::new();
...
pub(super) fn write_self_study_outputs(dir: &Path, report: &Value) -> Result<Value, String> {
    ...
    let top_holes = top_holes(&feedback_rows);
    ...
    write_daily_report(&daily_report_path, report, &trend_report["delta"], &top_holes)?;
