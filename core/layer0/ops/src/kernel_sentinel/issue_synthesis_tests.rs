...
use crate::kernel_sentinel::{
    KernelSentinelFindingCategory, KernelSentinelSeverity, KERNEL_SENTINEL_FINDING_SCHEMA_VERSION,
};
use std::fs;
...
fn repeated_fingerprint_produces_one_issue_draft() {
    ...
    assert_eq!(
        report["issue_drafts"][0]["occurrence_count"],
        Value::from(2)
    );
    assert_eq!(
    ...
    assert_eq!(
        report["issue_drafts"][0]["todo_actionability"]["todo_ready"],
        true
    );
    assert_eq!(
...
fn cluster_key_collapses_sessions_when_root_frame_and_invariant_match() {
    ...
    first.evidence =
        vec!["gateway://ollama/flap;session=a;surface=gateway;receipt_type=quarantine".to_string()];
    let mut second = first.clone();
    second.evidence =
        vec!["gateway://ollama/flap;session=b;surface=gateway;receipt_type=quarantine".to_string()];
...
fn repeated_visible_symptom_patches_are_flagged_as_structural_loop() {
    ...
    first.summary =
        "visible symptom patch left the taskbar connectivity indicator offline".to_string();
    first.recommended_action = "stop_patching symptoms and repair the gateway boundary".to_string();

    let mut second = first.clone();
    second.id = "finding-2".to_string();
    second.fingerprint = "gateway_isolation:duplicate_dashboard_restart_symptom".to_string();
    second.summary =
        "local patch hid one startup symptom but duplicate gateway restart remained".to_string();
...
fn issue_quality_guard_rejects_advisory_only_vague_drafts() {
    ...
    assert!(failures[0]["reasons"]
        .as_array()
        .unwrap()
        .contains(&Value::from("missing_deterministic_evidence")));
    assert!(failures[0]["reasons"]
        .as_array()
        .unwrap()
        .contains(&Value::from("missing_component")));
    assert!(failures[0]["reasons"]
        .as_array()
        .unwrap()
        .contains(&Value::from("missing_validation_route")));
...
fn issue_draft_carries_todo_specific_repair_contract() {
    ...
    assert_eq!(
        draft["todo_actionability"]["validation_route_present"],
        true
    );
