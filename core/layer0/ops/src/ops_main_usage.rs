pub(super) fn print_usage() {
    ...
    println!("  protheus-ops langgraph-bridge <status|register-graph|checkpoint-run|inspect-state|coordinate-subgraph|record-trace|stream-graph> [--payload-base64=<base64_json>] [--state-path=<path>] [--swarm-state-path=<path>] [--trace-path=<path>]");
