pub(super) fn print_usage() {
    ...
    println!("  protheus-ops stomach-kernel <run|status|rollback|retention|purge> [flags]");
    ...
    println!("  protheus-ops command-permission-kernel <evaluate|match-pattern|extract-pattern> [--payload=<json>|--payload-base64=<base64_json>]");
    println!("  protheus-ops source-comment-filter-kernel <filter|detect-language> [--payload=<json>|--payload-base64=<base64_json>]");
