pub(super) fn print_usage() {
    ...
    println!("  infring-ops kernel-sentinel <run|status|report|auto|schedule|heartbeat|help> [--strict=1|0] [--findings-path=<path>] [--auto-artifact=<path>] [--schedule-artifact=<path>] [--max-runtime-ms=<n>]  # schedule/heartbeat write stale-aware Sentinel self-study artifacts");
