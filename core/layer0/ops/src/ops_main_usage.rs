pub(super) fn print_usage() {
    ...
    println!("  protheus-ops langgraph-bridge <status|register-graph|checkpoint-run|inspect-state|interrupt-run|resume-run|coordinate-subgraph|record-trace|stream-graph> [--payload-base64=<base64_json>] [--state-path=<path>] [--swarm-state-path=<path>] [--trace-path=<path>]");
    println!("  protheus-ops langchain-bridge <status|register-chain|execute-chain|register-middleware|run-deep-agent|register-memory-bridge|recall-memory|import-integration|route-prompt|parse-structured-output|record-trace|checkpoint-run|assimilate-intake> [--payload-base64=<base64_json>] [--state-path=<path>] [--swarm-state-path=<path>]");
