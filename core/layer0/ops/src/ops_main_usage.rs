pub(super) fn print_usage() {
    ...
    println!("  infring-ops web-conduit|browse <status|receipts|fetch|search> [--url=<https://...>] [--query=<terms>] [--human-approved=1] [--summary-only=1] [--limit=<n>]");
    println!("  infring-ops batch-query <query|status|policy> [--source=web] [--query=<terms>] [--aperture=small|medium]");
    println!("  infring-ops context-stacks <create|list|archive|tail-merge|tail-promote|render|batch-class|scheduler-check|status|policy> [flags]");
    ...
    println!("  infring-ops adaptive-layer-store-kernel <paths|is-within-root|resolve-path|read-json|ensure-json|set-json|delete-path> [--payload-base64=<base64_json>]");
    ...
    println!("  infring-ops queue-sqlite-kernel <open|ensure-schema|migrate-history|upsert-item|append-event|insert-receipt|queue-stats> [--payload-base64=<base64_json>]");
    ...
    println!("  infring-ops fixed-microbenchmark <run|status> [--rounds=9] [--warmup-runs=2] [--sample-ms=800] [--work-factor=16] [--workload-id=sha256_fixed_workload_v1]");
    ...
    println!("  infring-ops coverage-badge-kernel [run] [--ts=<path>] [--rust=<path>] [--out-json=<path>] [--out-badge=<path>]");
    ...
    println!("  infring-ops top1-assurance <status|proof-coverage|proof-vm|size-gate|benchmark-thresholds|comparison-matrix|run-all> [flags]");
