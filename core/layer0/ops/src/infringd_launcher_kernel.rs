...
fn usage() {
    println!("infringd-launcher-kernel commands:");
...
fn run_gate(argv: &[String]) -> Value {
    ...
    let strict = std::env::var("INFRING_CONDUIT_STRICT")
        .ok()
        .as_deref()
        .map(str::trim)
        .filter(|raw| !raw.is_empty())
    ...
    let conduit_missing = std::env::var("INFRING_CONDUIT_AVAILABLE")
        .ok()
        .as_deref()
        .map(str::trim)
        .map(|raw| raw == "0")
