...
pub fn run(root: &Path, argv: &[String]) -> i32 {
    ...
    if matches!(command.as_str(), "help" | "--help" | "-h") {
        ...
        println!(
            "  protheus-ops metakernel microkernel-safety [--syscall=<id>] [--allow=<csv>] [--session=<id>] [--instance-dna=<id>] [--step=<n>] [--step-cap=<n>] [--drift=<0..1>] [--drift-threshold=<0..1>] [--strict=1|0]"
        );
    ...
    let payload = match command.as_str() {
        ...
        "microkernel-safety" => run_microkernel_safety(
            root,
            strict,
            syscall_flag,
            session_flag,
...
mod tests {
    ...
    fn microkernel_safety_rejects_unknown_syscall_and_triggers_judicial_lock() {
        ...
        assert_eq!(out.get("ok").and_then(Value::as_bool), Some(false));
        assert_eq!(
            out.pointer("/judicial_lock/triggered").and_then(Value::as_bool),
            Some(true)
        );
        assert!(out
