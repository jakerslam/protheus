...
fn build_pdf_tool_cli_request(parsed: &crate::ParsedArgs) -> Result<Value, String> {
    let pdfs = parse_json_flag(
        parsed
            .flags
            .get("pdfs-json")
            .or_else(|| parsed.flags.get("pdfs_json")),
    )?;
    let mut request = cli_media_request_from_parsed(parsed);
    if let Some(obj) = request.as_object_mut() {
        obj.insert(
            "pdf".to_string(),
            json!(clean_text(
                parsed.flags.get("pdf").map(String::as_str).unwrap_or(""),
                4000
            )),
        );
        obj.insert("pdfs".to_string(), pdfs);
        obj.insert(
            "prompt".to_string(),
            json!(clean_text(
                parsed.flags.get("prompt").map(String::as_str).unwrap_or(""),
        ...
        obj.insert(
            ...
            json!(clean_text(
                parsed
                    .flags
                    .get("provider")
                    .or_else(|| parsed.flags.get("model-provider"))
        ...
        obj.insert(
            "pages".to_string(),
            json!(clean_text(
                parsed.flags.get("pages").map(String::as_str).unwrap_or(""),
                200
            )),
        );
        obj.insert(
            "max_bytes_mb".to_string(),
            json!(parse_u64(
                parsed
                    .flags
                    .get("max-bytes-mb")
                    .or_else(|| parsed.flags.get("max_bytes_mb")),
...
fn run_document_cli_command(
    ...
    match command {
        ...
        "pdf-tool" => match build_pdf_tool_cli_request(parsed) {
            Ok(request) => Ok(Some(api_pdf_tool(root, &request))),
            Err(err) => Err(print_json_error_and_exit(
                "web_conduit_pdf_tool",
                "invalid_pdfs_json",
                &err,
            )),
        },
