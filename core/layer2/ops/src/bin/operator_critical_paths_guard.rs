...
fn main() {
    ...
    let plan =
        arg_value(&args, "plan").unwrap_or_else(|| DEFAULT_NODE_BURNDOWN_PLAN_PATH.to_string());
    let out_json = arg_value(&args, "out-json")
        .unwrap_or_else(|| DEFAULT_OPERATOR_CRITICAL_REPORT_PATH.to_string());
    let out_markdown = arg_value(&args, "out-markdown")
        .unwrap_or_else(|| DEFAULT_OPERATOR_CRITICAL_MARKDOWN_PATH.to_string());
