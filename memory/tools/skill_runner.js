...
async function main() {
  ...
  try {
    ...
    console.error(`  node /Users/jay/.openclaw/workspace/memory/tools/trust_add.js ${skillPath} "approval note"`);
    console.error('');
    process.exit(1);
  }
  ...
  if (hasDirectImport || warnings.length > 0) {
    ...
    console.warn('');
    console.warn('Recommended fix: Use execCompacted() from lib/exec_compacted.js');
    console.warn('');
  }
