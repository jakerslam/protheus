...
const SCRIPT = path.join(ROOT, 'systems/ops/permanent_guidelines_drift_gate.js');
...
try {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'permanent_guidelines_drift_gate-'));
  ...
  out = run(['verify', '--policy=' + policyPath, '--strict=1', '--apply=1', '--fail-checks=canonical_guidelines_artifact']);
  ...
  assert.ok(Array.isArray(out.payload.failed_checks) && out.payload.failed_checks.includes('canonical_guidelines_artifact'), 'failed check id should be listed');
  ...
  console.log('permanent_guidelines_drift_gate.test.js: OK');
} catch (err) {
  console.error('permanent_guidelines_drift_gate.test.js: FAIL: ' + err.message);
  process.exit(1);
}
