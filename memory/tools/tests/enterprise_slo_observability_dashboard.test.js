...
const SCRIPT = path.join(ROOT, 'systems/observability/enterprise_slo_observability_dashboard.js');
...
try {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'enterprise_slo_observability_dashboard-'));
  ...
  let out = run(['slo', '--policy=' + policyPath, '--strict=1', '--apply=1']);
  ...
  out = run(['slo', '--policy=' + policyPath, '--strict=1', '--apply=1', '--fail-checks=slo_contract_definitions']);
  ...
  assert.ok(Array.isArray(out.payload.failed_checks) && out.payload.failed_checks.includes('slo_contract_definitions'), 'failed check id should be listed');
  ...
  console.log('enterprise_slo_observability_dashboard.test.js: OK');
} catch (err) {
  console.error('enterprise_slo_observability_dashboard.test.js: FAIL: ' + err.message);
  process.exit(1);
}
