...
try {
  ...
  writeJson(policyPath, {
    ...
    fail_closed: true,
    venom_script: 'systems/security/venom_containment_layer.js',
    routes: {
      mcp_discover: { script: 'skills/mcp/mcp_gateway.js', args: ['discover', '--query=memory', '--risk-tier=2'] },
      a2a_delegate: { script: 'systems/a2a/a2a_delegation_plane.js', args: ['execute', '--owner=system', '--task=delegate', '--risk-tier=2'] }
    },
    contract_lanes: [
      { id: 'RR-001', script: 'systems/ops/config_flag_conflict_check.js', check_cmd: 'check' }
    ],
    paths: {
