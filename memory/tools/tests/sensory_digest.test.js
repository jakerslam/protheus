...
function runTests() {
  ...
  try {
    ...
    const result = execSync(
      `node habits/scripts/sensory_digest.js daily ${dateStr}`,
  ...
  try {
    ...
    const result = execSync(
      `node habits/scripts/sensory_digest.js weekly ${dateStr}`,
      { cwd: workspaceRoot, encoding: 'utf8' }
    );
  ...
  try {
    ...
    execSync(
      `node habits/scripts/sensory_digest.js daily ${anomalyDateStr}`,
      { cwd: workspaceRoot, encoding: 'utf8' }
    );
