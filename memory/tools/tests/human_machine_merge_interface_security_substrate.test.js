...
const SCRIPT = path.join(ROOT, 'systems/continuity/human_machine_merge_interface_security_substrate.js');
...
try {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'human_machine_merge_interface_security_substrate-'));
  ...
  let out = run(['merge', '--policy=' + policyPath, '--strict=1', '--apply=1']);
  ...
  out = run(['merge', '--policy=' + policyPath, '--strict=1', '--apply=1', '--fail-checks=consent_ceremony_gate']);
  ...
  assert.ok(Array.isArray(out.payload.failed_checks) && out.payload.failed_checks.includes('consent_ceremony_gate'), 'failed check id should be listed');
  ...
  console.log('human_machine_merge_interface_security_substrate.test.js: OK');
} catch (err) {
  console.error('human_machine_merge_interface_security_substrate.test.js: FAIL: ' + err.message);
  process.exit(1);
}
