...
const SCRIPT = path.join(ROOT, 'systems/ops/black_swan_disaster_gamebooks.js');
...
try {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'black_swan_disaster_gamebooks-'));
  ...
  let out = run(['drill', '--policy=' + policyPath, '--strict=1', '--apply=1']);
  ...
  out = run(['drill', '--policy=' + policyPath, '--strict=1', '--apply=1', '--fail-checks=scenario_library']);
  ...
  assert.ok(Array.isArray(out.payload.failed_checks) && out.payload.failed_checks.includes('scenario_library'), 'failed check id should be listed');
  ...
  console.log('black_swan_disaster_gamebooks.test.js: OK');
} catch (err) {
  console.error('black_swan_disaster_gamebooks.test.js: FAIL: ' + err.message);
  process.exit(1);
}
