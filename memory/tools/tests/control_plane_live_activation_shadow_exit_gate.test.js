...
const SCRIPT = path.join(ROOT, 'systems/ops/control_plane_live_activation_shadow_exit_gate.js');
...
try {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'control_plane_live_activation_shadow_exit_gate-'));
  ...
  out = run(['activate', '--policy=' + policyPath, '--strict=1', '--apply=1', '--fail-checks=protheus_control_plane_live']);
  ...
  assert.ok(Array.isArray(out.payload.failed_checks) && out.payload.failed_checks.includes('protheus_control_plane_live'), 'failed check id should be listed');
  ...
  console.log('control_plane_live_activation_shadow_exit_gate.test.js: OK');
} catch (err) {
  console.error('control_plane_live_activation_shadow_exit_gate.test.js: FAIL: ' + err.message);
  process.exit(1);
}
