...
test('safe: read file in workspace => ALLOW', () => {
  const result = evaluateTask('Read the file ~/.openclaw/workspace/config/settings.json');
...
test('safe: list files in habits directory => ALLOW', () => {
  const result = evaluateTask('List all files in the habits/scripts directory');
  assert.strictEqual(result.decision, 'ALLOW');
