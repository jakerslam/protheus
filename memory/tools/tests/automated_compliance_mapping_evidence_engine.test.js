...
const SCRIPT = path.join(ROOT, 'systems/ops/automated_compliance_mapping_evidence_engine.js');
...
try {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'automated_compliance_mapping_evidence_engine-'));
  ...
  let out = run(['map', '--policy=' + policyPath, '--strict=1', '--apply=1']);
  ...
  out = run(['map', '--policy=' + policyPath, '--strict=1', '--apply=1', '--fail-checks=control_evidence_mapping']);
  ...
  assert.ok(Array.isArray(out.payload.failed_checks) && out.payload.failed_checks.includes('control_evidence_mapping'), 'failed check id should be listed');
  ...
  console.log('automated_compliance_mapping_evidence_engine.test.js: OK');
} catch (err) {
  console.error('automated_compliance_mapping_evidence_engine.test.js: FAIL: ' + err.message);
  process.exit(1);
}
