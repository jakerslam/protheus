...
function testFileChangeSpikeProposal() {
  ...
  try {
    ...
    const result = execSync(
      `node habits/scripts/sensory_insight.js daily ${testDate} 2>&1`,
      {
        cwd: workspaceRoot,
        encoding: 'utf8',
...
function testLowSignalProposal() {
  ...
  try {
    ...
    execSync(
      `node habits/scripts/sensory_insight.js daily ${testDate}`,
      {
        cwd: workspaceRoot,
        encoding: 'utf8',
        env: { ...process.env, SENSORY_TEST_DIR: TEST_DIR }
...
function testHighChurnProposal() {
  ...
  try {
      ...
      anomalies: [
        {
          ...
          severity: 'medium',
          message: 'config/settings.js modified across 4 days',
          days_active: 4
        }
      ],
      metrics: {
    ...
    assert.ok(churnProposal.title.includes('churn') || churnProposal.title.includes('config/settings'),
...
function testWeeklyAggregation() {
  ...
  try {
    ...
    // v1.2.0: Must pass SENSORY_TEST_DIR env var
    execSync(
      `node habits/scripts/sensory_insight.js weekly ${testDate}`,
      {
        cwd: workspaceRoot,
        encoding: 'utf8',
        env: { ...process.env, SENSORY_TEST_DIR: TEST_DIR }
