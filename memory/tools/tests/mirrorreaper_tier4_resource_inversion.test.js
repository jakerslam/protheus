...
const SCRIPT = path.join(ROOT, 'systems/security/mirrorreaper_tier4_resource_inversion.js');
...
try {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'mirrorreaper_tier4_resource_inversion-'));
  ...
  console.log('mirrorreaper_tier4_resource_inversion.test.js: OK');
} catch (err) {
  console.error('mirrorreaper_tier4_resource_inversion.test.js: FAIL: ' + err.message);
  process.exit(1);
}
