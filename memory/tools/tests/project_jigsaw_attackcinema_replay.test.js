...
const SCRIPT = path.join(ROOT, 'systems/security/jigsaw/attackcinema_replay_theater.js');
...
try {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'project_jigsaw_attackcinema_replay-'));
  ...
  let out = run(['capture', '--policy=' + policyPath, '--strict=1', '--apply=1']);
  ...
  out = run(['capture', '--policy=' + policyPath, '--strict=1', '--apply=1', '--fail-checks=recording_engine_live']);
  ...
  assert.ok(Array.isArray(out.payload.failed_checks) && out.payload.failed_checks.includes('recording_engine_live'), 'failed check id should be listed');
  ...
  console.log('project_jigsaw_attackcinema_replay.test.js: OK');
} catch (err) {
  console.error('project_jigsaw_attackcinema_replay.test.js: FAIL: ' + err.message);
  process.exit(1);
}
