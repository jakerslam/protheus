 ...
 * 
 * Run: node memory/tools/tests/dopamine_engine.test.js
 */
...
// Load engine from habits/scripts
const {
  calculateSDS,
  logWorkEntry,
  calculateDailyScore,
...
function runTests() {
  ...
  try {
    ...
    // Add revenue action
    const output = execSync(
      'node habits/scripts/dopamine_engine.js revenue lead "Acme Corp"',
      { cwd: path.join(__dirname, '..', '..', '..'), encoding: 'utf8' }
    );
  ...
  try {
    ...
    // Tap switch twice
    execSync(
      'node habits/scripts/dopamine_engine.js switch',
      { cwd: path.join(__dirname, '..', '..', '..'), encoding: 'utf8' }
    );
    execSync(
      'node habits/scripts/dopamine_engine.js switch',
  ...
  try {
    ...
    // Test dop score
    const output = execSync(
      'PATH="$HOME/.local/bin:$PATH" node habits/scripts/dop score',
      { cwd: workspace, encoding: 'utf8', shell: '/bin/zsh' }
    );
  ...
  try {
    ...
    // Run via dop wrapper
    execSync(
      'PATH="$HOME/.local/bin:$PATH" node habits/scripts/dop switch',
      { cwd: path.join(__dirname, '..', '..', '..'), encoding: 'utf8', shell: '/bin/zsh' }
    );
  ...
  try {
    ...
    // Run via dop wrapper
    execSync(
      'PATH="$HOME/.local/bin:$PATH" node habits/scripts/dop revenue invoice "Test Client"',
      { cwd: path.join(__dirname, '..', '..', '..'), encoding: 'utf8', shell: '/bin/zsh' }
    );
  ...
  try {
    ...
    assert.strictEqual(shouldSkipFile('logs/test.log'), true, 'Should skip logs/ files');
    assert.strictEqual(shouldSkipFile('config/trusted_skills.json'), true, 'Should skip trusted_skills.json');
  ...
  try {
    ...
    // Run closeout
    const output = execSync(
      'PATH="$HOME/.local/bin:$PATH" node habits/scripts/dop closeout',
      { cwd: workspace, encoding: 'utf8', shell: '/bin/zsh', timeout: 30000 }
    );
  ...
  try {
    ...
    assert.strictEqual(shouldSkipFile('state/daily_logs/2026-02-17.json'), true, 'Should skip state/ files');
    assert.strictEqual(shouldSkipFile('logs/test.log'), true, 'Should skip logs/ files');
    assert.strictEqual(shouldSkipFile('src/code.js'), false, 'Should not skip normal src files');
    
    console.log(`   ✅ Recursion guard works: state/, logs/ excluded`);
  } catch (err) {
    console.error('   ❌ FAILED:', err.message);
    throw err;
  }
