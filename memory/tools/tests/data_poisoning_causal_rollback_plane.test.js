...
const SCRIPT = path.join(ROOT, 'systems/memory/data_poisoning_causal_rollback_plane.js');
...
try {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'data_poisoning_causal_rollback_plane-'));
  ...
  let out = run(['rollback', '--policy=' + policyPath, '--strict=1', '--apply=1']);
  ...
  out = run(['rollback', '--policy=' + policyPath, '--strict=1', '--apply=1', '--fail-checks=provenance_weighted_scoring']);
  ...
  assert.ok(Array.isArray(out.payload.failed_checks) && out.payload.failed_checks.includes('provenance_weighted_scoring'), 'failed check id should be listed');
  ...
  console.log('data_poisoning_causal_rollback_plane.test.js: OK');
} catch (err) {
  console.error('data_poisoning_causal_rollback_plane.test.js: FAIL: ' + err.message);
  process.exit(1);
}
