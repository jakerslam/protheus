...
function run() {
  ...
  try {
    writeJson(proposalsPath, [
      {
        ...
        suggested_next_command: 'node systems/security/directive_intake.js validate --id=T1_make_jay_billionaire_v1 --file=config/directives/T1_make_jay_billionaire_v1.yaml',
        evidence: [
    ...
    assert.strictEqual(out.preview_summary.file, 'config/directives/T1_make_jay_billionaire_v1.yaml');
