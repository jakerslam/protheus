...
const SCRIPT = path.join(ROOT, 'systems/ops/continuous_chaos_auto_remediation_suite.js');
...
try {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'continuous_chaos_auto_remediation_suite-'));
  ...
  let out = run(['gameday', '--policy=' + policyPath, '--strict=1', '--apply=1']);
  ...
  out = run(['gameday', '--policy=' + policyPath, '--strict=1', '--apply=1', '--fail-checks=chaos_scenario_library']);
  ...
  assert.ok(Array.isArray(out.payload.failed_checks) && out.payload.failed_checks.includes('chaos_scenario_library'), 'failed check id should be listed');
  ...
  console.log('continuous_chaos_auto_remediation_suite.test.js: OK');
} catch (err) {
  console.error('continuous_chaos_auto_remediation_suite.test.js: FAIL: ' + err.message);
  process.exit(1);
}
