...
const CONFIG_PATH = '/Users/jay/.openclaw/workspace/config/trusted_skills.json';
...
function verifySkillOrThrow(targetPath) {
  ...
  if (!isPathAllowlisted(resolvedPath, config.allowlist_roots)) {
    throw new Error(
      ...
      `To approve: node /Users/jay/.openclaw/workspace/memory/tools/trust_add.js ${resolvedPath} "approval note"`
    );
  }
  ...
  if (currentHash !== trustedEntry.sha256) {
    throw new Error(
      ...
      `To re-approve: node /Users/jay/.openclaw/workspace/memory/tools/trust_add.js ${resolvedPath} "re-approval note"`
    );
  }
