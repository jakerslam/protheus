...
| BL-021 | todo | End-to-end pipeline integration tests (`eyes -> insight -> queue -> execute -> receipt -> score`) | Catch regressions that unit tests miss in autonomous flow handoffs | CI runs deterministic E2E scenarios with fixtures and fails on contract/semantic drift across stage boundaries |
| BL-022 | todo | Two-phase autonomous change execution with automatic rollback | Prevent partial/bad self-changes from persisting | `plan -> apply -> verify -> commit` workflow with health checks; failed verify auto-reverts and logs receipt with root cause |
| BL-023 | todo | Unified global budget governor across reflex/autonomy/focus/dream/spawn | Prevent resource contention and runaway loops under concurrent adaptive workloads | One central budget API enforces hard caps (CPU/RAM/tokens/time) and priority-based throttling with auditable deny events |
| BL-024 | todo | Event/state schema versioning + validators + migrations | Eliminate silent JSON/JSONL contract drift over time | All persisted contracts carry `schema_version`; reads validate; controlled migrations exist for version bumps |
