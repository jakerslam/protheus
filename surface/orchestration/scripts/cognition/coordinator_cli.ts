...
function runCoordinatorCli(argv = [], api = {}) {
  ...
  const assignScopesToPartitions = typeof api.assignScopesToPartitions === 'function' ? api.assignScopesToPartitions : null;
  const detectScopeOverlaps = typeof api.detectScopeOverlaps === 'function' ? api.detectScopeOverlaps : null;
  ...
  if (!runCoordinator || !partitionWork || !assignScopesToPartitions || !detectScopeOverlaps || !loadScratchpad || !handleTimeout) {
    return {
      ok: false,
  ...
  if (command === 'partition') {
    ...
    const agentCount = Number(parsed.flags['agent-count'] || parsed.flags.agent_count || 1);
    const scopeCheck = detectScopeOverlaps(scopesPayload.value);
    if (!scopeCheck.ok) {
      return {
        ok: false,
        type: 'orchestration_partition',
    ...
    const partitions = assignScopesToPartitions(partitionWork(itemsPayload.value, agentCount), scopeCheck.normalized_scopes);
    return {
      ok: true,
      type: 'orchestration_partition',
      partitions
