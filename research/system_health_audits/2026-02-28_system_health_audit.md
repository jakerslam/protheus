...
| 5. Primitive-First Purity | 6.5 | Core strict roots are TS-paired; advisory JS holdouts remain high in `habits/scripts` + `memory/tools/tests` (430 unpaired) |
...
- Advisory roots still large JS surface (430 files) in habits/tests tooling.
- Action:
  1. Split advisory JS migration into measurable waves (habits first, then memory/tools tests).
...
10. Start advisory JS migration wave for `habits/scripts` and `memory/tools`.
