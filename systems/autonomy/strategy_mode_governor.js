...
const { loadActiveStrategy, strategyExecutionMode, strategyPromotionPolicy } = require('../../lib/strategy_resolver');
...
const MODE_GOVERNOR_CANARY_DISABLE_LEGACY_FALLBACK_AFTER_QUALITY_RECEIPTS = Number(process.env.AUTONOMY_MODE_GOVERNOR_CANARY_DISABLE_LEGACY_FALLBACK_AFTER_QUALITY_RECEIPTS || 10);
const MODE_GOVERNOR_CANARY_MAX_SUCCESS_CRITERIA_QUALITY_INSUFFICIENT_RATE = Number(process.env.AUTONOMY_MODE_GOVERNOR_CANARY_MAX_SUCCESS_CRITERIA_QUALITY_INSUFFICIENT_RATE || 0.4);
...
const MODE_GOVERNOR_REQUIRE_QUALITY_LOCK_FOR_EXECUTE = String(process.env.AUTONOMY_MODE_GOVERNOR_REQUIRE_QUALITY_LOCK_FOR_EXECUTE || '1') !== '0';
...
const MODE_GOVERNOR_CANARY_RELAX_READINESS_CHECKS = new Set(String(process.env.AUTONOMY_CANARY_RELAX_READINESS_CHECKS || 'success_criteria_pass_rate')
    .split(',')
    .map((v) => String(v || '').trim())
    .filter(Boolean));
...
function canaryFailedChecksAllowed(failedChecks) {
    const failed = Array.isArray(failedChecks)
        ...
        : [];
    if (!failed.length || MODE_GOVERNOR_CANARY_RELAX_READINESS_CHECKS.size === 0)
        return false;
    for (const check of failed) {
        if (!MODE_GOVERNOR_CANARY_RELAX_READINESS_CHECKS.has(check))
            return false;
    }
    return true;
}
...
function governorPolicy() {
    return {
        ...
        canary_disable_legacy_fallback_after_quality_receipts: Math.max(0, Number(MODE_GOVERNOR_CANARY_DISABLE_LEGACY_FALLBACK_AFTER_QUALITY_RECEIPTS || 0)),
        canary_max_success_criteria_quality_insufficient_rate: Math.max(0, Math.min(1, Number(MODE_GOVERNOR_CANARY_MAX_SUCCESS_CRITERIA_QUALITY_INSUFFICIENT_RATE || 1))),
...
function lastModeChangeEvent(strategyId) {
    ...
    for (let i = rows.length - 1; i >= 0; i--) {
        ...
        if (t !== 'strategy_mode_change' && t !== 'strategy_mode_auto_change' && t !== 'strategy_mode_auto_revert')
            continue;
        if (strategyId && String(row.strategy_id || '') !== String(strategyId))
            continue;
        return row;
    }
    return null;
...
function cooldownState(last, minHours) {
    ...
    if (!last || !last.ts || !Number.isFinite(minHours) || minHours <= 0)
        return out;
    ...
    if (!Number.isFinite(ageMs) || ageMs < 0 || ageMs >= minMs)
        return out;
    out.active = true;
...
function canaryMetrics(summary, policy, qualityLock) {
    ...
    const criteriaQualityInsufficientReceipts = Number(summary?.receipts?.autonomy?.success_criteria_quality_insufficient_receipts
...
function runPolicyRootAuthorize({ scope, target, leaseToken, approvalNote, source }) {
    ...
    if (leaseToken)
        args.push(`--lease-token=${String(leaseToken).trim()}`);
    if (source)
        args.push(`--source=${String(source).trim()}`);
...
function spcAllowsEscalation(spc, policy) {
    if (!policy || policy.require_spc !== true)
        return true;
...
function computeStreakUpdate(currentMode, readiness, canary, policy, spc, prevStreak) {
    ...
    if (mode === 'score_only')
        escalateReady = rs.ready_for_canary && previewReady && spcReady;
    else if (mode === 'canary_execute')
        escalateReady = rs.ready_for_execute && executeReady && spcReady;
...
function buildStatus(dateStr, days, strategy, policy, prevStreak) {
    ...
    const transitionBlockReason = (mode === 'score_only'
        && readinessEval.ready_for_canary === true
        && canary
        && canary.preview_ready_for_canary === false) ? 'preview_success_criteria_below_min'
