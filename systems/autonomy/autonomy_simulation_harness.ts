...
const identityAnchor = require('../identity/identity_anchor');

function fallbackSummarizeIdentityEvaluations(rows, threshold) {
  ...
  return {
    total,
    blocked,
    passed: Math.max(0, total - blocked),
    threshold: Number(threshold || 0.58)
  };
}

function fallbackLoadIdentityContext() {
  const blockUnknown = SIM_IDENTITY_BLOCK_UNKNOWN_OBJECTIVE != null
    ? SIM_IDENTITY_BLOCK_UNKNOWN_OBJECTIVE === true
    : String(process.env.SPINE_IDENTITY_BLOCK_UNKNOWN_OBJECTIVE || '1').trim() !== '0';
  return {
    policy_path: SIM_IDENTITY_POLICY_PATH || null,
    active_objective_ids: ['T1_make_jay_billionaire_v1'],
    policy: {
      max_identity_drift_score: 0.58,
      enforcement: {
        block_on_unknown_active_objective: blockUnknown
      }
    }
  };
}

function fallbackEvaluateWorkflowDraft(candidate, opts) {
  const context = opts && opts.context && typeof opts.context === 'object' ? opts.context : {};
  const policy = context.policy && typeof context.policy === 'object' ? context.policy : {};
  const enforcement = policy.enforcement && typeof policy.enforcement === 'object'
    ? policy.enforcement
    : {};
  const blockUnknown = enforcement.block_on_unknown_active_objective === true;
  const objectiveId = normalizeText(candidate && candidate.objective_id || '');
  const activeObjectiveIds = new Set(
    Array.isArray(context.active_objective_ids)
      ? context.active_objective_ids.map((id) => normalizeText(id)).filter(Boolean)
      : []
  );
  const unknownObjective = objectiveId && !activeObjectiveIds.has(objectiveId);
  const blocked = Boolean(blockUnknown && (!objectiveId || unknownObjective));
  return {
    blocked,
    blocking_codes: blocked ? ['unknown_active_objective'] : [],
    objective_id: objectiveId || null
  };
}

const loadIdentityContext = typeof identityAnchor.loadIdentityContext === 'function'
  ? identityAnchor.loadIdentityContext
  : fallbackLoadIdentityContext;
const evaluateWorkflowDraft = typeof identityAnchor.evaluateWorkflowDraft === 'function'
  ? identityAnchor.evaluateWorkflowDraft
  : fallbackEvaluateWorkflowDraft;
const summarizeIdentityEvaluations = typeof identityAnchor.summarizeIdentityEvaluations === 'function'
  ? identityAnchor.summarizeIdentityEvaluations
  : fallbackSummarizeIdentityEvaluations;
