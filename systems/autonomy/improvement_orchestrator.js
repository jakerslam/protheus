 ...
 *
 * Systems-layer orchestrator over generic systems controllers:
 ...
 *
 * Extra safeguards:
 * - Commits touching systems/config require explicit --allow-system-scope=1
 * - System/config scope also requires --approval-note and --auto-revert=1
 *
 * Commands:
 *   node systems/autonomy/improvement_orchestrator.js propose [YYYY-MM-DD] [--days=N]
 *   node systems/autonomy/improvement_orchestrator.js queue
 *   node systems/autonomy/improvement_orchestrator.js start-next [YYYY-MM-DD] [--id=<lane_id>] --commit=<sha> [--trial-days=N] [--scorecard-days=N] [--auto-revert=1] [--allow-system-scope=1] [--approval-note=...] [--dry-run]
 *   node systems/autonomy/improvement_orchestrator.js evaluate-open [YYYY-MM-DD] [--trial-id=<id>] [--id=<lane_id>] [--force=1] [--auto-revert=1] [--dry-run]
 *   node systems/autonomy/improvement_orchestrator.js run-daily [YYYY-MM-DD] [--commit=<sha>] [--auto-revert=1] [--allow-system-scope=1] [--approval-note=...] [--dry-run]
 */
...
function commitTouchedPaths(commit) {
  const r = runGit(['show', '--name-only', '--pretty=format:', '--diff-filter=ACMRTUXB', commit]);
...
function classifyCommitScope(paths) {
  const p = Array.isArray(paths) ? paths : [];
  const systems = p.filter(x => x.startsWith('systems/'));
  const config = p.filter(x => x.startsWith('config/'));
  const governance = p.filter(x =>
    x.startsWith('systems/security/')
    || x.startsWith('systems/spine/')
    || x.startsWith('systems/autonomy/improvement_controller.js')
    || x.startsWith('systems/autonomy/improvement_orchestrator.js')
  );
  return {
    total_paths: p.length,
    systems_paths: systems.slice(0, 20),
    config_paths: config.slice(0, 20),
    governance_paths: governance.slice(0, 20),
    touches_systems: systems.length > 0,
    touches_config: config.length > 0,
    touches_governance: governance.length > 0,
    requires_system_scope_approval: systems.length > 0 || config.length > 0
  };
}
...
function startNextCmd(dateStr) {
  ...
  const allowSystemScope = optEnabled('allow-system-scope', false);
  const approvalNote = String(optValue('approval-note', '') || '').trim().slice(0, 240);
  const dryRun = optEnabled('dry-run', false);
  const touchedPaths = commitTouchedPaths(commit);
  const scope = classifyCommitScope(touchedPaths);

  if (scope.requires_system_scope_approval && !allowSystemScope) {
    process.stdout.write(JSON.stringify({
      ok: false,
      error: 'system_scope_requires_allow_system_scope_flag',
      lane_id: item.id,
      commit,
      scope,
      ts: nowIso()
  ...
  if (scope.requires_system_scope_approval && approvalNote.length < 12) {
    process.stdout.write(JSON.stringify({
      ok: false,
      error: 'system_scope_requires_approval_note',
      lane_id: item.id,
      commit,
      scope,
      ts: nowIso()
  ...
  if (scope.requires_system_scope_approval && !autoRevert) {
    process.stdout.write(JSON.stringify({
      ok: false,
      error: 'system_scope_requires_auto_revert',
      lane_id: item.id,
      commit,
      scope,
      ts: nowIso()
  ...
  const args = [
    ...
    `--note=lane:${item.id}:${item.strategy}${approvalNote ? `:${approvalNote}` : ''}`
  ];
  ...
  item.scope = scope;
  item.approval_note = approvalNote || null;
...
function runDailyCmd(dateStr) {
  ...
  const evalRes = runNodeJsonLoose(SELF_PATH, evalArgs);
  ...
  const proposeRes = runNodeJsonLoose(SELF_PATH, proposeArgs);
  ...
  if (commit) {
    ...
    if (optEnabled('allow-system-scope', false)) startArgs.push('--allow-system-scope=1');
    const approvalNote = String(optValue('approval-note', '') || '').trim();
    if (approvalNote) startArgs.push(`--approval-note=${approvalNote}`);
    if (dryRun) startArgs.push('--dry-run');
    const startRes = runNodeJsonLoose(SELF_PATH, startArgs);
...
function usage() {
  console.log('Usage:');
  console.log('  node systems/autonomy/improvement_orchestrator.js propose [YYYY-MM-DD] [--days=N]');
  console.log('  node systems/autonomy/improvement_orchestrator.js queue');
  console.log('  node systems/autonomy/improvement_orchestrator.js start-next [YYYY-MM-DD] [--id=<lane_id>] --commit=<sha> [--trial-days=N] [--scorecard-days=N] [--auto-revert=1] [--allow-system-scope=1] [--approval-note=...] [--dry-run]');
  console.log('  node systems/autonomy/improvement_orchestrator.js evaluate-open [YYYY-MM-DD] [--trial-id=<id>] [--id=<lane_id>] [--force=1] [--auto-revert=1] [--dry-run]');
  console.log('  node systems/autonomy/improvement_orchestrator.js run-daily [YYYY-MM-DD] [--commit=<sha>] [--auto-revert=1] [--allow-system-scope=1] [--approval-note=...] [--dry-run]');
}
