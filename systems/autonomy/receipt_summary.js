...
const SUCCESS_CRITERIA_MIN_CONTRACT_VERSION = String(process.env.AUTONOMY_SUCCESS_CRITERIA_MIN_CONTRACT_VERSION || '1.0').trim() || '1.0';
const SUCCESS_CRITERIA_MAX_UNKNOWN_RATE = Math.max(0, Math.min(1, Number(process.env.AUTONOMY_SUCCESS_CRITERIA_MAX_UNKNOWN_RATE || 0.4)));
const SUCCESS_CRITERIA_MAX_UNSUPPORTED_RATE = Math.max(0, Math.min(1, Number(process.env.AUTONOMY_SUCCESS_CRITERIA_MAX_UNSUPPORTED_RATE || 0.5)));
function nowIso() {
...
function assessCriteriaQuality(rec, criteria) {
    ...
    if (criteria && criteria.synthesized === true)
        reasons.push('synthesized_criteria');
    if (unknownRate > SUCCESS_CRITERIA_MAX_UNKNOWN_RATE)
        reasons.push('high_unknown_rate');
    if (unsupportedRate > SUCCESS_CRITERIA_MAX_UNSUPPORTED_RATE)
        reasons.push('high_unsupported_rate');
    if (verification.criteria_quality_insufficient === true)
        reasons.push('criteria_quality_insufficient_flag');
...
function summarizeRuns(rows) {
    ...
    const nonActionable = runs.filter(r => {
        const res = String(r.result || '');
        if (res === 'score_only_preview' || res === 'score_only_evidence')
            return true;
        return res.startsWith('stop_init_gate_') || res.startsWith('init_gate_');
    });
    const qualityScopeRuns = runs.filter(r => {
        ...
        if (res === 'score_only_preview' || res === 'score_only_evidence')
            return false;
        if (res.startsWith('stop_init_gate_') || res.startsWith('init_gate_'))
            return false;
        return true;
    });
    ...
    for (const run of runs) {
        ...
        if (!row._pulse_score_total)
            row._pulse_score_total = 0;
        if (!row._pulse_score_samples)
            row._pulse_score_samples = 0;
        if (!row._allocation_total)
            row._allocation_total = 0;
        if (!row._allocation_samples)
            row._allocation_samples = 0;
        const pulse = run.directive_pulse && typeof run.directive_pulse === 'object'
            ? run.directive_pulse
            : null;
    ...
    const objectiveScorecardSorted = Object.fromEntries(Object.entries(objectiveScorecard)
        .map(([objectiveId, row]) => {
...
function autonomyPrimaryFailure(rec) {
    ...
    if (v && v.primary_failure)
        return String(v.primary_failure);
    if (Array.isArray(v.failed) && v.failed.length)
        return String(v.failed[0]);
    if (rec.error && rec.error.code)
        return String(rec.error.code);
    if (rec.error)
        return String(rec.error);
    if (rec.verdict && String(rec.verdict).toLowerCase() === 'fail')
...
function summarizeAutonomyReceipts(rows) {
    ...
    for (const rec of receipts) {
        ...
        if (objectiveId) {
            ...
            bucket.total += 1;
            if (String(rec.verdict || '').toLowerCase() === 'pass')
                bucket.pass += 1;
            else if (String(rec.verdict || '').toLowerCase() === 'fail')
                bucket.fail += 1;
        }
        ...
        if (criteria.synthesized === true)
            criteriaSynthesizedReceipts += 1;
        criteriaReceipts += 1;
        if (criteria.required === true)
            criteriaRequiredReceipts += 1;
        if (criteria.passed === true)
            criteriaReceiptPass += 1;
        if (isPreview) {
            previewCriteriaReceipts += 1;
            if (criteria.passed === true)
                previewCriteriaPass += 1;
        }
        if (quality.quality_valid === true) {
            qualityCriteriaReceipts += 1;
            if (criteria.passed === true)
                qualityCriteriaPass += 1;
            if (isPreview) {
                previewQualityCriteriaReceipts += 1;
                if (criteria.passed === true)
                    previewQualityCriteriaPass += 1;
            }
        }
        ...
        if (objectiveId) {
            ...
            if (criteria.passed === true)
                bucket.success_criteria_pass += 1;
        }
    }
    const byObjectiveSorted = Object.fromEntries(Object.entries(byObjective)
        .map(([id, row]) => [id, {
...
function actuationFailureReason(rec) {
    ...
    if (rec.error && rec.error.code)
        return String(rec.error.code);
    if (rec.error && rec.error.message)
        return String(rec.error.message);
    if (rec.error)
        return String(rec.error);
    if (rec.ok === false)
        return 'unknown_failure';
...
function summarizeActuationReceipts(rows) {
    ...
    for (const r of receipts) {
        ...
        if (!byAdapter[adapter])
            byAdapter[adapter] = { total: 0, ok: 0, verified: 0 };
        byAdapter[adapter].total += 1;
        if (r && r.ok === true)
            byAdapter[adapter].ok += 1;
        if (r && r.receipt_contract && r.receipt_contract.verified === true)
            byAdapter[adapter].verified += 1;
    }
    const failures = tallyBy(receipts.filter(r => r && r.ok !== true), actuationFailureReason);
    const byAdapterSorted = Object.fromEntries(Object.entries(byAdapter).sort((a, b) => {
        const bt = Number(b[1] && b[1].total || 0);
        const at = Number(a[1] && a[1].total || 0);
        if (bt !== at)
            return bt - at;
    ...
    return {
        ...
        receipts: {
            ...
            combined: {
                ...
                top_failure_reasons: mergeTallies(autonomyReceipts.top_failure_reasons, actuationReceipts.top_failure_reasons)
            }
        }
    };
}
