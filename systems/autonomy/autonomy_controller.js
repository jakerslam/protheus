...
const AUTONOMY_HUMAN_ESCALATION_DEDUPE_MINUTES = Math.max(
  1,
  Number(process.env.AUTONOMY_HUMAN_ESCALATION_DEDUPE_MINUTES || 20)
);
...
function isRecentTsWithin(ts, minutes) {
  ...
  return ageMs >= 0 && ageMs <= (Math.max(1, Number(minutes || 1)) * 60000);
}

function latestAutonomyRunByResult(events, result) {
  const target = String(result || '');
  if (!target) return null;
  for (let i = (Array.isArray(events) ? events.length : 0) - 1; i >= 0; i -= 1) {
    const evt = events[i];
    if (!evt || evt.type !== 'autonomy_run') continue;
    if (String(evt.result || '') !== target) continue;
    return evt;
  }
  return null;
}
...
function isScoreOnlyResult(result) {
  ...
  return r === 'score_only_preview'
    ...
    || r === 'no_candidates_policy_preview_structural_hold'
...
function isScoreOnlyFailureLikeEvent(evt) {
  ...
  if (evt.result === 'no_candidates_policy_preview_structural_hold') return true;
...
function subDirectiveV2SignalsForProposal(proposal, opts = {}) {
...
function runProposalQueueReconcile(days = 2, dryRun = false) {
  const script = path.join(REPO_ROOT, 'habits', 'scripts', 'proposal_queue.js');
  const args = [script, 'reconcile', `--days=${Math.max(1, Number(days || 2))}`];
...
function objectiveIdFromRunEvent(evt) {
  if (!evt || evt.type !== 'autonomy_run' || evt.result !== 'executed') return '';
  const pulse = evt.directive_pulse && typeof evt.directive_pulse === 'object' ? evt.directive_pulse : {};
  const candidates = [pulse.objective_id, evt.objective_id, evt.directive_objective_id];
  for (const row of candidates) {
    const id = sanitizeDirectiveObjectiveId(row);
    if (id) return id;
  }
...
function objectiveIdForCandidate(cand) {
  const row = cand && typeof cand === 'object' ? cand : {};
  const pulse = row.directive_pulse && typeof row.directive_pulse === 'object' ? row.directive_pulse : {};
  const binding = row.objective_binding && typeof row.objective_binding === 'object' ? row.objective_binding : {};
  ...
  for (const rowValue of candidates) {
    const id = sanitizeDirectiveObjectiveId(rowValue);
    if (id) return id;
  }
  return '';
}

function objectiveMixStatsForWindow(endDateStr, days) {
  const windowDays = Math.max(1, Number(days || AUTONOMY_OBJECTIVE_MIX_WINDOW_DAYS));
  const counts = {};
  let total = 0;
  for (const d of dateWindow(endDateStr, windowDays)) {
    for (const evt of readRuns(d)) {
      const id = objectiveIdFromRunEvent(evt);
      if (!id) continue;
      counts[id] = Number(counts[id] || 0) + 1;
      total += 1;
    }
  }
  return { window_days: windowDays, total, counts };
}

function assessObjectiveMixGovernor(endDateStr, eligible) {
  const candidates = Array.isArray(eligible) ? eligible : [];
  if (!AUTONOMY_OBJECTIVE_MIX_ENABLED) {
    return { applied: false, reason: 'disabled', preferred_objectives: [] };
  }
  if (candidates.length < 2) {
    return { applied: false, reason: 'insufficient_eligible_candidates', preferred_objectives: [] };
  }
  const objectiveSet = new Set();
  for (const cand of candidates) {
    const id = objectiveIdForCandidate(cand);
    if (id) objectiveSet.add(id);
  }
  if (objectiveSet.size < 2) {
    return {
      applied: false,
      reason: 'single_objective_candidate_pool',
      preferred_objectives: [],
      candidate_objectives: Array.from(objectiveSet)
    };
  }
  const stats = objectiveMixStatsForWindow(endDateStr, AUTONOMY_OBJECTIVE_MIX_WINDOW_DAYS);
  if (stats.total < AUTONOMY_OBJECTIVE_MIX_MIN_EXECUTED) {
    return {
      applied: false,
      reason: 'insufficient_history',
      preferred_objectives: [],
      history_total: stats.total,
      min_executed: AUTONOMY_OBJECTIVE_MIX_MIN_EXECUTED
    };
  }
  let dominantObjective = '';
  let dominantCount = 0;
  for (const [objectiveId, countRaw] of Object.entries(stats.counts || {})) {
    if (!objectiveSet.has(String(objectiveId))) continue;
    const count = Number(countRaw || 0);
    if (count > dominantCount) {
      dominantCount = count;
      dominantObjective = String(objectiveId);
    }
  }
  if (!dominantObjective) {
    return {
      applied: false,
      reason: 'no_objective_history_for_candidate_pool',
      preferred_objectives: [],
      history_total: stats.total
    };
  }
  const dominantShare = dominantCount / Math.max(1, stats.total);
  if (dominantShare <= AUTONOMY_OBJECTIVE_MIX_MAX_SHARE) {
    return {
      applied: false,
      reason: 'dominant_share_within_cap',
      preferred_objectives: [],
      dominant_objective: dominantObjective,
      dominant_share: Number(dominantShare.toFixed(3)),
      max_share: AUTONOMY_OBJECTIVE_MIX_MAX_SHARE,
      history_total: stats.total
    };
  }
  const preferredObjectives = Array.from(objectiveSet)
    .filter((id) => id && id !== dominantObjective)
    .sort((a, b) => String(a).localeCompare(String(b)));
  if (preferredObjectives.length === 0) {
    return {
      applied: false,
      reason: 'no_alternative_objectives_available',
      preferred_objectives: [],
      dominant_objective: dominantObjective,
      dominant_share: Number(dominantShare.toFixed(3)),
      max_share: AUTONOMY_OBJECTIVE_MIX_MAX_SHARE,
  ...
  return {
    applied: true,
    reason: 'dominant_objective_share_exceeded',
    dominant_objective: dominantObjective,
    dominant_share: Number(dominantShare.toFixed(3)),
    max_share: AUTONOMY_OBJECTIVE_MIX_MAX_SHARE,
    history_total: stats.total,
    window_days: stats.window_days,
    preferred_objectives: preferredObjectives
  };
}
...
function primaryDirectiveObjectiveId() {
  let directives = [];
  try {
    directives = loadActiveDirectives({ allowMissing: true });
  } catch {
  ...
  for (const row of list) {
    ...
    const metadata = data.metadata && typeof data.metadata === 'object' ? data.metadata : {};
    const objectiveId = String(row.id || metadata.id || '').trim();
    if (!objectiveId) continue;
    const tierRaw = Number.isFinite(Number(row.tier))
      ? Number(row.tier)
      : Number(metadata.tier);
    const tier = Number.isFinite(tierRaw) ? normalizeDirectiveTier(tierRaw, 9) : 9;
    if (!best || tier < best.tier || (tier === best.tier && objectiveId < best.id)) {
      best = { id: objectiveId, tier };
    }
  }
  return best ? best.id : '';
}
...
function applyCampaignDecompositionSeeds(dateStr, strategy, opts = {}) {
  ...
  if (!Array.isArray(proposals) || proposals.length <= 0) {
    return { ok: true, enabled: true, created: 0, proposal_ids: [], reason: 'no_proposals' };
  }
  const defaultObjectiveId = primaryDirectiveObjectiveId();
  const plan = buildCampaignDecompositionPlans(proposals, strategy, {
    min_open_per_type: AUTONOMY_CAMPAIGN_DECOMPOSE_MIN_OPEN_PER_TYPE,
    max_additions: AUTONOMY_CAMPAIGN_DECOMPOSE_MAX_PER_RUN,
    default_objective_id: defaultObjectiveId,
    default_risk: AUTONOMY_CAMPAIGN_DECOMPOSE_DEFAULT_RISK,
    default_impact: AUTONOMY_CAMPAIGN_DECOMPOSE_DEFAULT_IMPACT
  });
  ...
  if (additions.length <= 0) {
    return {
      ...
      created: 0,
      proposal_ids: [],
      campaign_count: Number(plan && plan.campaign_count || 0),
      min_open_per_type: Number(plan && plan.min_open_per_type || AUTONOMY_CAMPAIGN_DECOMPOSE_MIN_OPEN_PER_TYPE),
      max_additions: Number(plan && plan.max_additions || AUTONOMY_CAMPAIGN_DECOMPOSE_MAX_PER_RUN)
    };
  }
  ...
  if (opts.logRun === true) {
    writeRun(dateStr, {
      ts: nowIso(),
      type: 'autonomy_campaign_decomposition',
      result: 'seeded',
      campaign_count: Number(plan && plan.campaign_count || 0),
      created: additions.length,
      proposal_ids: additions.map((row) => String(row && row.id || '')).filter(Boolean).slice(0, 32),
      min_open_per_type: Number(plan && plan.min_open_per_type || AUTONOMY_CAMPAIGN_DECOMPOSE_MIN_OPEN_PER_TYPE),
      max_additions: Number(plan && plan.max_additions || AUTONOMY_CAMPAIGN_DECOMPOSE_MAX_PER_RUN)
    });
  }
  return {
    ...
    enabled: true,
    created: additions.length,
    proposal_ids: additions.map((row) => String(row && row.id || '')).filter(Boolean),
    campaign_count: Number(plan && plan.campaign_count || 0),
    min_open_per_type: Number(plan && plan.min_open_per_type || AUTONOMY_CAMPAIGN_DECOMPOSE_MIN_OPEN_PER_TYPE),
    max_additions: Number(plan && plan.max_additions || AUTONOMY_CAMPAIGN_DECOMPOSE_MAX_PER_RUN)
  };
...
function readinessCmd(dateStr) {
  ...
  let campaignDecomposition = {
    enabled: AUTONOMY_CAMPAIGN_DECOMPOSE_ENABLED,
    created: 0,
    proposal_ids: []
  };
  if (!proposalDate) {
    ...
    campaignDecomposition = applyCampaignDecompositionSeeds(proposalDate, strategy, { logRun: false });
  ...
  const out = {
    ...
    campaign_decomposition: {
      ...
      proposal_ids: Array.isArray(campaignDecomposition && campaignDecomposition.proposal_ids)
        ? campaignDecomposition.proposal_ids.slice(0, 16)
        : []
    },
    blockers
  };
...
function runCmd(dateStr, opts = {}) {
  ...
  const refreshAttempted = !!(opts && opts.refreshAttempted === true);
  ...
  const campaignDecomposition = applyCampaignDecompositionSeeds(proposalDate, strategy, {
    logRun: !shadowOnly
  });
  ...
  if (AUTONOMY_UNCHANGED_SHORT_CIRCUIT_ENABLED) {
    ...
    if (shortCircuit.hit) {
      ...
      writeRun(dateStr, {
        ts: nowIso(),
        type: 'autonomy_run',
        result: 'no_candidates_policy_unchanged_state',
        legacy_result: 'stop_repeat_gate_unchanged_state',
        policy_hold: true,
        hold_scope: 'capacity',
  ...
  if (!shadowOnly && maxRunsPerDay > 0 && attemptsTodayForCap >= maxRunsPerDay && dailyCapOverride.consumed !== true && !executionQuotaDeficit) {
    writeRun(dateStr, {
      ...
      legacy_result: 'stop_repeat_gate_daily_cap',
      policy_hold: true,
      hold_scope: 'capacity',
  ...
  if (
    ...
    writeRun(dateStr, {
      ts: nowIso(),
      type: 'autonomy_run',
      result: 'no_candidates_policy_canary_cap',
      legacy_result: 'stop_repeat_gate_canary_cap',
      policy_hold: true,
      hold_scope: 'capacity',
  ...
  if (!shadowOnly && AUTONOMY_HUMAN_ESCALATION_BLOCK_RUNS) {
    ...
    if (activeEscalations.length > 0) {
      ...
      const dedupeWindowMinutes = Math.max(
        AUTONOMY_HUMAN_ESCALATION_DEDUPE_MINUTES,
        Math.round(Math.max(0, Number(AUTONOMY_HUMAN_ESCALATION_DEDUPE_HOURS || 0)) * 60)
      );
      const lastPending = latestAutonomyRunByResult(priorRuns, 'no_candidates_policy_manual_review_pending')
        || latestAutonomyRunByResult(priorRuns, 'stop_repeat_gate_human_escalation_pending');
      const samePending = !!(
        lastPending
        && lastPending.top_escalation
        && String(lastPending.top_escalation.signature || '') === String(topEscalation.signature || '')
        && String(lastPending.top_escalation.proposal_id || '') === String(topEscalation.proposal_id || '')
      );
      const deduped = samePending && isRecentTsWithin(lastPending.ts, dedupeWindowMinutes);
      if (deduped) {
        writeRun(dateStr, {
          ts: nowIso(),
          type: 'autonomy_human_escalation_watch',
          result: 'deduped_pending',
          dedupe_window_minutes: dedupeWindowMinutes,
          next_clear_at: nextAt,
          top_escalation: topEscalation
        });
      } else {
        writeRun(dateStr, {
          ts: nowIso(),
  ...
  const candidateAuditPolicy = {
    ...
    objective_mix: {
      enabled: AUTONOMY_OBJECTIVE_MIX_ENABLED,
      window_days: AUTONOMY_OBJECTIVE_MIX_WINDOW_DAYS,
      min_executed: AUTONOMY_OBJECTIVE_MIX_MIN_EXECUTED,
      max_share: AUTONOMY_OBJECTIVE_MIX_MAX_SHARE
    },
  ...
  for (const cand of pool) {
    ...
    if (
      !shadowOnly
      && AUTONOMY_EXECUTE_REQUIRE_T1
      && isExecuteMode(executionMode)
      && !isTier1CandidateObjective({ objective_binding: objectiveBinding, directive_pulse: pulse })
    ) {
      skipStats.t1_execute_required += 1;
      bumpCount(candidateRejectedByGate, 't1_execute_required');
      pushCandidateAudit({
        proposal_id: proposalId,
        proposal_type: proposalType,
        risk,
        pass: false,
        gate: 't1_execute_required',
        score: Number(cand.score || 0),
        objective_binding: {
          objective_id: objectiveBinding.objective_id || null,
          source: objectiveBinding.source || null
        ...
        reasons: ['execute_mode_requires_t1_objective']
      });
      if (!sampleT1ExecuteRequired) {
        sampleT1ExecuteRequired = {
          proposal_id: cand.proposal.id,
          objective_id: objectiveBinding.objective_id || pulse.objective_id || null,
          tier: pulse.tier == null ? null : pulse.tier,
          reason: 'execute_mode_requires_t1_objective'
        };
      }
      continue;
    }
  ...
  if (eligible.length > 0) {
    ...
    objectiveMix = assessObjectiveMixGovernor(dateStr, eligible);
    if (objectiveMix && objectiveMix.applied) {
      writeRun(dateStr, {
        ts: nowIso(),
        type: 'autonomy_objective_mix',
        result: 'active',
        dominant_objective: objectiveMix.dominant_objective || null,
        dominant_share: Number(objectiveMix.dominant_share || 0),
        max_share: Number(objectiveMix.max_share || AUTONOMY_OBJECTIVE_MIX_MAX_SHARE),
        history_total: Number(objectiveMix.history_total || 0),
        window_days: Number(objectiveMix.window_days || AUTONOMY_OBJECTIVE_MIX_WINDOW_DAYS),
        preferred_objectives: Array.isArray(objectiveMix.preferred_objectives)
          ? objectiveMix.preferred_objectives.slice(0, 8)
          : []
      });
    }
    const objectiveMixEligible = objectiveMix && objectiveMix.applied
      ? eligible.filter((cand) => {
        const id = objectiveIdForCandidate(cand);
        return id && objectiveMix.preferred_objectives.includes(id);
      })
      : [];
    const nonFallbackEligible = eligible.filter(c => !isDeprioritizedSourceProposal(c && c.proposal));
    if (
      !tierReservation
      && objectiveMixEligible.length > 0
    ) {
      const laneSelection = shadowOnly
        ? chooseEvidenceSelectionMode(objectiveMixEligible, priorRuns, 'objective_mix')
        : chooseSelectionMode(objectiveMixEligible, priorRuns);
      const lanePick = objectiveMixEligible[laneSelection.index] || objectiveMixEligible[0];
      ...
      selection = {
        ...
        mode: shadowOnly
          ? laneSelection.mode
          : (laneSelection.mode === 'explore' ? 'objective_mix_explore' : 'objective_mix_exploit'),
        index: fullIdx >= 0 ? fullIdx : 0
      };
    } else if (
      !tierReservation
  ...
  if (!pick) {
    if (AUTONOMY_CANDIDATE_EXHAUSTED_REFRESH_ENABLED && !refreshAttempted && !forceProposalId) {
      const reconcileRefresh = runProposalQueueReconcile(2, false);
      const enrichRefresh = runProposalEnricher(proposalDate, false);
      const refreshPayload = enrichRefresh.payload && typeof enrichRefresh.payload === 'object'
        ? enrichRefresh.payload
        : null;
      writeRun(dateStr, {
        ts: nowIso(),
        type: 'autonomy_candidate_refresh',
        refresh_reason: 'candidate_exhausted',
        proposal_date: proposalDate,
        reconcile: {
          ok: reconcileRefresh.ok,
          code: reconcileRefresh.code,
          payload: reconcileRefresh.payload && typeof reconcileRefresh.payload === 'object'
            ? {
                files_scanned: Number(reconcileRefresh.payload.files_scanned || 0),
                files_updated: Number(reconcileRefresh.payload.files_updated || 0),
                proposals_updated: Number(reconcileRefresh.payload.proposals_updated || 0),
                progress_events_added: Number(reconcileRefresh.payload.progress_events_added || 0)
              }
            : null
        },
        enrich: {
          ok: enrichRefresh.ok,
          code: enrichRefresh.code,
          payload: refreshPayload
            ? {
                ok: refreshPayload.ok === true,
                admission: refreshPayload.admission && typeof refreshPayload.admission === 'object'
                  ? {
                      total: Number(refreshPayload.admission.total || 0),
                      eligible: Number(refreshPayload.admission.eligible || 0),
                      blocked: Number(refreshPayload.admission.blocked || 0)
                    }
                  : null
              }
            : null
      ...
      if (reconcileRefresh.ok || enrichRefresh.ok) {
    ...
    if (
      ...
      writeRun(dateStr, {
        ts: nowIso(),
        type: 'autonomy_run',
        result: 'stop_init_gate_t1_objective_required',
        execute_require_t1: AUTONOMY_EXECUTE_REQUIRE_T1,
    ...
    writeRun(dateStr, {
      ...
      reason: `all_candidates_exhausted quality_or_directive_fit_or_actionability_or_optimization_good_enough_or_objective_binding_or_t1_execute_or_value_or_budget_pacing_or_composite_or_capability_cooldown_or_medium_risk_or_eye_no_progress_or_directive_pulse_or_objective_runtime`,
