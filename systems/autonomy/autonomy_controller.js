...
} = require('./objective_runtime_governor');
const { reconcileHumanEscalations } = require('./escalation_resolver');
...
function suggestAutonomyRunBatchMax(dateStr) {
  ...
  const priorRuns = priorRunsForStrategySelection;
...
function candidateNonYieldPenaltySignal(cand, opts = {}) {
  const out = {
    applied: false,
    penalty: 0,
    samples: 0,
...
function strategyRankForCandidate(cand, strategy, opts = {}) {
...
function candidatePool(dateStr, strategyOverride = null) {
  ...
  const strategy = strategyOverride || strategyProfile();
  const allowedRisks = effectiveAllowedRisksSet(strategy);
  const duplicateWindowHours = strategyDuplicateWindowHours(strategy, 24);
...
function chooseQosLaneSelection(eligible, priorRuns, opts = {}) {
  ...
  const strategy = opts && opts.strategy && typeof opts.strategy === 'object'
    ? opts.strategy
    : null;
  ...
  const laneSelection = shadowOnly
    ...
    : chooseSelectionMode(laneEligible, priorRuns, strategy);
...
function exploreQuotaForDay(strategyOverride = null) {
  const caps = effectiveStrategyBudget(strategyOverride);
  const exp = effectiveStrategyExploration(strategyOverride);
...
function chooseSelectionMode(eligible, priorRuns, strategyOverride = null) {
  ...
  const quota = exploreQuotaForDay(strategyOverride);
  const exp = effectiveStrategyExploration(strategyOverride);
...
function statusCmd(dateStr) {
  ...
  const strategy = strategyProfile();
  const pool = candidatePool(effectiveDate, strategy);
  ...
  const strategyExplore = effectiveStrategyExploration(strategy);
  const executionMode = effectiveStrategyExecutionMode(strategy);
  const allowedRisks = effectiveAllowedRisksSet(strategy);
...
function readinessCmd(dateStr) {
  ...
  if (!proposalDate) {
    ...
    pool = candidatePool(proposalDate, strategy);
    if (!pool.length) {
...
function runCmd(dateStr, opts = {}) {
  ...
  const strategyBudget = effectiveStrategyBudget(strategy);
  let executionMode = shadowOnly ? 'score_only' : effectiveStrategyExecutionMode(strategy);
  if (!shadowOnly && AUTONOMY_EXECUTION_MODE_PERSISTENCE_GUARD) {
    ...
    executionMode = effectiveStrategyExecutionMode(strategy);
  ...
  let pick = null;
  let selection = { mode: 'exploit', index: 0, explore_used: 0, explore_quota: exploreQuotaForDay(strategy), exploit_used: 0 };
  ...
  if (eligible.length > 0) {
    ...
    if (
      ...
      const laneSelection = shadowOnly
        ...
        : chooseSelectionMode(objectiveMixEligible, priorRuns, strategy);
      ...
      const laneSelection = shadowOnly
        ...
        : chooseSelectionMode(nonFallbackEligible, priorRuns, strategy);
      ...
      selection = shadowOnly
        ...
        : chooseSelectionMode(eligible, priorRuns, strategy);
  ...
  if (!pick) {
    ...
    writeRun(dateStr, {
      ...
      reason: `all_candidates_exhausted unknown_type_quarantine_or_quality_or_directive_fit_or_actionability_or_optimization_good_enough_or_objective_binding_or_t1_execute_or_value_or_budget_pacing_or_composite_or_capability_cooldown_or_medium_risk_or_eye_no_progress_or_directive_pulse_or_objective_runtime`,
      skipped_unknown_type_quarantine: skipStats.unknown_type_quarantine,
    ...
    if (AUTONOMY_SCORE_ONLY_PROMOTION_ENABLED) {
      ...
      if (promotionPass && !shadowOnly && allowScoreOnlyPromotion) {
        ...
        const governorPayload = governor && governor.payload && typeof governor.payload === 'object'
          ? governor.payload
          : null;
        const modeAfter = effectiveStrategyExecutionMode(strategy);
