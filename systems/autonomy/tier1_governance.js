...
function dateWindow(endDateStr, days) {
    ...
    for (let i = n - 1; i >= 0; i -= 1)
        out.push(shiftDate(endDateStr, -i));
    return out;
}
function normalizeText(v) {
    return String(v == null ? '' : v).trim();
}
function truthyFlag(v, fallback = false) {
    if (v == null)
        return fallback;
    if (typeof v === 'boolean')
        return v;
...
function extractTokenUsage(evt) {
    ...
    if (Number.isFinite(routeEst) && routeEst >= 0)
        return routeEst;
...
function extractExpectedValueScore(evt) {
    ...
    if (Number.isFinite(expected))
        return clampNumber(expected, 0, 100);
...
function hasAlignmentSignals(evt = {}) {
    return extractExpectedValueScore(evt) != null
        || extractDirectiveFitScore(evt) != null
        || extractCompositeScore(evt) != null;
}
function isLegacyExecutedRow(evt = {}) {
    if (!hasAlignmentObjective(evt))
        return true;
    if (!hasAlignmentSignals(evt))
        return true;
    return false;
}
...
function evaluateCostGovernor(opts = {}) {
    ...
    if (month) {
        for (const [day, tok] of Object.entries(byDay)) {
            if (String(day).startsWith(month))
                monthTokensSpent += Number(tok || 0);
        }
    }
    ...
    if (burnRateExceeded)
        hardStopReasons.push('burn_rate_exceeded');
...
function evaluateDrift(opts = {}) {
    ...
    if (recent.length >= minSamples && errorRateRecent != null) {
        ...
        if (errorRateRecent > baselineBound)
            triggers.push('error_rate_spike');
    }
...
function weeklyAlignmentScore(executedEvents, endDateStr, opts = {}) {
    const includeLegacyExecuted = truthyFlag(opts.includeLegacyExecuted == null
...
function summarizeExceptionMemory(memoryPath, days = 7) {
    ...
    for (const ent of all) {
        ...
        if (Number.isFinite(first) && first >= cutoffMs)
            novelRecent += 1;
    }
    return {
...
function normalizeRecoveryAction(v) {
    ...
    if (s === 'escalate' || s === 'recover' || s === 'cooldown')
        return s;
    return 'recover';
}
...
function evaluateTier1Governance(opts = {}) {
    ...
    const blockers = [];
    if (cost.hard_stop)
        blockers.push({ gate: 'cost_governor', reasons: cost.hard_stop_reasons.slice(0, 6) });
    if (drift.hard_stop)
        blockers.push({ gate: 'drift_detection', reasons: drift.triggers.slice(0, 6) });
    if (alignment.escalate)
        blockers.push({ gate: 'alignment_oracle', reasons: ['low_weekly_alignment_two_weeks'] });
    return {
        enabled: true,
        date: dateStr,
        blockers,
