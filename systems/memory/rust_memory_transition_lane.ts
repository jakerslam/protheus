...
function writeStage1BenchmarkReport(summary) {
  const reportAbs = path.join(ROOT, 'benchmarks', 'memory-stage1.md');
  ...
  return path.relative(ROOT, reportAbs).replace(/\\/g, '/');
}
...
function runBenchmark(args, policy) {
  ...
  const stage1ReportPath = writeStage1BenchmarkReport({
  ...
  const out = {
    ...
    target_speedup: policy.thresholds.min_speedup_for_cutover,
    stable_runs: recent.length,
    stage1_report_path: stage1ReportPath
  };
  if (autoSelect) {
