...
function memorySecurityGateEnabled(opts: AnyObj = {}) {
...
function buildMemorySecurityRequest(command: string, args: string[], opts: AnyObj = {}) {
...
function evaluateMemorySecurityGate(command: string, args: string[], opts: AnyObj = {}) {
...
function runMemoryCli(command: string, args: string[] = [], timeoutMs = 180000, opts: AnyObj = {}) {
...
function recall(query: string, limit = 5) {
  return runMemoryCli('recall', [`--query=${cleanText(query, 400)}`, `--limit=${Math.max(1, Number(limit) || 5)}`]);
}

function ingest(id: string, content: string, tags: string[] = [], repetitions = 1, lambda = 0.02) {
...
function ebbinghausScore(ageDays: number, repetitions = 1, lambda = 0.02) {
...
function crdtExchange(payload: unknown) {
