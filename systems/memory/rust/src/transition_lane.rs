...
fn default_policy(root: &Path) -> TransitionPolicy {
    TransitionPolicy {
        ...
        shadow_only: true,
        paths: TransitionPaths {
            latest_path: root.join("state/memory/rust_transition/latest.json"),
            receipts_path: root.join("state/memory/rust_transition/receipts.jsonl"),
            selector_path: root.join("state/memory/rust_transition/backend_selector.json"),
            benchmark_path: root.join("state/memory/rust_transition/benchmark_history.json"),
            benchmark_latest_path: root.join("state/memory/rust_transition/benchmark_latest.json"),
            benchmark_report_path: root.join("benchmarks/memory-stage1.md"),
            memory_index_path: root.join("MEMORY_INDEX.md"),
            rust_crate_path: root.join("crates/memory"),
        },
        thresholds: TransitionThresholds {
    ...
    TransitionPolicy {
        ...
        paths: TransitionPaths {
            latest_path: resolve_path(
                ...
                "state/memory/rust_transition/latest.json",
            ),
            receipts_path: resolve_path(
                root,
                paths_raw.get("receipts_path"),
                "state/memory/rust_transition/receipts.jsonl",
            ),
            selector_path: resolve_path(
                root,
                paths_raw.get("selector_path"),
                "state/memory/rust_transition/backend_selector.json",
            ),
            benchmark_path: resolve_path(
                root,
                paths_raw.get("benchmark_path"),
                "state/memory/rust_transition/benchmark_history.json",
            ),
            benchmark_latest_path: resolve_path(
                root,
                paths_raw.get("benchmark_latest_path"),
                "state/memory/rust_transition/benchmark_latest.json",
            ),
            ...
            rust_crate_path: resolve_path(root, paths_raw.get("rust_crate_path"), "crates/memory"),
        },
        thresholds: TransitionThresholds {
...
pub fn maybe_run(root: &Path, argv: &[String]) -> Option<i32> {
    ...
    let policy_path = {
        ...
        if let Some(path) = explicit {
            ...
            root.join("config/rust_memory_transition_policy.json")
        }
    };
...
mod tests {
    ...
    fn evaluate_auto_selector_matches_threshold_gate() {
        ...
        fs::create_dir_all(root.join("state/memory/rust_transition")).expect("mkdir state");
        fs::write(&policy_path, "{}").expect("write policy");
        let policy = load_policy(&root, &policy_path);
        let scope_id = policy_scope_id(&policy);
        let history_path = root.join("state/memory/rust_transition/benchmark_history.json");
        let rows = (0..12)
            .map(|idx| {
                json!({
                    "policy_scope": scope_id,
    ...
    fn transition_receipt_write_is_deterministic_and_claim_bounded() {
        ...
        let claims = vec![ClaimEvidenceRow {
            ...
            evidence: vec![
                "path:state/memory/rust_transition/benchmark_history.json".to_string(),
        ...
        let first = fs::read_to_string(root.join("state/memory/rust_transition/latest.json"))
            .expect("read first");
        write_transition_receipt(&policy, &payload_b, &claims);
        let second = fs::read_to_string(root.join("state/memory/rust_transition/latest.json"))
            .expect("read second");
