...
const ALLOWED_TYPES = new Set(
  String(process.env.MEMORY_BRIDGE_ALLOWED_TYPES || 'external_intel,cross_signal_opportunity,collector_remediation,infrastructure_outage,opportunity_capture')
    .split(',')
...
function itemHashFromProposal(p) {
  ...
  if (evidenceHash) return evidenceHash;
...
function makeCandidate(p) {
  ...
  if (!title || /\[stub\]/i.test(title)) return null;
  ...
  const reasons = [];
  if (!eligible) reasons.push('below_bridge_threshold');
...
function runBridge(dateStr, maxNodes) {
  ...
  const eligible = proposals
    ...
    .sort((a, b) => {
      ...
      if (b.composite_eligibility_score !== a.composite_eligibility_score) return b.composite_eligibility_score - a.composite_eligibility_score;
