...
function collectObservations(dates, limits = {}) {
  const maxTopicsPerItem = clamp((limits && limits.maxTopicsPerItem), 1, 12, 6);
  const maxObsPerEyeTopicPerDay = clamp((limits && limits.maxObsPerEyeTopicPerDay), 1, 100, 8);
  const minTopicLength = clamp((limits && limits.minTopicLength), 3, 16, 4);
