...
const MANIFEST = path.join(ROOT, 'crates', 'swarm', 'Cargo.toml');
...
function binaryCandidates() {
    const explicit = cleanText(process.env.PROTHEUS_SWARM_RUST_BIN || '', 500);
    const out = [
        explicit,
        path.join(ROOT, 'target', 'release', 'swarm_core'),
        path.join(ROOT, 'target', 'debug', 'swarm_core'),
        path.join(ROOT, 'crates', 'swarm', 'target', 'release', 'swarm_core'),
        path.join(ROOT, 'crates', 'swarm', 'target', 'debug', 'swarm_core')
    ].filter(Boolean);
...
function runSwarm(request, opts = {}) {
