...
pub fn build_envelope(within_target: bool, completed_lanes: usize) -> serde_json::Value {
    let status = if completed_lanes >= 9 { "ready_for_guardrail_gate" } else { "incomplete" };
...
mod tests {
    ...
    fn envelope_includes_guardrails() {
        ...
        assert!(v.get("guardrails").and_then(|x| x.as_array()).map(|a| !a.is_empty()).unwrap_or(false));
    }
}
