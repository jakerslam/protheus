...
function listCanonicalFiles(eventsDir) {
...
function loadPolicy(policyPath = DEFAULT_POLICY_PATH) {
    ...
    const includeTypes = Array.isArray(raw.include_event_types)
        ? raw.include_event_types.map((row) => normalizeToken(row, 80)).filter(Boolean)
...
function existingExplainedEventIds(indexPath) {
...
function shouldConsiderEvent(row, policy, sinceMs) {
    const tsMs = Date.parse(String(row && row.ts || ''));
    if (!Number.isFinite(tsMs) || tsMs < sinceMs)
        return false;
    const type = normalizeToken(row && row.type || '', 80);
    if (!policy.include_event_types.includes(type))
        return false;
    if (String(row && row.phase || '') !== 'finish')
        return false;
    if (policy.only_failures === true && row.ok !== false)
        return false;
    return true;
}
...
function emitExplanation(eventId, summary, apply = true) {
    const args = [
...
function runAutoEmit(args) {
    ...
    for (const filePath of listCanonicalFiles(policy.canonical_events_dir)) {
        for (const row of readJsonl(filePath)) {
            if (!shouldConsiderEvent(row, policy, sinceMs))
                continue;
            const eventId = cleanText(row && row.event_id || '', 120);
            if (!eventId || explained.has(eventId))
                continue;
            candidates.push({
                event_id: eventId,
                ts: row.ts || null,
...
function main() {
    ...
    if (cmd === 'run') {
        ...
        if (toBool(args.strict, false) && out.ok !== true)
            process.exit(1);
        return;
    }
    if (cmd === 'status') {
