...
function ensureFileModeOwnerReadWrite(filePath) {
    try {
...
function resolveSigningKey(policy) {
    ...
    if (keyFilePath && fs.existsSync(keyFilePath)) {
        try {
            ...
            if (fileKey)
                return { env_name: envName, key: fileKey, source: 'file', key_file_path: keyFilePath };
        }
        catch {
            // fall through
        }
...
function readChainState(policy) {
...
function writeChainState(policy, row) {
...
function buildActorContext(policy, args = {}) {
...
function readPassport(policy) {
    return readJson(policy.state.passport_path, null);
}
function passportIsActive(passport) {
    if (!passport || typeof passport !== 'object')
        return false;
    const exp = parseIsoMs(passport.expires_at);
    return Number.isFinite(exp) && exp > Date.now();
}
function signPayload(payload, policy) {
    const keyInfo = resolveSigningKey(policy);
    if (!keyInfo.key)
        return { ok: false, reason: 'signing_key_missing', key_env: keyInfo.env_name };
    return {
        ok: true,
...
function bootstrapSigningKey(args = {}) {
...
function issuePassport(args = {}, opts = {}) {
    const policyPath = resolvePath(args.policy || DEFAULT_POLICY_PATH, 'config/agent_passport_policy.json');
    const policy = loadPolicy(policyPath);
    ...
    const ttlHours = clampInt(args['ttl-hours'] || args.ttl_hours || args.ttl || policy.passport_ttl_hours, 1, 24 * 3650, policy.passport_ttl_hours);
...
function ensureActivePassport(policy, args = {}) {
    const active = readPassport(policy);
    if (passportIsActive(active))
        return { ok: true, passport: active };
    ...
    if (!issued.ok)
        return { ok: false, reason: issued.error || 'auto_issue_failed' };
    const next = readPassport(policy);
    if (!passportIsActive(next))
        return { ok: false, reason: 'auto_issue_inactive' };
    return { ok: true, passport: next };
}
function parseActionInput(args) {
...
function normalizeAction(payload = {}) {
    return {
        ...
        receipt_seq: clampInt(payload.receipt_seq, 0, 1000000000, 0) || null,
        receipt_hash: cleanText(payload.receipt_hash || '', 80) || null,
        metadata: payload.metadata && typeof payload.metadata === 'object' ? payload.metadata : {}
    };
}
function appendAction(args = {}) {
    const policyPath = resolvePath(args.policy || DEFAULT_POLICY_PATH, 'config/agent_passport_policy.json');
    const policy = loadPolicy(policyPath);
    if (policy.enabled !== true) {
...
function appendActionFromReceipt(input = {}) {
    ...
    const action = {
        ...
        target: cleanText(rec.objective
            || rec.summary
            || rec.action
            || rec.id
            || rec.proposal_id
...
function verifyActionChain(args = {}) {
    const policyPath = resolvePath(args.policy || DEFAULT_POLICY_PATH, 'config/agent_passport_policy.json');
    const policy = loadPolicy(policyPath);
...
function buildDeterministicPdf(lines) {
...
function exportPassportPdf(args = {}) {
    const policyPath = resolvePath(args.policy || DEFAULT_POLICY_PATH, 'config/agent_passport_policy.json');
    const policy = loadPolicy(policyPath);
    const rows = readJsonl(policy.state.action_log_path);
    ...
    for (const row of canonical.sample_rows.slice(0, 40)) {
        lines.push(`${String(row.seq).padStart(4, ' ')} | ${cleanText(row.ts, 24)} | ${cleanText(row.action_type, 24)} | ${cleanText(row.status || '-', 10)} | ${cleanText(row.target || '-', 72)}`);
    }
...
function main() {
    ...
    if (cmd === 'bootstrap')
        out = bootstrapSigningKey(args);
    else if (cmd === 'issue')
        out = issuePassport(args, { apply: true });
    else if (cmd === 'append')
        out = appendAction(args);
    else if (cmd === 'verify')
        out = verifyActionChain(args);
    else if (cmd === 'export-pdf')
        out = exportPassportPdf(args);
    else if (cmd === 'status')
        out = status(args);
    else {
        usage();
