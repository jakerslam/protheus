...
function normalizeText(v, maxLen = 4096) {
...
function parseSnapshotTsMs(snapshotId) {
...
function resolveDestination(argsValue, envKey, fallbackPath) {
    ...
    if (!path.isAbsolute(abs))
        throw new Error('destination_must_be_absolute');
    return abs;
}
function loadLocalSnapshotContext(policy, args) {
    ...
    if (!ids.length)
        throw new Error(`source_snapshot_missing profile=${profile}`);
    ...
    if (!manifest || typeof manifest !== 'object')
        throw new Error(`source_manifest_missing snapshot=${snapshotId}`);
...
function normalizeKeyBuffer(rawKey) {
...
function loadEncryptionKey(policy) {
...
function decryptBuffer(encrypted, key, ivHex, tagHex) {
...
function loadOffsiteSnapshotContext(policy, args) {
    const profile = normalizeText(args.profile || policy.default_profile, 120) || 'runtime_state';
    const offsiteDest = resolveDestination(args['offsite-dest'] || args.offsite_dest, policy.offsite.destination_env, policy.offsite.destination_default);
    const profileDir = path.join(offsiteDest, profile);
    const ids = listSnapshotIds(profileDir);
    if (!ids.length)
        throw new Error(`offsite_snapshot_missing profile=${profile}`);
    const snapshotId = normalizeText(args.snapshot, 120) || ids[ids.length - 1];
    const snapshotDir = path.join(profileDir, snapshotId);
    const manifestPath = path.join(snapshotDir, 'manifest.json');
    const manifest = readJsonSafe(manifestPath, null);
    if (!manifest || typeof manifest !== 'object')
        throw new Error(`offsite_manifest_missing snapshot=${snapshotId}`);
    const manifestType = normalizeText(manifest.type, 120);
...
function cmdRestoreDrill(args) {
    ...
    for (const row of files) {
        ...
        let plain;
        try {
            plain = decryptBuffer(encrypted, keyState.key, normalizeText(row && row.iv_hex, 64), normalizeText(row && row.tag_hex, 64));
        }
        catch {
            mismatch += 1;
...
function cmdStatus(args) {
    ...
    const lastDrillReasons = Array.isArray(lastDrill && lastDrill.reasons)
        ? lastDrill.reasons.map((row) => normalizeText(row, 120)).filter(Boolean).slice(0, 8)
        : [];
    return {
        ok: true,
...
function main() {
    ...
    if (cmd === 'sync')
        out = cmdSync(args);
    else if (cmd === 'restore-drill')
        out = cmdRestoreDrill(args);
    else if (cmd === 'status')
        out = cmdStatus(args);
    else if (cmd === 'diagnose')
        out = cmdDiagnose(args);
    else if (cmd === 'list')
        out = cmdList(args);
    else {
        usage();
...
if (require.main === module) {
    try {
        ...
        process.stdout.write(JSON.stringify({
            ok: false,
            error: String(err && err.message ? err.message : err || 'offsite_backup_failed')
        }) + '\n');
        process.exit(1);
    }
}
