...
/** Runtime anchor for BL-044 */
const fs = require('fs');
const path = require('path');
const { nowIso, stableHash } = require('../../../lib/queued_backlog_runtime');
const ROOT = path.resolve(__dirname, '..', '..', '..');
const LANE_ID = 'BL-044';
const TITLE = 'Persona continuity + arbitration + structured outputs hardening';
const DEPENDENCIES = ['BL-043'];
const REQUIRED_REFS = [
  'systems/personas/cli.ts',
  'systems/ops/protheusctl.ts',
  'personas/organization/stance_cache.json',
  'memory/tools/tests/personas_lens_cli.test.js'
];
