...
function runJsonCommand(command) {
    ...
    const timeoutMs = Math.max(5000, Number(process.env.SYSTEM_HEALTH_AUDIT_CHECK_TIMEOUT_MS || 90000));
...
function buildSections(fullMode, quickMode) {
    const maybeCiCheck = fullMode
...
function runAudit(args) {
    ...
    for (const section of sectionsDef) {
        ...
        for (const check of section.checks) {
            ...
            const reason = ok
                ? null
                : cleanText((result.timed_out ? `check_timeout_${result.timeout_ms}ms` : null)
                    || (result.parsed && (result.parsed.reason || result.parsed.error))
                    || result.stderr
                    || result.stdout
                    || `exit_${result.status}`, 240) || 'check_failed';
            checks.push({
                id: check.id,
                title: check.title,
                ok,
