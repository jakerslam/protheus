...
function missingOffsetOrgans(gateOut) {
    const checks = Array.isArray(gateOut && gateOut.checks) ? gateOut.checks : [];
    const row = checks.find((item) => String(item && item.id || '') === 'new_organs_offset_receipts');
    const detail = cleanText(row && row.detail || '', 1000);
    if (!detail || !detail.startsWith('missing_offsets='))
        return [];
    return detail
...
function cmdRun(args) {
    ...
    const existing = new Set(readJsonl(simplicityPolicy.offset_receipts_path)
        .filter((row) => row && row.approved === true)
        .map((row) => normalizeToken(row.organ_id || '', 80))
        .filter(Boolean));
    ...
    for (const organId of missing) {
        ...
        if (apply)
            appendJsonl(simplicityPolicy.offset_receipts_path, row);
        created.push(row);
