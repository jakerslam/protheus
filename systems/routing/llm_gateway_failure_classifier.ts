...
function classifyGateway(row: AnyObj) {
  ...
  if (!blocked) return { category: 'ok', reason: 'ok' };
  if (code.includes('test_opacity')) return { category: 'opacity_gate', reason: code || 'test_opacity_blocked' };
  if (code.includes('timeout')) return { category: 'timeout', reason: code };
  if (code.includes('budget') || code.includes('burn')) return { category: 'budget_gate', reason: code };
  if (code.includes('provider') || code.includes('api')) return { category: 'provider_error', reason: code };
  if (code.includes('policy') || code.includes('deny')) return { category: 'policy_denied', reason: code };
  return { category: 'gateway_unknown', reason: code || 'gateway_unknown' };
}

function classifyCanonical(row: AnyObj) {
...
function runClassifier(args: AnyObj) {
  ...
  for (const row of gatewayRows) {
    ...
    if (!includeOk && cls.category === 'ok') continue;
