...
function promptCacheFingerprint(parts) {
...
function promptCachePathForKey(key) {
...
function savePromptCacheIndex(index) {
  index.updated_at = nowIso();
  writeJson(PROMPT_CACHE_INDEX_PATH, index);
}

function prunePromptCacheIndex(index, nowMs = Date.now()) {
...
function readPromptCache(key, nowMs = Date.now()) {
...
function writePromptCache(key, payload, nowMs = Date.now()) {
...
function runLocalOllamaPrompt(opts = {}) {
  ...
  const sourceFingerprint = String(opts.source_fingerprint || opts.cache_source_fingerprint || '').trim();
  const cacheAllowed = PROMPT_CACHE_ENABLED && opts.use_cache !== false;
  const cacheTtlMs = Number.isFinite(Number(opts.cache_ttl_ms)) ? Number(opts.cache_ttl_ms) : PROMPT_CACHE_TTL_MS;
