...
function loadLocalWarmupState() {
  ...
  return {
    ...
    last_models: Array.isArray(raw.last_models) ? raw.last_models.map((x: any) => String(x || "")).filter(Boolean) : [],
    last_recovered_models: Array.isArray(raw.last_recovered_models)
      ? raw.last_recovered_models.map((x: any) => String(x || "")).filter(Boolean)
      : []
  };
}

function saveLocalWarmupState(state: AnyObj) {
...
function localWarmupCandidates({ allowlist, localModelAllowed, staleMs, recoverUnavailable }: AnyObj) {
  const out: AnyObj[] = [];
...
function runPeriodicLocalWarmup({
  ...
  const nextState = {
    ...
    last_warmup_ms: nowMs,
    last_reason: reason,
    last_models: selected.map((x: AnyObj) => x.model),
  ...
  appendJsonl(DECISIONS_LOG, {
    ...
    models: selected.map((x: AnyObj) => x.model),
