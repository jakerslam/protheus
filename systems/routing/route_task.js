...
function loadTrustedHabits() {
    ...
    try {
        ...
        if (!parsed || typeof parsed !== 'object')
            return { trusted_files: {} };
        if (!parsed.trusted_files || typeof parsed.trusted_files !== 'object')
...
function loadReflexRoutines() {
    if (!fs.existsSync(REFLEX_ROUTINES_PATH))
        return {};
    try {
...
function requiredInputKeys(habit) {
    if (!habit || !habit.inputs_schema || !Array.isArray(habit.inputs_schema.required))
        return [];
...
function pickReflexMatch(routinesMap, intentKey, task) {
    ...
    const direct = rows.find((r) => {
        if (!r || String(r.status || '').toLowerCase() !== 'enabled')
            return false;
...
function shouldEmitFullRoute() {
    if (process.argv.includes('--full-route') || process.argv.includes('--full_route'))
        return true;
...
function main() {
    ...
    const routeBudgetBlocked = !!(routeMeta &&
        routeMeta.budget_enforcement &&
        routeMeta.budget_enforcement.blocked === true);
    const routeGlobalBudgetBlocked = !!(routeMeta &&
    ...
    if (routeBudgetBlocked || routeGlobalBudgetBlocked) {
        const blockReason = String(routeMeta && routeMeta.budget_enforcement && routeMeta.budget_enforcement.reason
            || routeMeta && routeMeta.budget_global_guard && routeMeta.budget_global_guard.reason
            || 'router_budget_blocked').slice(0, 120);
        const out = {
            decision: 'MANUAL',
