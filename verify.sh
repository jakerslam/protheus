...
VERIFY_TIMEOUT_SEC="${INFRING_VERIFY_TIMEOUT_SEC:-45}"
VERIFY_DEFER_HOST_STALL="${INFRING_VERIFY_DEFER_HOST_STALL:-1}"
VERIFY_NPM_TIMEOUT_SEC="${INFRING_VERIFY_NPM_TIMEOUT_SEC:-60}"
VERIFY_RUST_TIMEOUT_SEC="${INFRING_VERIFY_RUST_TIMEOUT_SEC:-180}"
VERIFY_PROOF_TIMEOUT_SEC="${INFRING_VERIFY_PROOF_TIMEOUT_SEC:-420}"
VERIFY_ARTIFACT_MODE="${INFRING_VERIFY_ARTIFACT_MODE:-ephemeral}"
VERIFY_PROFILE="${INFRING_VERIFY_PROFILE:-release}"
VERIFY_PROFILES_PATH="tests/tooling/config/verify_profiles.json"

if [[ "$VERIFY_ARTIFACT_MODE" == "ephemeral" ]]; then
  VERIFY_TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/infring-verify-XXXXXX")"
  trap 'rm -rf "${VERIFY_TMP_DIR:-}"' EXIT
  CLIENT_LAYER_AUDIT_OUT="$VERIFY_TMP_DIR/shell_layer_boundary_audit_current.json"
  ...
  CLIENT_SCOPE_OUT="$VERIFY_TMP_DIR/shell_scope_inventory_current.json"
  CLIENT_SURFACE_OUT="$VERIFY_TMP_DIR/shell_surface_disposition_current.json"
  CLIENT_TARGET_OUT="$VERIFY_TMP_DIR/shell_target_contract_audit_current.json"
  VERIFY_PROFILE_OUT="$VERIFY_TMP_DIR/verify_profile_current.json"
else
  CLIENT_LAYER_AUDIT_OUT="$ROOT/core/local/artifacts/shell_layer_boundary_audit_current.json"
  ...
  CLIENT_SCOPE_OUT="$ROOT/core/local/artifacts/shell_scope_inventory_current.json"
  CLIENT_SURFACE_OUT="$ROOT/core/local/artifacts/shell_surface_disposition_current.json"
  CLIENT_TARGET_OUT="$ROOT/core/local/artifacts/shell_target_contract_audit_current.json"
