...
function errorPayload(errorCode, detail) {
  const payload = {
    ok: false,
    type: 'moltstack_quality_check_compat_wrapper',
    error: errorCode,
...
try {
  ...
  const payload = errorPayload('moltstack_quality_check_runtime_load_failed', error && error.message ? error.message : String(error));
...
if (runtime && typeof runtime.createCognitionModule === 'function' && typeof runtime.runAsMain === 'function') {
  const mod = runtime.createCognitionModule(
    __dirname,
    'quality-check',
    'COGNITION-SKILLS-MOLTSTACK-SCRIPTS-QUALITY-CHECK'
  );
  ...
  const payload = errorPayload('moltstack_quality_check_runtime_contract_invalid', 'legacy wrapper missing required exports');
  if (require.main === module) {
    process.stderr.write(`${JSON.stringify(payload)}\n`);
    process.exit(1);
  }
