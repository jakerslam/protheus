...
function loadBridge() {
  try {
    ...
    return {
      ok: false,
      error: 'publish_bridge_load_failed',
...
const mod =
  runtime && typeof runtime.createCognitionModule === 'function'
    ? runtime.createCognitionModule(__dirname, 'publish', 'COGNITION-SKILLS-MOLTSTACK-SCRIPTS-PUBLISH')
    : { ok: false, error: 'publish_bridge_missing_createCognitionModule' };
...
if (require.main === module) {
  ...
  if (!runtime || typeof runtime.runAsMain !== 'function') {
    process.stderr.write(JSON.stringify({ ok: false, error: 'publish_bridge_missing_runAsMain' }) + '\n');
    process.exit(1);
  }
