...
function errorPayload(errorCode, detail) {
  const payload = {
    ok: false,
    type: 'moltstack_publish_compat_wrapper',
    error: errorCode,
...
try {
  ...
  const payload = errorPayload('moltstack_publish_runtime_load_failed', error && error.message ? error.message : String(error));
...
if (runtime && typeof runtime.createCognitionModule === 'function' && typeof runtime.runAsMain === 'function') {
  const mod = runtime.createCognitionModule(
    __dirname,
    'publish',
    'COGNITION-SKILLS-MOLTSTACK-SCRIPTS-PUBLISH'
  );
  ...
  const payload = errorPayload('moltstack_publish_runtime_contract_invalid', 'legacy wrapper missing required exports');
  if (require.main === module) {
    process.stderr.write(`${JSON.stringify(payload)}\n`);
    process.exit(1);
  }
