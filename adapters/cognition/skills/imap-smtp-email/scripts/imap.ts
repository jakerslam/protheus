...
const mod =
  runtime && typeof runtime.createCognitionModule === 'function'
    ? runtime.createCognitionModule(__dirname, 'imap', 'COGNITION-SKILLS-IMAP-SMTP-EMAIL-SCRIPTS-IMAP')
    : { ok: false, error: 'imap_bridge_missing_createCognitionModule' };
...
if (require.main === module) {
  ...
  if (!runtime || typeof runtime.runAsMain !== 'function') {
    process.stderr.write(JSON.stringify({ ok: false, error: 'imap_bridge_missing_runAsMain' }) + '\n');
    process.exit(1);
  }
