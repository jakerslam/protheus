...
try {
  ...
  const payload = errorPayload('imap_runtime_load_failed', error && error.message ? error.message : String(error));
...
if (runtime && typeof runtime.createCognitionModule === 'function' && typeof runtime.runAsMain === 'function') {
  const mod = runtime.createCognitionModule(
    __dirname,
    'imap',
    'COGNITION-SKILLS-IMAP-SMTP-EMAIL-SCRIPTS-IMAP'
  );
  ...
  const payload = errorPayload('imap_runtime_contract_invalid', 'legacy wrapper missing required exports');
  if (require.main === module) {
    process.stderr.write(`${JSON.stringify(payload)}\n`);
    process.exit(1);
  }
