...
const RUN_INFRING_OPS_PATH = path.join(ROOT, 'adapters', 'runtime', 'run_infring_ops.ts');
...
function loadBridgeInvoker() {
  delete require.cache[require.resolve(RUN_INFRING_OPS_PATH)];
  delete require.cache[require.resolve(OPS_LANE_BRIDGE_PATH)];
  return require(RUN_INFRING_OPS_PATH).invokeInfringOpsViaBridge;
}

function invokeKernel(domain, command, payload = {}, options = {}) {
  const invokeInfringOpsViaBridge = loadBridgeInvoker();
  const args = [
    domain,
    command,
