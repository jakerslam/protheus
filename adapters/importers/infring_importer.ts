...
const runDomain = createDomainProxy(__dirname, 'IMPORTER_INFRING', 'execution-yield-recovery');
...
function runViaConduit(payloadBase64: string) {
  const out = runDomain(['importer-infring', `--payload-base64=${String(payloadBase64 || '')}`]);
