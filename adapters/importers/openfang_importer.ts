...
const { createDomainProxy } = require(require('path').resolve(process.cwd(), 'client/lib/legacy_conduit_proxy.ts'));

type AnyObj = Record<string, any>;

const runDomain = createDomainProxy(__dirname, 'IMPORTER_OPENFANG', 'execution-yield-recovery');
...
function runViaConduit(payloadBase64: string) {
  const out = runDomain(['importer-openfang', `--payload-base64=${String(payloadBase64 || '')}`]);
