...
const SCRIPT = path.join(ROOT, 'client/runtime/systems/security/insider_threat_split_trust_command_governance.js');
...
try {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'insider_threat_split_trust_command_governance-'));
  ...
  let out = run(['approve', '--policy=' + policyPath, '--strict=1', '--apply=1']);
  ...
  out = run(['approve', '--policy=' + policyPath, '--strict=1', '--apply=1', '--fail-checks=threshold_quorum_requirements']);
  ...
  assert.ok(Array.isArray(out.payload.failed_checks) && out.payload.failed_checks.includes('threshold_quorum_requirements'), 'failed check id should be listed');
  ...
  console.log('insider_threat_split_trust_command_governance.test.js: OK');
} catch (err) {
  console.error('insider_threat_split_trust_command_governance.test.js: FAIL: ' + err.message);
  process.exit(1);
}
