...
const SCRIPT = path.join(ROOT, 'client/runtime/systems/continuity/sovereign_decommission_succession_protocol.js');
...
try {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'sovereign_decommission_succession_protocol-'));
  ...
  let out = run(['decommission', '--policy=' + policyPath, '--strict=1', '--apply=1']);
  ...
  out = run(['decommission', '--policy=' + policyPath, '--strict=1', '--apply=1', '--fail-checks=cryptographic_handoff']);
  ...
  assert.ok(Array.isArray(out.payload.failed_checks) && out.payload.failed_checks.includes('cryptographic_handoff'), 'failed check id should be listed');
  ...
  console.log('sovereign_decommission_succession_protocol.test.js: OK');
} catch (err) {
  console.error('sovereign_decommission_succession_protocol.test.js: FAIL: ' + err.message);
  process.exit(1);
}
