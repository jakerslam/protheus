...
const SCRIPT = path.join(ROOT, 'client/runtime/systems/security/independent_safety_coprocessor_veto_plane.js');
...
try {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'independent_safety_coprocessor_veto_plane-'));
  ...
  let out = run(['veto', '--policy=' + policyPath, '--strict=1', '--apply=1']);
  ...
  out = run(['veto', '--policy=' + policyPath, '--strict=1', '--apply=1', '--fail-checks=independent_guardian_runtime']);
  ...
  assert.ok(Array.isArray(out.payload.failed_checks) && out.payload.failed_checks.includes('independent_guardian_runtime'), 'failed check id should be listed');
  ...
  console.log('independent_safety_coprocessor_veto_plane.test.js: OK');
} catch (err) {
  console.error('independent_safety_coprocessor_veto_plane.test.js: FAIL: ' + err.message);
  process.exit(1);
}
