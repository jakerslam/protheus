...
const SCRIPT = path.join(ROOT, 'client/runtime/systems/security/formal_mind_sovereignty_verification.js');
...
try {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'formal_mind_sovereignty_verification-'));
  ...
  out = run(['verify', '--policy=' + policyPath, '--strict=1', '--apply=1', '--fail-checks=sovereignty_invariant_spec']);
  ...
  assert.ok(Array.isArray(out.payload.failed_checks) && out.payload.failed_checks.includes('sovereignty_invariant_spec'), 'failed check id should be listed');
  ...
  console.log('formal_mind_sovereignty_verification.test.js: OK');
} catch (err) {
  console.error('formal_mind_sovereignty_verification.test.js: FAIL: ' + err.message);
  process.exit(1);
}
