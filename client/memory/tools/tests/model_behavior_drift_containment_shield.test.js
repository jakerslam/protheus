...
const SCRIPT = path.join(ROOT, 'client/runtime/systems/routing/model_behavior_drift_containment_shield.js');
...
try {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'model_behavior_drift_containment_shield-'));
  ...
  let out = run(['fingerprint', '--policy=' + policyPath, '--strict=1', '--apply=1']);
  ...
  out = run(['fingerprint', '--policy=' + policyPath, '--strict=1', '--apply=1', '--fail-checks=model_fingerprint_canary']);
  ...
  assert.ok(Array.isArray(out.payload.failed_checks) && out.payload.failed_checks.includes('model_fingerprint_canary'), 'failed check id should be listed');
  ...
  console.log('model_behavior_drift_containment_shield.test.js: OK');
} catch (err) {
  console.error('model_behavior_drift_containment_shield.test.js: FAIL: ' + err.message);
  process.exit(1);
}
