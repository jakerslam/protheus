...
const SCRIPT = path.join(ROOT, 'client/runtime/systems/ops/legal_regulatory_autodiff_governance_router.js');
...
try {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'legal_regulatory_autodiff_governance_router-'));
  ...
  let out = run(['diff', '--policy=' + policyPath, '--strict=1', '--apply=1']);
  ...
  out = run(['diff', '--policy=' + policyPath, '--strict=1', '--apply=1', '--fail-checks=legal_source_diff_ingestion']);
  ...
  assert.ok(Array.isArray(out.payload.failed_checks) && out.payload.failed_checks.includes('legal_source_diff_ingestion'), 'failed check id should be listed');
  ...
  console.log('legal_regulatory_autodiff_governance_router.test.js: OK');
} catch (err) {
  console.error('legal_regulatory_autodiff_governance_router.test.js: FAIL: ' + err.message);
  process.exit(1);
}
