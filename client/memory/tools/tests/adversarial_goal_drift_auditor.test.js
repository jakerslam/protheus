...
const SCRIPT = path.join(ROOT, 'client/runtime/systems/security/adversarial_goal_drift_auditor.js');
...
try {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'adversarial_goal_drift_auditor-'));
  ...
  let out = run(['audit', '--policy=' + policyPath, '--strict=1', '--apply=1']);
  ...
  out = run(['audit', '--policy=' + policyPath, '--strict=1', '--apply=1', '--fail-checks=independent_objective_lens']);
  ...
  assert.ok(Array.isArray(out.payload.failed_checks) && out.payload.failed_checks.includes('independent_objective_lens'), 'failed check id should be listed');
  ...
  console.log('adversarial_goal_drift_auditor.test.js: OK');
} catch (err) {
  console.error('adversarial_goal_drift_auditor.test.js: FAIL: ' + err.message);
  process.exit(1);
}
