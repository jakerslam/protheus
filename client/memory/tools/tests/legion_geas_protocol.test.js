...
const SCRIPT = path.join(ROOT, 'client/runtime/systems/security/legion_geas_protocol.js');
...
try {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'legion_geas_protocol-'));
  ...
  console.log('legion_geas_protocol.test.js: OK');
} catch (err) {
  console.error('legion_geas_protocol.test.js: FAIL: ' + err.message);
  process.exit(1);
}
