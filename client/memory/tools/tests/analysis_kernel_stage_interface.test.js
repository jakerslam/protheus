...
const SCRIPT = path.join(ROOT, 'client/runtime/systems/sensory/analysis_kernel_stage_interface.js');
...
try {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'analysis_kernel_stage_interface-'));
  ...
  let out = run(['emit', '--policy=' + policyPath, '--strict=1', '--apply=1']);
  ...
  out = run(['emit', '--policy=' + policyPath, '--strict=1', '--apply=1', '--fail-checks=stage_contract_interface']);
  ...
  assert.ok(Array.isArray(out.payload.failed_checks) && out.payload.failed_checks.includes('stage_contract_interface'), 'failed check id should be listed');
  ...
  console.log('analysis_kernel_stage_interface.test.js: OK');
} catch (err) {
  console.error('analysis_kernel_stage_interface.test.js: FAIL: ' + err.message);
  process.exit(1);
}
