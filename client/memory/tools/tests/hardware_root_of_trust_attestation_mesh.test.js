...
const SCRIPT = path.join(ROOT, 'client/runtime/systems/security/hardware_root_of_trust_attestation_mesh.js');
...
try {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'hardware_root_of_trust_attestation_mesh-'));
  ...
  out = run(['attest', '--policy=' + policyPath, '--strict=1', '--apply=1', '--fail-checks=tpm_tee_enrollment']);
  ...
  assert.ok(Array.isArray(out.payload.failed_checks) && out.payload.failed_checks.includes('tpm_tee_enrollment'), 'failed check id should be listed');
  ...
  console.log('hardware_root_of_trust_attestation_mesh.test.js: OK');
} catch (err) {
  console.error('hardware_root_of_trust_attestation_mesh.test.js: FAIL: ' + err.message);
  process.exit(1);
}
