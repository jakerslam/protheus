...
const SCRIPT = path.join(ROOT, 'client/runtime/systems/security/phoenix_protocol_respawn_continuity.js');
...
try {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'phoenix_protocol_respawn_continuity-'));
  ...
  let out = run(['respawn', '--policy=' + policyPath, '--strict=1', '--apply=1']);
  ...
  out = run(['respawn', '--policy=' + policyPath, '--strict=1', '--apply=1', '--fail-checks=red_legion_down_trigger']);
  ...
  assert.ok(Array.isArray(out.payload.failed_checks) && out.payload.failed_checks.includes('red_legion_down_trigger'), 'failed check id should be listed');
  ...
  console.log('phoenix_protocol_respawn_continuity.test.js: OK');
} catch (err) {
  console.error('phoenix_protocol_respawn_continuity.test.js: FAIL: ' + err.message);
  process.exit(1);
}
