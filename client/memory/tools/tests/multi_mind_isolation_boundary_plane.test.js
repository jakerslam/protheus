...
const SCRIPT = path.join(ROOT, 'client/runtime/systems/security/multi_mind_isolation_boundary_plane.js');
...
try {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'multi_mind_isolation_boundary_plane-'));
  ...
  let out = run(['partition', '--policy=' + policyPath, '--strict=1', '--apply=1']);
  ...
  out = run(['partition', '--policy=' + policyPath, '--strict=1', '--apply=1', '--fail-checks=per_mind_namespaces']);
  ...
  assert.ok(Array.isArray(out.payload.failed_checks) && out.payload.failed_checks.includes('per_mind_namespaces'), 'failed check id should be listed');
  ...
  console.log('multi_mind_isolation_boundary_plane.test.js: OK');
} catch (err) {
  console.error('multi_mind_isolation_boundary_plane.test.js: FAIL: ' + err.message);
  process.exit(1);
}
