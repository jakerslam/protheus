...
const SCRIPT = path.join(ROOT, 'client/runtime/systems/security/crimson_wraith_protocol.js');
...
try {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'crimson_wraith_protocol-'));
  ...
  let out = run(['mission', '--policy=' + policyPath, '--strict=1', '--apply=1']);
  ...
  out = run(['mission', '--policy=' + policyPath, '--strict=1', '--apply=1', '--fail-checks=one_shot_spawn_type']);
  ...
  assert.ok(Array.isArray(out.payload.failed_checks) && out.payload.failed_checks.includes('one_shot_spawn_type'), 'failed check id should be listed');
  ...
  console.log('crimson_wraith_protocol.test.js: OK');
} catch (err) {
  console.error('crimson_wraith_protocol.test.js: FAIL: ' + err.message);
  process.exit(1);
}
