...
1. Move routine file to `client/cognition/habits/_archive/<id>.<date>.js`
2. Mark registry `status: archived`
3. Keep trusted hash record, add `"status": "archived"`
4. Log to `client/runtime/local/logs/habits/gc.ndjson`:
   ```json
...
| Write paths | Explicit only | No broad globs beyond `client/memory/*.md`, `client/runtime/local/logs/habits/*`, `client/cognition/habits/registry.json` |
...
### Per-Run Logging
Each habit run logs to `client/runtime/local/logs/habits/habit_runs.ndjson`:
```json
{
  "ts": "ISO-8601",
  "habit_id": "...",
