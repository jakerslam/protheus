...
function main() {
  ...
  const writeAllowlist = writeAllowIndex !== -1 && args[writeAllowIndex + 1]
    ...
    : ['client/memory/*.md', 'client/runtime/local/logs/habits/*'];
  ...
  const proposal = {
    ...
    entrypoint: `client/cognition/habits/routines/${slug}.js`,
  ...
  console.log(`  1. Create: client/cognition/habits/routines/${slug}.js`);
  console.log(`  2. Trust: node client/cognition/habits/scripts/trust_add_habit.js client/cognition/habits/routines/${slug}.js "${description.substring(0, 50)}"`);
