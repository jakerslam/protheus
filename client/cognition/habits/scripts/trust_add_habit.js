...
const TRUSTED_HABITS_PATH = '/Users/jay/.openclaw/workspace/client/runtime/config/trusted_habits.json';
...
function main() {
  ...
  if (args.length < 2) {
    ...
    console.error('Example: node trust_add_habit.js client/cognition/habits/routines/rebuild_validate_memory.js "Core validation habit"');
    process.exit(1);
  }
  ...
  // Check file is in client/cognition/habits/routines
  if (!targetPath.includes('client/cognition/habits/routines')) {
    console.error('ERROR: Habit must be in client/cognition/habits/routines/ directory');
    process.exit(1);
  }
