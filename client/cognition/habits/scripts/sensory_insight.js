 ...
 * 
 * Commands:
 *   node client/cognition/habits/scripts/sensory_insight.js daily [YYYY-MM-DD]
 *   node client/cognition/habits/scripts/sensory_insight.js weekly [YYYY-MM-DD]
 * 
 * Outputs:
...
function generateDailyProposals(dateStr, digestContent, anomalyData) {
  ...
  if (fileChangeSpike) {
    const proposal = createProposal(
      ...
      'node client/cognition/habits/scripts/sensory_capture.js capture --lookback-hours=12'
    );
  ...
  if (aqsDrop) {
    const proposal = createProposal(
      ...
      'node client/memory/tools/tests/aie.test.js && node client/cognition/habits/scripts/sensory_insight.js daily'
    );
    addEvidence(proposal, 'anomaly', `state/sensory/anomalies/${dateStr}.json`,
  ...
  if (typeof signalRatio === 'number' && signalRatio < 0.6) {
    const proposal = createProposal(
      ...
      'low',
      [
        'Review capture ignore_patterns in client/runtime/config/work_roots.json',
      ...
      'node client/cognition/habits/scripts/sensory_capture.js capture && check signal_ratio'
    );
  ...
  if (!digestContent) {
    const proposal = createProposal(
      ...
      'node client/cognition/habits/scripts/sensory_capture.js capture && node client/cognition/habits/scripts/sensory_digest.js daily'
    );
...
function generateWeeklyProposals(dates, dailyAnalyses) {
  ...
  if (lowSignalDays >= 3) {
    const proposal = createProposal(
      ...
      'review client/runtime/config/work_roots.json ignore_patterns'
    );
  ...
  switch (cmd) {
    case 'daily': {
      ...
      if (!digestContent && !anomalyData) {
        ...
        console.log(`   Run: node client/cognition/habits/scripts/sensory_digest.js daily ${dateStr}`);
        process.exit(1);
      }
    ...
    default:
      ...
      console.log('  node client/cognition/habits/scripts/sensory_insight.js daily [YYYY-MM-DD]');
      console.log('  node client/cognition/habits/scripts/sensory_insight.js weekly [YYYY-MM-DD]');
      console.log('');
      console.log('Outputs:');
