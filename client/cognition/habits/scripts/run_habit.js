...
const RECEIPTS_DIR = path.join(CLIENT_ROOT, 'runtime', 'local', 'state', 'client', 'cognition', 'habits', 'receipts');
...
function verifyHabitOrThrow(habitId, habit, trusted) {
  ...
  if (!trustedEntry) {
    throw new Error(
      ...
      `To approve: node client/cognition/habits/scripts/trust_add_habit.js ${entrypoint} "habit approval: ${habitId}"`
    );
  }
