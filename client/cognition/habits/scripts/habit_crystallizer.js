 ...
 *
 * Usage:
 *   node client/cognition/habits/scripts/habit_crystallizer.js --from "task text" [--tokens_est N] [--repeats_14d N] [--errors_30d N] [--intent_key key]
 */
...
function usage() {
  console.log('Usage:');
  console.log('  node client/cognition/habits/scripts/habit_crystallizer.js --from "task text" [--tokens_est N] [--repeats_14d N] [--errors_30d N] [--intent_key key] [--auto_trust=0|1]');
  console.log('');
...
function addHabit(registry, params) {
  ...
  const entrypoint = `client/cognition/habits/routines/${habitId}.js`;
  ...
  const record = {
    ...
    test_plan: `Run node client/cognition/habits/scripts/run_habit.js --id ${habitId} --json '{}' and verify stable output.`,
    idempotent: true,
    inputs_schema: {
      type: 'object',
      required: [],
    ...
    permissions: {
      network: 'deny',
      write_paths_allowlist: ['client/runtime/local/logs/habits/*'],
      exec_allowlist: []
    },
    status: 'candidate',
    outcome: {
