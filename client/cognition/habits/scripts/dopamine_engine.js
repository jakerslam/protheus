  ...
  if (mode === 'git') {
    ...
    try {
      ...
      const findCmd = `find "${workspaceRoot}" -type f -mtime -1 \
        -not -path "*/state/*" \
        -not -path "*/client/runtime/local/logs/*" \
        -not -path "*/node_modules/*" \
        -not -path "*/.git/*" \
        -not -path "*/client/runtime/config/trusted_skills.json" \
...
function closeout() {
  ...
  if (!summary.has_artifacts) {
    suggestion = "→ Run: node client/cognition/habits/scripts/dopamine_engine.js log_artifact note \"what you shipped\"";
  } else if (Number(summary.revenue_actions_verified_count || 0) < 1) {
    suggestion = "→ Run: node client/cognition/habits/scripts/dopamine_engine.js revenue lead \"what revenue move you made\" --verified";
  ...
  switch (cmd) {
    ...
    case 'log_artifact': {
      ...
      if (!ref) {
        ...
        console.log('Example: log_artifact file client/runtime/patches/websocket.js T1_make_jay_billionaire_v1');
        break;
      }
    ...
    case 'agent_test': {
      ...
      const log1 = agentLog({
        ...
        artifacts: [{ type: 'file', ref: 'client/cognition/habits/git-hooks/post-commit' }],
        outcome: 'success'
      });
