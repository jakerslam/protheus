...
const REFLEX_SPECS: ReflexSpec[] = [
  {
    ...
    build: (input) => ({
      ...
      command: `node client/runtime/systems/memory/memory_recall.js query --q="${escapeArg(input || 'current context')}" --top=1 --expand=none --context-budget-tokens=1200 --context-budget-mode=trim`
    })
  },
  {
    id: 'write_quick',
  ...
  },
  {
    ...
    build: (input) => ({
      ...
      command: `node client/runtime/systems/memory/memory_recall.js query --q="${escapeArg(input || 'relevant context')}" --top=3 --expand=none --context-budget-tokens=1500 --context-budget-mode=trim`
    })
  }
];
...
function usage() {
  console.log('Usage:');
  console.log('  node client/cognition/reflexes/index.js list');
  console.log('  node client/cognition/reflexes/index.js run --id=<read_snippet|write_quick|summarize_brief|git_status|memory_lookup> [--input="..."]');
}
