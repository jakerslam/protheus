...
- `client/state` symlink (legacy runtime alias)
- root `local/` runtime leakage (migrated into `client/local/state/*`)

## Canonical runtime roots

- Client runtime/user data: `client/local/*`
...
- `client/systems/ops/runtime_state_surface_guard.ts`
- `client/systems/ops/migrate_cleanup.ts`
...
- Follow-up item: `V6-ROOT-INTERNAL-003` in `TODO.md` to either migrate these to `client/local/internal/*` or formally keep them as root exceptions.
