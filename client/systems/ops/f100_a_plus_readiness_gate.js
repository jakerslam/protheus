...
function evaluateChecks() {
  ...
  checks.push({
    ...
    ok: fileExists('client/docs/ENTERPRISE_SUPPORT_ENVELOPE_TEMPLATE.md'),
    expected: 'support template exists',
    actual: fileExists('client/docs/ENTERPRISE_SUPPORT_ENVELOPE_TEMPLATE.md') ? 'present' : 'missing',
    source: 'client/docs/ENTERPRISE_SUPPORT_ENVELOPE_TEMPLATE.md'
  });

  checks.push({
    id: 'case_study_template_present',
    ok: fileExists('client/docs/REFERENCE_CUSTOMER_CASE_STUDY_TEMPLATE.md'),
    expected: 'case study template exists',
    actual: fileExists('client/docs/REFERENCE_CUSTOMER_CASE_STUDY_TEMPLATE.md') ? 'present' : 'missing',
    source: 'client/docs/REFERENCE_CUSTOMER_CASE_STUDY_TEMPLATE.md'
  });

  checks.push({
    id: 'legal_packet_checklist_present',
    ok: fileExists('client/docs/LEGAL_ENTERPRISE_PACKET_CHECKLIST.md'),
    expected: 'legal packet checklist exists',
    actual: fileExists('client/docs/LEGAL_ENTERPRISE_PACKET_CHECKLIST.md') ? 'present' : 'missing',
    source: 'client/docs/LEGAL_ENTERPRISE_PACKET_CHECKLIST.md'
  });

  const humanActionsPath = 'client/docs/HUMAN_ONLY_ACTIONS.md';
...
function main() {
  ...
  process.stderr.write(
    `unknown_command:${args.command}\nusage: node client/systems/ops/f100_a_plus_readiness_gate.js [run|status] [--strict=1] [--write=1]\n`
  );
  process.exit(1);
}
