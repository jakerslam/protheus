...
function loadPolicy(args: Record<string, any>) {
  ...
  const statePath = resolvePath(paths.state_path || 'client/local/state/ops/runtime_retention_prune/latest.json');
  const receiptsPath = resolvePath(paths.receipts_path || 'client/local/state/ops/runtime_retention_prune/receipts.jsonl');
