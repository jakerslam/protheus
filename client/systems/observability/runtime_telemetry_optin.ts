...
function defaultPolicy(): JsonMap {
  return {
    ...
    enabled: false,
    paths: {
      events_path: 'client/local/state/observability/runtime_telemetry.jsonl',
      aggregate_path: 'client/local/state/observability/runtime_telemetry_latest.json'
    },
    privacy: {
...
function main(): void {
  ...
  process.stderr.write(
    'usage: node client/lib/ts_entrypoint.js client/systems/observability/runtime_telemetry_optin.ts <status|emit-usage|emit-crash|emit-perf> [--counter=.. --value=..]\n'
  );
  process.exit(1);
}
