...
function usage() {
  ...
  process.stdout.write('  node client/systems/migration/core_migration_bridge.js run --to=<owner/repo|url> [--workspace=<path>] [--apply=1|0] [--policy=<path>]\n');
  process.stdout.write('  node client/systems/migration/core_migration_bridge.js rollback --migration-id=<id> [--apply=1|0] [--approval-note=<note>] [--policy=<path>]\n');
  process.stdout.write('  node client/systems/migration/core_migration_bridge.js status [--policy=<path>]\n');
}
...
function loadPolicy(root: string, flags: JsonMap): { policyPath: string, policy: Policy } {
  const policyPath = resolvePath(
    root,
    flags.policy,
    'client/config/core_migration_bridge_policy.json'
  );
