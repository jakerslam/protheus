...
const TARGET = '../../../../surface/orchestration/scripts/strategy_learner.ts';
...
function loadTarget() {
  try {
    ...
    return {
      ok: false,
      error: 'strategy_learner_target_load_failed',
...
function run(args = process.argv.slice(2)) {
  if (!target || target.ok === false) {
    process.stderr.write(`${JSON.stringify(target || { ok: false, error: 'strategy_learner_target_unavailable' })}\n`);
    return 1;
  }
  if (typeof target.run !== 'function') {
    process.stderr.write(`${JSON.stringify({ ok: false, error: 'strategy_learner_target_missing_run' })}\n`);
    return 1;
  }
