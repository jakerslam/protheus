...
const DREAM_BUDGET_PREVIEW_BYPASS = String(process.env.IDLE_DREAM_BUDGET_PREVIEW_BYPASS || '0').trim() !== '0';
...
const CROSS_DOMAIN_SEEDS_ENABLED = String(process.env.IDLE_DREAM_CROSS_DOMAIN_ENABLED || '1').trim() !== '0';
const CROSS_DOMAIN_SEEDS_MAX = clampInt(process.env.IDLE_DREAM_CROSS_DOMAIN_MAX || 4, 0, 16);
const CROSS_DOMAIN_SEEDS_MIN_PAIR_SCORE = clampFloat(process.env.IDLE_DREAM_CROSS_DOMAIN_MIN_PAIR_SCORE || 8, 1, 200, 8);
const CROSS_DOMAIN_SEEDS_MIN_VALUE_SCORE = clampFloat(process.env.IDLE_DREAM_CROSS_DOMAIN_MIN_VALUE_SCORE || 8, 1, 200, 8);
const CROSS_DOMAIN_SEEDS_REQUIRE_OBJECTIVE = String(process.env.IDLE_DREAM_CROSS_DOMAIN_REQUIRE_OBJECTIVE || '1').trim() !== '0';
const CROSS_DOMAIN_SEEDS_REQUIRE_T1_ROOT = String(process.env.IDLE_DREAM_CROSS_DOMAIN_REQUIRE_T1_ROOT || '1').trim() !== '0';
const CROSS_DOMAIN_SEEDS_DEFAULT_OBJECTIVE_ID = normalizeText(process.env.IDLE_DREAM_CROSS_DOMAIN_DEFAULT_OBJECTIVE_ID || '');
...
function collectCrossDomainSeeds(dreamSeeds, hyperSeeds) {
  if (!CROSS_DOMAIN_SEEDS_ENABLED || CROSS_DOMAIN_SEEDS_MAX <= 0) return [];
  if (!Array.isArray(dreamSeeds) || !Array.isArray(hyperSeeds)) return [];
  if (dreamSeeds.length <= 0 || hyperSeeds.length <= 0) return [];
  const mapped = mapCrossDomainRows({
    ...
    max_mappings: CROSS_DOMAIN_SEEDS_MAX,
    min_pair_score: CROSS_DOMAIN_SEEDS_MIN_PAIR_SCORE,
    min_value_score: CROSS_DOMAIN_SEEDS_MIN_VALUE_SCORE,
    require_objective: CROSS_DOMAIN_SEEDS_REQUIRE_OBJECTIVE,
    require_t1_root: CROSS_DOMAIN_SEEDS_REQUIRE_T1_ROOT,
    default_objective_id: CROSS_DOMAIN_SEEDS_DEFAULT_OBJECTIVE_ID || ''
  });
  const selected = Array.isArray(mapped && mapped.selected) ? mapped.selected : [];
  return selected.map((row) => ({
    token: normalizeToken(row && row.token),
    score: clampInt(Math.round(Number(row && row.value_score || 0)), 1, 1000),
    source: 'cross_domain_mapper',
    sources: ['cross_domain_mapper', 'memory_dream', 'hyper_creative_mode'],
    refs: Array.isArray(row && row.refs) ? row.refs.slice(0, 4) : []
  })).filter((row) => !!row.token);
}

function buildIdleSeedSet(dateStr) {
  const dreamSeeds = collectDreamSeeds(dateStr, WINDOW_DAYS);
  const hyperSeeds = collectHyperSeeds(dateStr, WINDOW_DAYS);
  const failureSeeds = collectFailureSeeds(dateStr, WINDOW_DAYS);
  const crossDomainSeeds = collectCrossDomainSeeds(dreamSeeds, hyperSeeds);
  const seeds = [
    ...dreamSeeds,
    ...hyperSeeds,
    ...failureSeeds,
  ...
  const prioritizedFailureSeeds = ranked.filter((s) => Array.isArray(s.sources) && s.sources.includes('failure_memory'));
  if (prioritizedFailureSeeds.length <= 0) return ranked.slice(0, MAX_SEEDS);
  ...
  const selected = [];
  const used = new Set();
  for (const s of prioritizedFailureSeeds.slice(0, quota)) {
    selected.push(s);
    used.add(s.token);
  }
