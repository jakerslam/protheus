...
function approvalsPage() {
  return {
    ...
    async approve(id) {
      try {
        await InfringAPI.post('/api/approvals/' + id + '/approve', {});
        InfringToast.success('Approved');
        await this.loadData();
      } catch(e) {
        InfringToast.error(e.message);
      }
    },
    ...
    async reject(id) {
      ...
      InfringToast.confirm('Reject Action', 'Are you sure you want to reject this action?', async function() {
        try {
          await InfringAPI.post('/api/approvals/' + id + '/reject', {});
          InfringToast.success('Rejected');
          await self.loadData();
        } catch(e) {
          InfringToast.error(e.message);
        }
