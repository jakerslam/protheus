...
function resolveEngineInvocation(cwd) {
  const engineJs = path.join(cwd, 'client', 'runtime', 'systems', 'security', 'formal_invariant_engine.js');
  if (hasFile(engineJs)) {
    return [engineJs];
  }
  const engineTs = path.join(cwd, 'client', 'runtime', 'systems', 'security', 'formal_invariant_engine.ts');
