...
function resolveWorkspacePath(raw: unknown) {
  ...
  const rewrite = (input: string) => {
    ...
    if (norm === 'local' || norm.startsWith('local/')) {
      ...
      return path.join('client', 'runtime', 'local', suffix);
    }
...
function loadSpec() {
  const base = {
    ...
    schema_version: '1.0',
    state_path: 'client/runtime/local/state/security/formal_invariant_engine/latest.json',
    history_path: 'client/runtime/local/state/security/formal_invariant_engine/history.jsonl',
    invariants: []
  };
