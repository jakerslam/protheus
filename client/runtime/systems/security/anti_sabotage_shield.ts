...
function storagePathForRel(rel) {
  const parts = String(rel == null ? '' : rel)
    .replace(/\\/g, '/')
    .split('/')
    .filter(Boolean)
    .map((part) => {
      if (part === '..') return '__up__';
      return part.replace(/[^a-zA-Z0-9._-]+/g, '_');
    });
  return parts.length ? parts.join('/') : '__root__';
}

function normalizeManifestHashEntries(manifest) {
  const hashes = manifest && manifest.hashes && typeof manifest.hashes === 'object'
    ? manifest.hashes
    : {};
  const out = {};
  for (const [rel, value] of Object.entries(hashes)) {
    const relKey = String(rel || '');
    if (!relKey) continue;
    if (typeof value === 'string') {
      out[relKey] = { hash: value, storage_path: storagePathForRel(relKey) };
      continue;
    }
    if (value && typeof value === 'object') {
      const hash = normalizeText(value.hash || value.sha256 || '', 160);
      if (!hash) continue;
      out[relKey] = {
        hash,
        storage_path: normalizeText(value.storage_path || value.storagePath || storagePathForRel(relKey), 320) || storagePathForRel(relKey)
      };
    }
  }
  return out;
...
function readLatestIncidentSummary(policy) {
  const fp = incidentLogPath(policy);
  if (!fs.existsSync(fp)) return null;
  let lines = [];
  try {
    lines = String(fs.readFileSync(fp, 'utf8') || '')
      .split('\n')
      .map((line) => line.trim())
      .filter(Boolean);
  } catch {
  ...
  for (let i = lines.length - 1; i >= 0; i -= 1) {
    try {
      ...
      if (String(row.type || '') !== 'anti_sabotage_verify') continue;
      return {
        incident_id: normalizeText(row.incident_id || '', 120) || null,
        ts: normalizeText(row.ts || '', 64) || null,
        source: normalizeText(row.source || '', 64) || null,
        snapshot_id: normalizeText(row.snapshot_id || '', 120) || null,
        violated: row.violated === true,
        mismatch_count: Math.max(0, Number(row.mismatch_count || 0)),
        missing_count: Math.max(0, Number(row.missing_count || 0)),
        extra_count: Math.max(0, Number(row.extra_count || 0)),
        rollback_plan_hash: normalizeText(
          row.rollback_plan && (row.rollback_plan.plan_hash || row.rollback_plan.hash)
            ? row.rollback_plan.plan_hash || row.rollback_plan.hash
            : '',
          120
        ) || null
      };
...
function createSnapshot(policy, label = '') {
  ...
  for (const absFile of files) {
    ...
    const storagePath = storagePathForRel(rel);
    hashes[rel] = {
      hash: hashFile(absFile),
      storage_path: storagePath
    };
    copyFileTo(absFile, path.join(paths.files_root, storagePath));
  }
  const manifest = {
...
function evaluateSnapshot(policy, loaded) {
  ...
  const expectedHashes = normalizeManifestHashEntries(manifest);
  ...
  for (const [rel, expected] of Object.entries(expectedHashes)) {
    const expectedHash = normalizeText(expected && expected.hash || '', 160);
    const storagePath = normalizeText(expected && expected.storage_path || storagePathForRel(rel), 320) || storagePathForRel(rel);
    const abs = currentByRel.get(rel);
    if (!abs || !fs.existsSync(abs)) {
      missing.push({ file: rel, storage_path: storagePath });
      continue;
    }
    const actual = hashFile(abs);
    if (String(actual) !== String(expectedHash)) {
      mismatches.push({ file: rel, expected: expectedHash, actual, storage_path: storagePath });
    }
  }
...
function restoreFromSnapshot(policy, loaded, evalOut) {
  ...
  for (const row of evalOut.mismatches) {
    const rel = row.file;
    const storagePath = normalizeText(row.storage_path || storagePathForRel(rel), 320) || storagePathForRel(rel);
    const absCurrent = path.resolve(ROOT, rel);
    const absSnapshot = path.join(loaded.paths.files_root, storagePath);
    if (fs.existsSync(absCurrent)) {
      copyFileTo(absCurrent, path.join(quarantineRoot, storagePath));
...
function runVerifyOnce(policy, opts = {}) {
  ...
  const rollbackPlan = {
    restore_count: evalOut.mismatches.length + evalOut.missing.length,
    quarantine_count: evalOut.mismatches.length + evalOut.extra.length,
    delete_count: evalOut.extra.length,
    plan_hash: stableHash(JSON.stringify({
      snapshot_id: loaded.snapshot_id,
      mismatches: evalOut.mismatches.map((row) => row.file),
      missing: evalOut.missing.map((row) => row.file),
      extra: evalOut.extra.map((row) => row.file)
    })).slice(0, 20)
  };
  const incidentId = recovery && recovery.incident_id
...
function cmdStatus() {
  ...
  const latestIncidentSummary = readLatestIncidentSummary(policy);
  const out = {
    ok: true,
    ...
    latest_incident_summary: latestIncidentSummary,
