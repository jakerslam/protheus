...
const DEFAULT_POLICY_PATH = process.env.SYSTEM_MAP_GENERATOR_POLICY_PATH
  ...
  : path.join(ROOT, 'client', 'runtime', 'config', 'system_map_generator_policy.json');

function usage() {
  console.log('Usage:');
  console.log('  node client/runtime/systems/ops/system_map_generator.js run [--apply=1|0] [--strict=1|0] [--policy=<path>]');
  console.log('  node client/runtime/systems/ops/system_map_generator.js status [--policy=<path>]');
}
...
function defaultPolicy() {
  return {
    ...
    enabled: true,
    strict_default: true,
    registry_path: 'client/runtime/config/system_map_registry.json',
    outputs: {
      markdown_path: 'docs/client/architecture/SYSTEM_MAP.md',
      latest_path: 'client/runtime/local/state/ops/system_map/latest.json',
      history_path: 'client/runtime/local/state/ops/system_map/history.jsonl'
    }
  };
}
...
function renderMarkdown(entries: MapEntry[], generatedAt: string) {
  ...
  lines.push('');
  lines.push('This map is generated from `client/runtime/config/system_map_registry.json` via `system_map_generator` and is the canonical quick-reference for subsystem purpose, ownership, and health checks.');
  lines.push('');
