...
const DEFAULT_POLICY_PATH = process.env.SOURCE_RUNTIME_CLASSIFIER_POLICY_PATH
  ...
  : (
    fs.existsSync(path.join(ROOT, 'client', 'runtime', 'config', 'source_runtime_classifier_policy.json'))
      ? path.join(ROOT, 'client', 'runtime', 'config', 'source_runtime_classifier_policy.json')
      : path.join(ROOT, 'config', 'source_runtime_classifier_policy.json')
  );

function usage() {
  console.log('Usage:');
  console.log('  node client/runtime/systems/ops/source_runtime_classifier_contract.js check [--strict=1|0] [--policy=<path>]');
  console.log('  node client/runtime/systems/ops/source_runtime_classifier_contract.js status [--policy=<path>]');
}
...
function defaultPolicy() {
  return {
    ...
    enabled: true,
    strict_default: true,
    runtime_roots: ['client/runtime/local', 'core/local'],
    source_memory_root: 'client/memory',
    runtime_memory_roots: ['client/runtime/local/memory', 'client/runtime/local/state/memory'],
    ...
    paths: {
      latest_path: 'client/runtime/local/state/ops/source_runtime_classifier/latest.json',
      receipts_path: 'client/runtime/local/state/ops/source_runtime_classifier/receipts.jsonl'
    }
  };
}
