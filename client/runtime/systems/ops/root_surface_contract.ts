...
const DEFAULT_POLICY_PATH = process.env.ROOT_SURFACE_CONTRACT_POLICY_PATH
  ...
  : (
    fs.existsSync(path.join(ROOT, 'client', 'runtime', 'config', 'root_surface_contract.json'))
      ? path.join(ROOT, 'client', 'runtime', 'config', 'root_surface_contract.json')
      : path.join(ROOT, 'config', 'root_surface_contract.json')
  );

function usage() {
  console.log('Usage:');
  console.log('  node client/runtime/systems/ops/root_surface_contract.js check [--strict=1|0] [--policy=<path>]');
  console.log('  node client/runtime/systems/ops/root_surface_contract.js status [--policy=<path>]');
}
...
function defaultPolicy() {
  return {
    ...
    paths: {
      latest_path: 'client/runtime/local/state/ops/root_surface_contract/latest.json',
      receipts_path: 'client/runtime/local/state/ops/root_surface_contract/receipts.jsonl'
    }
  };
}
