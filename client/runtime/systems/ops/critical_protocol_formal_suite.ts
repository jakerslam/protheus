...
const DEFAULT_POLICY_PATH = process.env.CRITICAL_PROTOCOL_FORMAL_SUITE_POLICY_PATH
  ...
  : path.join(ROOT, 'client', 'runtime', 'config', 'critical_protocol_formal_suite_policy.json');

function usage() {
  console.log('Usage:');
  console.log('  node client/runtime/systems/ops/critical_protocol_formal_suite.js run [--strict=1|0] [--policy=<path>]');
  console.log('  node client/runtime/systems/ops/critical_protocol_formal_suite.js status [--policy=<path>]');
}
...
function defaultPolicy() {
  return {
    ...
    required_paths: [
      ...
      'client/runtime/systems/actuation/actuation_executor.js',
      'client/runtime/systems/security/critical_path_formal_verifier.js'
    ],
    verifier_cmd: ['node', 'client/runtime/systems/security/critical_path_formal_verifier.js', 'run', '--strict=1'],
    paths: {
      latest_path: 'client/runtime/local/state/ops/critical_protocol_formal_suite/latest.json',
      receipts_path: 'client/runtime/local/state/ops/critical_protocol_formal_suite/receipts.jsonl'
    }
  };
}
