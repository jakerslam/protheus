...
const DEFAULT_POLICY_PATH = process.env.ENTRYPOINT_RUNTIME_CONTRACT_POLICY_PATH
  ...
  : (
    fs.existsSync(path.join(ROOT, 'client', 'runtime', 'config', 'entrypoint_runtime_contract_policy.json'))
      ? path.join(ROOT, 'client', 'runtime', 'config', 'entrypoint_runtime_contract_policy.json')
...
function usage() {
  console.log('Usage:');
  console.log('  node client/runtime/systems/ops/entrypoint_runtime_contract.js check [--strict=1|0] [--policy=<path>]');
  console.log('  node client/runtime/systems/ops/entrypoint_runtime_contract.js status [--policy=<path>]');
}
...
function defaultPolicy() {
  return {
    ...
    paths: {
      ...
      latest_path: 'client/runtime/local/state/ops/entrypoint_runtime_contract/latest.json',
      receipts_path: 'client/runtime/local/state/ops/entrypoint_runtime_contract/receipts.jsonl'
    }
  };
}
