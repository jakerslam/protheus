...
const DEFAULT_POLICY_PATH = process.env.BACKLOG_IMPLEMENTATION_REVIEW_POLICY_PATH
  ...
  : (() => {
      const migrated = path.join(ROOT, 'client', 'runtime', 'config', 'backlog_implementation_review_policy.json');
...
function loadPolicy(policyPath = DEFAULT_POLICY_PATH) {
  ...
  return {
    ...
    source_registry_path: resolvePath(raw.source_registry_path, 'client/runtime/config/backlog_registry.json'),
    outputs: {
      review_registry_path: resolvePath(outputs.review_registry_path, 'client/runtime/config/backlog_review_registry.json'),
      reviewed_view_path: resolvePath(outputs.reviewed_view_path, 'docs/client/backlog_views/reviewed.md'),
    ...
    search: {
      ...
      exclude_paths: asList(
        search.exclude_paths || [
          ...
          'client/runtime/config/backlog_review_registry.json',
          'node_modules/'
        ],
        260
      )
...
function classifyBucket(relPath: string) {
  ...
  if (v.startsWith('config/') || v.startsWith('client/runtime/config/')) return 'config';
  if (v.startsWith('docs/') || v.startsWith('docs/client/')) return 'docs';
  if (
