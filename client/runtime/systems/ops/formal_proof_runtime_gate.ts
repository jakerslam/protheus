...
const DEFAULT_POLICY_PATH = process.env.FORMAL_PROOF_RUNTIME_GATE_POLICY_PATH
  ...
  : path.join(ROOT, 'client', 'runtime', 'config', 'formal_proof_runtime_gate_policy.json');

function usage() {
  console.log('Usage:');
  console.log('  node client/runtime/systems/ops/formal_proof_runtime_gate.js run [--strict=1|0] [--runner=local|docker] [--policy=<path>]');
  console.log('  node client/runtime/systems/ops/formal_proof_runtime_gate.js status [--policy=<path>]');
}
  ...
  return {
    ...
    commands: [
      ...
      },
      {
        ...
        bin: 'node',
        args: ['client/runtime/systems/security/critical_path_formal_verifier.js', 'run', '--strict=1'],
        required: true,
        timeout_ms: 180000
      },
      {
      ...
      },
      {
        ...
        bin: 'node',
        args: ['client/runtime/systems/ops/critical_protocol_formal_suite.js', 'run', '--strict=1'],
        required: true,
        timeout_ms: 180000
      },
      {
    ...
    paths: {
      latest_path: 'client/runtime/local/state/ops/formal_proof_runtime_gate/latest.json',
      receipts_path: 'client/runtime/local/state/ops/formal_proof_runtime_gate/receipts.jsonl',
