...
const DEFAULT_POLICY_PATH = process.env.PROOF_PACK_THRESHOLD_GATE_POLICY_PATH
  ...
  : path.join(ROOT, 'client', 'runtime', 'config', 'proof_pack_threshold_gate_policy.json');

function usage() {
  console.log('Usage:');
  console.log('  node client/runtime/systems/ops/proof_pack_threshold_gate.js run [--strict=1|0] [--runner=local|docker] [--policy=<path>]');
  console.log('  node client/runtime/systems/ops/proof_pack_threshold_gate.js status [--policy=<path>]');
}
...
function defaultPolicy() {
  return {
    ...
    commands: {
      mech_benchmark: {
        bin: 'node',
        args: ['client/runtime/systems/ops/mech_suit_benchmark.js', 'run'],
        timeout_ms: 360000
      },
      harness_6m: {
        bin: 'node',
        args: ['client/runtime/systems/autonomy/autonomy_simulation_harness.js', 'run', '--days=180', '--strict=0', '--write=1'],
        timeout_ms: 240000
      },
      protheus_vs_openclaw: {
        bin: 'node',
        args: ['client/runtime/systems/ops/narrow_agent_parity_harness.js', 'run', '--days=180', '--strict=0'],
        timeout_ms: 240000
      },
      formal_invariants: {
        bin: 'npm',
    ...
    paths: {
      latest_path: 'client/runtime/local/state/ops/proof_pack_threshold_gate/latest.json',
      receipts_path: 'client/runtime/local/state/ops/proof_pack_threshold_gate/receipts.jsonl',
      proof_pack_dir: 'docs/client/reports/runtime_snapshots/ops/proof_pack'
    }
  };
}
