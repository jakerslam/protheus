...
// Client role: thin policy-check shim for developer/runtime validation.
...
function run(rawArgs = {}) {
  ...
  const latestPath = path.join(ROOT, (policy.paths && policy.paths.latest_path) || 'client/runtime/local/state/ops/dependency_boundary_guard/latest.json');
  const receiptsPath = path.join(ROOT, (policy.paths && policy.paths.receipts_path) || 'client/runtime/local/state/ops/dependency_boundary_guard/receipts.jsonl');
