...
const DEFAULT_POLICY_PATH = process.env.SUBCONSCIOUS_BOUNDARY_GUARD_POLICY_PATH
  ...
  : (
    fs.existsSync(path.join(ROOT, 'client', 'runtime', 'config', 'subconscious_boundary_guard_policy.json'))
      ? path.join(ROOT, 'client', 'runtime', 'config', 'subconscious_boundary_guard_policy.json')
      : path.join(ROOT, 'config', 'subconscious_boundary_guard_policy.json')
  );

function usage() {
  console.log('Usage:');
  console.log('  node client/runtime/systems/ops/subconscious_boundary_guard.js check [--strict=1|0] [--policy=<path>]');
  console.log('  node client/runtime/systems/ops/subconscious_boundary_guard.js status [--policy=<path>]');
}
...
function defaultPolicy() {
  return {
    ...
    paths: {
      latest_path: 'client/runtime/local/state/ops/subconscious_boundary_guard/latest.json',
      receipts_path: 'client/runtime/local/state/ops/subconscious_boundary_guard/receipts.jsonl'
    }
  };
}
