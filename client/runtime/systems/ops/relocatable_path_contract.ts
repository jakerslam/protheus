...
const DEFAULT_POLICY_PATH = process.env.RELOCATABLE_PATH_CONTRACT_POLICY_PATH
  ...
  : (
    fs.existsSync(path.join(ROOT, 'client', 'runtime', 'config', 'relocatable_path_contract_policy.json'))
      ? path.join(ROOT, 'client', 'runtime', 'config', 'relocatable_path_contract_policy.json')
      : path.join(ROOT, 'config', 'relocatable_path_contract_policy.json')
  );

function usage() {
  console.log('Usage:');
  console.log('  node client/runtime/systems/ops/relocatable_path_contract.js check [--strict=1|0] [--policy=<path>]');
  console.log('  node client/runtime/systems/ops/relocatable_path_contract.js status [--policy=<path>]');
}
...
function defaultPolicy() {
  return {
    ...
    paths: {
      ...
      receipts_path: 'client/runtime/local/state/ops/relocatable_path_contract/receipts.jsonl',
      rewrite_inventory_path: 'client/runtime/local/state/ops/relocatable_path_contract/path_rewrite_inventory.json'
    }
  };
}
