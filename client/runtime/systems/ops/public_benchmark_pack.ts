...
function benchmarkVerdict(snapshot) {
  ...
  const runtimePass = snapshot && snapshot.runtime_efficiency
    ? snapshot.runtime_efficiency.pass === true
    : true;
  if (simVerdict === 'fail' || redCritical > 0) return 'warn';
  if (simVerdict === 'warn') return 'warn';
  if (!runtimePass) return 'warn';
  return 'pass';
}
...
function renderDoc(snapshot) {
  ...
  const coldStartP95 = snapshot && snapshot.runtime_efficiency ? snapshot.runtime_efficiency.cold_start_p95_ms : null;
  const idleRssP95 = snapshot && snapshot.runtime_efficiency ? snapshot.runtime_efficiency.idle_rss_p95_mb : null;
  const artifactMb = snapshot && snapshot.runtime_efficiency ? snapshot.runtime_efficiency.install_artifact_mb : null;
  const runtimePass = snapshot && snapshot.runtime_efficiency ? snapshot.runtime_efficiency.pass : null;
  ...
  return [
    ...
    `| Cold start p95 (ms) | ${coldStartP95 == null ? 'n/a' : Number(coldStartP95).toFixed(3)} |`,
    `| Idle RSS p95 (MB) | ${idleRssP95 == null ? 'n/a' : Number(idleRssP95).toFixed(3)} |`,
    `| Install artifact (MB) | ${artifactMb == null ? 'n/a' : Number(artifactMb).toFixed(3)} |`,
    `| Runtime efficiency pass | ${runtimePass == null ? 'n/a' : (runtimePass ? 'yes' : 'no')} |`,
    '',
...
function runCmd(dateStr, args) {
  ...
  const runtimeEfficiencyRun = runJson([
    'systems/ops/runtime_efficiency_floor.js',
    'run',
    '--strict=0'
  ]);
  ...
  const runtimePayload = runtimeEfficiencyRun.payload && typeof runtimeEfficiencyRun.payload === 'object'
    ? runtimeEfficiencyRun.payload
    : {};
  ...
  const snapshot = {
    ...
    runtime_efficiency: {
      pass: runtimePayload.pass === true,
      result: runtimePayload.result || 'unknown',
      cold_start_p95_ms: runtimePayload.metrics ? Number(runtimePayload.metrics.cold_start_p95_ms || 0) : null,
      cold_start_threshold_ms: runtimePayload.metrics ? Number(runtimePayload.metrics.cold_start_threshold_ms || 0) : null,
      idle_rss_p95_mb: runtimePayload.metrics ? Number(runtimePayload.metrics.idle_rss_p95_mb || 0) : null,
      idle_rss_threshold_mb: runtimePayload.metrics ? Number(runtimePayload.metrics.idle_rss_threshold_mb || 0) : null,
      install_artifact_mb: runtimePayload.metrics ? Number(runtimePayload.metrics.install_artifact_total_mb || 0) : null,
      install_artifact_threshold_mb: runtimePayload.metrics ? Number(runtimePayload.metrics.install_artifact_threshold_mb || 0) : null
    },
    sources: {
      ...
      runtime_efficiency_ok: runtimeEfficiencyRun.ok
    }
  };
  ...
  appendJsonl(HISTORY_PATH, {
    ...
    runtime_efficiency_pass: snapshot.runtime_efficiency.pass === true,
    runtime_cold_start_p95_ms: snapshot.runtime_efficiency.cold_start_p95_ms,
    runtime_idle_rss_p95_mb: snapshot.runtime_efficiency.idle_rss_p95_mb,
    runtime_install_artifact_mb: snapshot.runtime_efficiency.install_artifact_mb
  });
...
function statusCmd(dateArg) {
  ...
  process.stdout.write(`${JSON.stringify({
    ...
    runtime_efficiency_pass: payload.runtime_efficiency ? payload.runtime_efficiency.pass === true : null,
    runtime_cold_start_p95_ms: payload.runtime_efficiency ? payload.runtime_efficiency.cold_start_p95_ms : null,
    runtime_idle_rss_p95_mb: payload.runtime_efficiency ? payload.runtime_efficiency.idle_rss_p95_mb : null,
    runtime_install_artifact_mb: payload.runtime_efficiency ? payload.runtime_efficiency.install_artifact_mb : null
  })}\n`);
}
