...
const TARGET = '../../../../surface/orchestration/scripts/route_task.ts';
...
function loadTarget() {
  try {
    ...
    return {
      ok: false,
      error: 'route_task_target_load_failed',
...
function run(args = process.argv.slice(2)) {
  if (!target || target.ok === false) {
    process.stderr.write(`${JSON.stringify(target || { ok: false, error: 'route_task_target_unavailable' })}\n`);
    return 1;
  }
  if (typeof target.run !== 'function') {
    process.stderr.write(`${JSON.stringify({ ok: false, error: 'route_task_target_missing_run' })}\n`);
    return 1;
  }
