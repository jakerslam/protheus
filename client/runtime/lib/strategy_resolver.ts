...
function effectiveAllowedRisks(defaultSet, strategy) {
    ...
    if (!Array.isArray(result))
        return new Set(defaults.map((x) => String(x).toLowerCase()));
...
function resolveStrategyRankingContext(strategy, context = {}) {
...
function strategyRankingWeights(strategy, context = {}) {
...
function strategyCampaigns(strategy, activeOnly = false) {
