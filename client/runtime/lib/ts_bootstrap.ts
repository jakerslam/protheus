...
function bootstrap(entryPath, mod) {
  const sourcePath = String(entryPath || '');
  let tsPath = sourcePath;
  let syntheticJsPath = sourcePath;
  if (sourcePath.endsWith('.js')) {
    tsPath = sourcePath.replace(/\.js$/i, '.ts');
  } else if (sourcePath.endsWith('.ts')) {
    syntheticJsPath = sourcePath.replace(/\.ts$/i, '.js');
  } else {
    throw new Error(`ts_bootstrap: invalid_entry_path:${sourcePath}`);
  }
  ...
  runGlobalSecurityGate(syntheticJsPath, repoRoot);
  const runtimeMode = resolveRuntimeMode(repoRoot);
  const distPath = distJsPathFor(repoRoot, syntheticJsPath);
  ...
  mod._compile(transpiled, syntheticJsPath);
}
