 ...
 *
 * Usage:
 *   node client/habits/scripts/habit_crystallizer.js --from "task text" [--tokens_est N] [--repeats_14d N] [--errors_30d N] [--intent_key key]
 */
...
function usage() {
  console.log('Usage:');
  console.log('  node client/habits/scripts/habit_crystallizer.js --from "task text" [--tokens_est N] [--repeats_14d N] [--errors_30d N] [--intent_key key] [--auto_trust=0|1]');
  console.log('');
...
function addHabit(registry, params) {
  ...
  const entrypoint = `client/habits/routines/${habitId}.js`;
  ...
  const record = {
    ...
    test_plan: `Run node client/habits/scripts/run_habit.js --id ${habitId} --json '{}' and verify stable output.`,
    idempotent: true,
    inputs_schema: {
      type: 'object',
      required: [],
