...
function normalizeCampaignPhase(raw, index) {
  const src = raw && typeof raw === 'object' ? raw : {};
  const id = asString(src.id) || `phase_${index + 1}`;
  return {
    id: id.toLowerCase().replace(/[^a-z0-9:_-]+/g, '_').replace(/_+/g, '_').replace(/^_+|_+$/g, '').slice(0, 48) || `phase_${index + 1}`,
    name: asString(src.name) || id,
    status: normalizeStatus(src.status),
    order: normalizeInteger(src.order, 1, 99, index + 1),
    priority: normalizeInteger(src.priority, 0, 100, 50),
    objective_id: asString(src.objective_id),
    proposal_types: asStringArray(src.proposal_types).map(x => x.toLowerCase()),
    source_eyes: asStringArray(src.source_eyes).map(x => x.toLowerCase()),
    tags: asStringArray(src.tags).map(x => x.toLowerCase())
  };
}

function normalizeCampaign(raw, index, warnings) {
  const src = raw && typeof raw === 'object' ? raw : {};
  const idRaw = asString(src.id) || `campaign_${index + 1}`;
  const id = idRaw.toLowerCase().replace(/[^a-z0-9:_-]+/g, '_').replace(/_+/g, '_').replace(/^_+|_+$/g, '').slice(0, 48) || `campaign_${index + 1}`;
  const phasesRaw = Array.isArray(src.phases) ? src.phases : [];
  const phases = phasesRaw
    .map((phase, i) => normalizeCampaignPhase(phase, i))
    .filter((phase) => phase && phase.id);
  if (!phases.length) {
    phases.push(normalizeCampaignPhase({
      id: `${id}_phase_1`,
      name: `${idRaw} Phase 1`,
      status: 'active',
      order: 1,
      priority: 50,
      objective_id: asString(src.objective_id),
      proposal_types: asStringArray(src.proposal_types),
      source_eyes: asStringArray(src.source_eyes),
      tags: asStringArray(src.tags)
    }, 0));
    if (Array.isArray(warnings)) warnings.push(`campaign_default_phase_inserted:${id}`);
  }
  const dedup = new Set();
  ...
  return {
    id,
    name: asString(src.name) || idRaw,
    status: normalizeStatus(src.status),
    priority: normalizeInteger(src.priority, 1, 100, 50),
    objective_id: asString(src.objective_id),
    proposal_types: asStringArray(src.proposal_types).map(x => x.toLowerCase()),
    source_eyes: asStringArray(src.source_eyes).map(x => x.toLowerCase()),
    tags: asStringArray(src.tags).map(x => x.toLowerCase()),
...
function normalizeCampaigns(raw, warnings) {
  if (!Array.isArray(raw)) return [];
  const out = [];
  const ids = new Set();
  for (let i = 0; i < raw.length; i++) {
    const campaign = normalizeCampaign(raw[i], i, warnings);
    if (!campaign || !campaign.id) continue;
    if (ids.has(campaign.id)) {
      if (Array.isArray(warnings)) warnings.push(`campaign_duplicate_filtered:${campaign.id}`);
      continue;
    }
    ids.add(campaign.id);
    out.push(campaign);
...
function normalizeStrategy(raw, filePath) {
  ...
  const campaigns = normalizeCampaigns(src.campaigns, warnings);
...
function strategyCampaigns(strategy, activeOnly = true) {
  const campaigns = strategy && Array.isArray(strategy.campaigns) ? strategy.campaigns : [];
  if (!activeOnly) return campaigns.slice();
  return campaigns.filter((campaign) => String(campaign && campaign.status || '').toLowerCase() === 'active');
}
