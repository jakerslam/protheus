...
function isStrict(channel, opts = {}) {
  ...
  const globalStrict = String(process.env.MUTATION_PROVENANCE_STRICT || '').trim();
  if (globalStrict === '1') return true;
  if (channel === 'adaptive') {
    const adaptiveStrict = String(process.env.ADAPTIVE_MUTATION_STRICT || '').trim();
    if (!adaptiveStrict) return true; // Default-hard for adaptive memory channel.
    if (adaptiveStrict === '1' || adaptiveStrict.toLowerCase() === 'true') return true;
    return false;
  }
...
function enforceMutationProvenance(channel, meta, opts = {}) {
  ...
  if (ch === 'adaptive' || String(process.env.MUTATION_PROVENANCE_AUDIT_ALL || '').trim() === '1') {
    appendJsonl(auditPath(ch), {
      ts: nowIso(),
      type: 'mutation_provenance_check',
      channel: ch,
      ok: out.ok === true,
      policy_version: policy.version,
      source: source || null,
