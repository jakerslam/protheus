...
pub fn run_legacy_script(
    ...
    cmd.env("PROTHEUS_RUST_LEGACY_BRIDGE", "1");
...
pub fn run_legacy_script_path(
    ...
    run_legacy_script(root, domain, script_path, args, forward_stdin)
}
...
pub fn split_legacy_fallback_flag(argv: &[String], module_env_key: &str) -> (bool, Vec<String>) {
    ...
    let fallback = fallback_from_arg.unwrap_or_else(|| {
        parse_bool_flag(env::var(module_env_key).ok().as_deref())
            || parse_bool_flag(env::var("PROTHEUS_OPS_LEGACY_FALLBACK").ok().as_deref())
    });
...
mod tests {
    ...
    fn execute_with_bin_captures_stdout() {
        let root = tempdir().expect("tempdir");
        let rel = "systems/ops/fake_bridge.sh";
...
"#,
    ...
    fn execute_with_bin_propagates_exit_code() {
        let root = tempdir().expect("tempdir");
        let rel = "systems/ops/fake_bridge_fail.sh";
        let script = root.path().join(rel);
        write_script(
            &script,
            r#"#!/bin/sh
