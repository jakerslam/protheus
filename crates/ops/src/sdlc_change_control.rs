...
const DEFAULT_POLICY_REL: &str = "config/sdlc_change_control_policy.json";
...
fn load_policy(root: &Path, policy_override: Option<&String>) -> Policy {
    ...
    let high_risk_path_prefixes = raw
        ...
        .unwrap_or_else(|| {
            vec![
                "crates/security/".to_string(),
                "crates/conduit/".to_string(),
                "systems/security/".to_string(),
                "config/protheus_conduit_policy.json".to_string(),
                "config/rust_source_of_truth_policy.json".to_string(),
            ]
        });
...
mod tests {
    ...
    fn write_policy(root: &Path) {
        write_text(
            &root.join("config/sdlc_change_control_policy.json"),
            &json!({
                "strict_default": true,
                ...
                "high_risk_path_prefixes": ["crates/security/", "systems/security/"],
                "major_path_prefixes": ["crates/ops/", "systems/ops/"],
                "outputs": {
    ...
    fn high_risk_change_requires_full_approval_bundle() {
        ...
        write_text(
            ...
            "- Risk class: high-risk\n- Rollback plan: revert and freeze\n- Rollback owner: ops-oncall\n- Approvers: alice\n- Approval receipts: docs/approvals/one.md\n- Rollback drill receipt: docs/drills/rollback.json\n",
        );
        write_text(
        ...
        write_text(&root.join("docs/approvals/one.md"), "ok");
        write_text(&root.join("docs/drills/rollback.json"), "{}");
    ...
    fn major_change_passes_with_rfc_and_single_approver() {
        ...
        write_text(&root.join("docs/rfc/RFC-1.md"), "rfc");
        write_text(&root.join("docs/approvals/approve-1.md"), "receipt");

        write_text(
            &root.join("state/ops/sdlc_change_control/pr_body.md"),
            "- Risk class: major\n- RFC link: docs/rfc/RFC-1.md\n- Rollback plan: git revert\n- Rollback owner: platform\n- Approvers: alice\n- Approval receipts: docs/approvals/approve-1.md\n",
        );
        write_text(
            &root.join("state/ops/sdlc_change_control/changed_paths.txt"),
