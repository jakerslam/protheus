...
fn build_receipt_with_ts(lane_id: &str, ts: &str) -> Value {
    let seed = json!({ "lane": lane_id, "ts": ts, "type": "legacy_retired_lane" }).to_string();
    let lane_hash = stable_hash(&seed, 32);
    json!({
        "ok": true,
...
mod tests {
    ...
    fn clean_lane_id_normalizes_case_and_symbols() {
        assert_eq!(clean_lane_id(" systems/ops/status "), "SYSTEMSOPSSTATUS");
        assert_eq!(
