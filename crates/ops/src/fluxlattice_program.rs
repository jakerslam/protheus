...
fn run_cargo_flux(root: &Path, args: &[String]) -> Value {
    let out = Command::new("cargo")
        ...
        .arg("--quiet")
        .arg("--manifest-path")
        .arg("crates/fluxlattice/Cargo.toml")
        .arg("--bin")
        .arg("fluxlattice")
        .arg("--")
        .args(args)
...
pub fn default_policy(root: &Path) -> Policy {
    Policy {
        ...
        paths: Paths {
            ...
            migration_profiles_path: root.join("config/fluxlattice_migration_profiles.json"),
            lens_mode_policy_path: root.join("config/lens_mode_policy.json"),
        },
        policy_path: root.join("config/fluxlattice_program_policy.json"),
    }
}
...
pub fn load_policy(root: &Path, policy_path: &Path) -> Policy {
    ...
    out.paths = Paths {
        ...
        migration_profiles_path: resolve_path(
            ...
            "config/fluxlattice_migration_profiles.json",
        ),
        lens_mode_policy_path: resolve_path(
            root,
            paths.get("lens_mode_policy_path"),
            "config/lens_mode_policy.json",
        ),
    };
...
fn run_lane(
    ...
    match id {
        ...
        "V4-PKG-001" => {
            let cargo_toml = root.join("crates/fluxlattice/Cargo.toml");
        ...
        "V4-PKG-002" => {
            let required = [
                root.join("crates/fluxlattice/README.md"),
                root.join("crates/fluxlattice/CHANGELOG.md"),
        ...
        "V4-PKG-003" => {
            ...
            let runbook_path = root.join("docs/FLUXLATTICE_MIGRATION_RUNBOOK.md");
            if apply {
        ...
        "V4-PKG-006" => {
            let narrative_path = root.join("docs/LENSMAP_INTERNAL_NARRATIVE.md");
            if apply {
...
pub fn usage() {
    println!("Usage:");
    println!("  node systems/ops/fluxlattice_program.js list");
    println!(
        "  node systems/ops/fluxlattice_program.js run --id=V4-ETH-001 [--apply=1|0] [--strict=1|0]"
    );
    println!("  node systems/ops/fluxlattice_program.js run-all [--apply=1|0] [--strict=1|0]");
    println!("  node systems/ops/fluxlattice_program.js status");
}
...
pub fn run(root: &Path, argv: &[String]) -> i32 {
    ...
    let policy_arg = parsed
        ...
        .get("policy")
        .map(PathBuf::from)
        .unwrap_or_else(|| root.join("config/fluxlattice_program_policy.json"));
