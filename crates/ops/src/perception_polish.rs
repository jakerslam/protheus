...
pub fn default_policy(root: &Path) -> Policy {
    Policy {
        ...
        paths: Paths {
            ...
            flags_path: root.join("config/feature_flags/perception_flags.json"),
            ...
            tone_policy_path: root.join("config/perception_tone_policy.json"),
            post_reveal_easter_egg_path: root.join("docs/blog/the_fort_was_empty_easter_egg.md"),
        },
        policy_path: root.join("config/perception_polish_program_policy.json"),
    }
}
...
pub fn load_policy(root: &Path, policy_path: &Path) -> Policy {
    ...
    out.paths = Paths {
        ...
        flags_path: resolve_path(
            ...
            "config/feature_flags/perception_flags.json",
        ),
...
pub fn usage() {
    println!("Usage:");
    println!("  node systems/ops/perception_polish_program.js list");
    println!("  node systems/ops/perception_polish_program.js run --id=V4-ILLUSION-001 [--apply=1|0] [--strict=1|0]");
    println!(
        "  node systems/ops/perception_polish_program.js run-all [--apply=1|0] [--strict=1|0]"
    );
    println!("  node systems/ops/perception_polish_program.js status");
}
...
pub fn run(root: &Path, argv: &[String]) -> i32 {
    ...
    let policy_arg = parsed
        ...
        .get("policy")
        .map(PathBuf::from)
        .unwrap_or_else(|| root.join("config/perception_polish_program_policy.json"));
