...
const DEFAULT_POLICY_REL: &str = "config/supply_chain_provenance_v2_policy.json";
...
fn load_policy(root: &Path, policy_override: Option<&String>) -> Policy {
    ...
    Policy {
        ...
        rollback_policy_path: resolve_path(
            ...
            "config/release_rollback_policy.json",
        ),
        vulnerability_sla: VulnerabilitySla {
            max_critical: sla
...
mod tests {
    ...
    fn write_policy(root: &Path) {
        write_text(
            &root.join("config/supply_chain_provenance_v2_policy.json"),
            &json!({
                "strict_default": true,
                "required_artifacts": [
                    {
                ...
                "rollback_policy_path": "config/release_rollback_policy.json",
                "vulnerability_sla": {
                    "max_critical": 0,
                    "max_high": 1,
                    "max_medium": 4,
    ...
    fn make_fixture(root: &Path, critical: u64) {
        ...
        write_text(
            &root.join("config/release_rollback_policy.json"),
            &json!({
        ...
        let bundle = json!({
            ...
            "rollback": {
                ...
                "policy_path": "config/release_rollback_policy.json"
            }
        });
        write_text(
