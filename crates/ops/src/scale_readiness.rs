...
pub fn default_policy(root: &Path) -> Policy {
    Policy {
        ...
        paths: Paths {
            ...
            contract_dir: root.join("config/scale_readiness"),
        ...
        policy_path: root.join("config/scale_readiness_program_policy.json"),
    }
}
...
pub fn load_policy(root: &Path, policy_path: &Path) -> Policy {
    ...
    out.paths = Paths {
        ...
        contract_dir: resolve_path(root, paths.get("contract_dir"), "config/scale_readiness"),
...
pub fn usage() {
    println!("Usage:");
    println!("  node systems/ops/scale_readiness_program.js list");
    println!("  node systems/ops/scale_readiness_program.js run --id=V4-SCALE-001 [--apply=1|0] [--strict=1|0]");
    println!("  node systems/ops/scale_readiness_program.js run-all [--apply=1|0] [--strict=1|0]");
    println!("  node systems/ops/scale_readiness_program.js status");
}
    ...
    let policy_arg = parsed
        ...
        .unwrap_or_else(|| {
            env::var("SCALE_READINESS_PROGRAM_POLICY_PATH")
                .map(PathBuf::from)
                .unwrap_or_else(|_| root.join("config/scale_readiness_program_policy.json"))
        });
