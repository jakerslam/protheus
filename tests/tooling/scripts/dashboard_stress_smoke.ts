...
async function createWsProbeAgent() {
  const requested = `stress-${Date.now().toString().slice(-6)}`;
  const out = await hit(
    '/api/agents',
    'POST',
    { name: requested, role: 'analyst' },
    15000
  );
  if (!out.ok) {
    throw new Error(`ws_probe_agent_create_failed_${out.status}`);
  }
  let parsed = {};
  try {
    parsed = JSON.parse(out.text || '{}');
  } catch {}
  const id = String((parsed && (parsed.id || parsed.agent_id)) || requested).trim();
  if (!id) throw new Error('ws_probe_agent_id_missing');
  return id;
}

async function wsFanout({ connections = 36, timeoutMs = 12000, agentId = '' }) {
  const openTimes = [];
  const firstReadyTimes = [];
  const errors = [];
  const wsPath = `${WS_BASE}/api/agents/${encodeURIComponent(String(agentId || ''))}/ws`;
    ...
    Array.from({ length: connections }, () =>
      new Promise((resolve) => {
        ...
        const timer = setTimeout(() => {
          errors.push('ws_timeout_ready_event');
          done(() => ws.terminate());
        }, timeoutMs);
        ...
        ws.on('message', (raw) => {
          try {
            ...
            if (type === 'context_state' || type === 'connected') {
              firstReadyTimes.push(performance.now() - (openedAt || started));
              clearTimeout(timer);
              done(() => ws.close());
            }
          } catch {}
  ...
  return {
    label: 'ws:/api/agents/:id/ws fanout',
    connections,
    ok: firstReadyTimes.length,
    errors: errors.length,
    ...
    first_ready_latency: {
      p50_ms: Number(percentile(firstReadyTimes, 50).toFixed(2)),
      p95_ms: Number(percentile(firstReadyTimes, 95).toFixed(2)),
      p99_ms: Number(percentile(firstReadyTimes, 99).toFixed(2)),
      max_ms: Number(
        (firstReadyTimes.length ? Math.max(...firstReadyTimes) : 0).toFixed(2)
      ),
    },
    sample_error: errors[0] || null,
  };
...
async function parsePostSnapshot() {
  ...
  try {
    ...
    const runtime =
      parsed && parsed.runtime_recommendation
        ? parsed.runtime_recommendation
        : parsed && parsed.runtime_sync
          ? parsed.runtime_sync
          : {};
      ...
      queue_depth:
        runtime.queue_depth != null
          ...
          : attention.depth != null
            ? attention.depth
            : attention.queue_depth != null
              ? attention.queue_depth
            : null,
      conduit_signals:
...
async function main() {
  ...
  const [health, status, snapshot, ws] = await Promise.all([
    ...
    wsFanout({ connections: 36, timeoutMs: 12000, agentId: wsAgentId }),
  ]);
