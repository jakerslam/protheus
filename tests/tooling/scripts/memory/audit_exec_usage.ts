...
const ALLOWED_FILES = [
  'client/runtime/lib/exec_compacted.ts',  // The authorized wrapper
  'tests/tooling/scripts/memory/skill_runner.ts', // Uses spawn for child process management (legitimate)
];
      ...
      if (pattern.test(line)) {
        ...
        if (!isAllowed) {
          findings.push({
            ...
            code: line.trim(),
            fix: 'Use execCompacted() or execFileCompacted() from client/runtime/lib/exec_compacted.ts'
          });
        }
      }
    }
