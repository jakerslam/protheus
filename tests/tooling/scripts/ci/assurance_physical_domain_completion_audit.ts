...
function scanCompatibilityMirrors(): {
  ...
  for (const file of mirrorFiles) {
    ...
    if (!payload.burn_down_required || typeof payload.burn_down_todo !== 'string' || payload.burn_down_todo.length === 0) {
      failures.push({
        id: 'mirror_registry_missing_burn_down_marker',
        path: file,
        detail: 'compatibility mirror registry must declare burn_down_required=true and burn_down_todo',
      });
    }
    if (fileMirrors.length === 0) {
      failures.push({
        id: 'mirror_registry_empty',
        path: file,
        detail: 'compatibility mirror registry must contain at least one old-location row',
      });
    }
