...
function validate(sourcePath: string, componentPath: string, stylesPath: string, runtimePath: string, buildPath: string, serverPath: string, readmePath: string, includeControlledViolation: boolean): Violation[] {
  ...
  if (!component.includes('<script') || !component.includes('onSubmitInput') || !component.includes('{#each messages as message') || !component.includes('{#each agentRows as agent') || !component.includes('{#each sessionRows as session') || !component.includes('onOpenMessageDetail') || !component.includes('onSubmitIssue') || !component.includes('onSubmitApprovalDecision') || !component.includes('onSetModel') || !component.includes('onSetGitTree') || !component.includes('activeDetailPanel') || !component.includes('projectionCounts') || !component.includes('receiptRefs')) {
    push(violations, 'missing_svelte_projection_component', componentPath, 'Browser Shell V2 must expose a Svelte projection/input component with agent/session selection, lazy details, event/search/issue/model/git projection telemetry, and Shell Socket action hooks without inventing a second visible dashboard substrate.');
  }
  ...
  if (!build.includes('rowsFromSelectorOptions') || !build.includes('setModel') || !build.includes('setGitTree') || !build.includes('/model') || !build.includes('/git-tree')) {
    push(violations, 'missing_browser_v2_selector_artifact_path', buildPath, 'Standalone Browser V2 artifact must retain bounded model/git selector request paths through Gateway without rendering an invented selector substrate.');
  }
  ...
  if (!build.includes('detailPanelFromProjection') || !build.includes('popup-window dashboard-popup-surface') || !build.includes('slice(0, 12)')) {
    push(violations, 'missing_browser_v2_detail_drawer_artifact_path', buildPath, 'Standalone Browser V2 artifact must render bounded detail projections inside the legacy popup surface.');
  }
