  ...
  try {
    if (dryRun) {
      const payload = {
        ...
        steps: [
          {
            name: 'srs_actionable_map:pre',
            command: 'node client/runtime/lib/ts_entrypoint.ts tests/tooling/scripts/ci/srs_actionable_map.ts',
          },
          {
          ...
          },
          {
            ...
            command: 'node client/runtime/lib/ts_entrypoint.ts tests/tooling/scripts/ci/srs_full_regression.ts',
          },
          {
            name: 'srs_top200_regression',
            command: 'node client/runtime/lib/ts_entrypoint.ts tests/tooling/scripts/ci/srs_top200_regression.ts',
          },
    ...
    steps.push(runTsStep('srs_actionable_map:pre', 'tests/tooling/scripts/ci/srs_actionable_map.ts'));
    ...
    steps.push(runTsStep('srs_full_regression', 'tests/tooling/scripts/ci/srs_full_regression.ts'));
    let full = readJson(FULL_JSON);
    let fullFail = Number(full?.summary?.regression?.fail || 0);
    if (fullFail !== 0) {
      steps.push(runTsStep('srs_full_regression:retry', 'tests/tooling/scripts/ci/srs_full_regression.ts'));
    ...
    steps.push(runTsStep('srs_top200_regression', 'tests/tooling/scripts/ci/srs_top200_regression.ts'));
    let top = readJson(TOP200_JSON);
    let topFail = Number(top?.summary?.regression?.fail || 0);
    if (topFail !== 0) {
      steps.push(runTsStep('srs_top200_regression:retry', 'tests/tooling/scripts/ci/srs_top200_regression.ts'));
    ...
    steps.push(runTsStep('srs_actionable_map:post', 'tests/tooling/scripts/ci/srs_actionable_map.ts'));
