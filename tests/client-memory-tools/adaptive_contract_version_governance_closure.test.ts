...
function main() {
  ...
  assertNoPlaceholderOrPromptLeak(out, 'adaptive_contract_version_governance_closure_test');
  assertStableToolingEnvelope(out, 'adaptive_contract_version_governance_closure_test');
  console.log('adaptive_contract_version_governance_closure.test.ts: OK');
}
try { main(); } catch (err) { console.error(`adaptive_contract_version_governance_closure.test.ts: FAIL: ${err.message}`); process.exit(1); }
