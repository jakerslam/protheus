...
function run() {
  ...
  console.log('autonomy_eye_outcome_count_window_rust_parity.test.ts: OK');
}

try {
  run();
} catch (err) {
  console.error(`autonomy_eye_outcome_count_window_rust_parity.test.ts: FAIL: ${err.message}`);
  process.exit(1);
}
