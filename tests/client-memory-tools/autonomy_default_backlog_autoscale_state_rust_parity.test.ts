...
function run() {
  ...
  console.log('autonomy_default_backlog_autoscale_state_rust_parity.test.ts: OK');
}

try {
  run();
} catch (err) {
  console.error(`autonomy_default_backlog_autoscale_state_rust_parity.test.ts: FAIL: ${err.message}`);
  process.exit(1);
}
