...
function main() {
  ...
  try {
    const okRun = runNode(['tests/tooling/scripts/ci/srs_repair_lane_runner.ts', '--id=V6-SOVEREIGN-002.1', '--dry-run=1']);
  ...
  try {
    const badRun = runNode(['tests/tooling/scripts/ci/srs_repair_lane_runner.ts', '--id=bad-id', '--dry-run=1']);
