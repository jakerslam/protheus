...
const POLICY = JSON.parse(fs.readFileSync(path.join(ROOT, 'client', 'runtime', 'config', 'inter_protheus_federation_trust_web_policy.json'), 'utf8'));
...
assert.match(POLICY.paths.latest_path, /inter_protheus_federation_trust_web\/latest\.json$/);

console.log(JSON.stringify({ ok: true, type: 'inter_protheus_federation_trust_web_test' }));
