...
function run() {
  ...
  console.log('autonomy_execute_confidence_cooldown_active_rust_parity.test.ts: OK');
}

try {
  run();
} catch (err) {
  console.error(`autonomy_execute_confidence_cooldown_active_rust_parity.test.ts: FAIL: ${err.message}`);
  process.exit(1);
}
