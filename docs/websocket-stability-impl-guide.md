...
| File | Purpose |
|------|---------|
| `patches/websocket-server-patch.js` | Server-side heartbeat + event buffer |
| `patches/websocket-client-patch.js` | Client-side reconnect + catch-up |
...
1. **Include server patch** in your gateway startup:
   ...
   const { patchWebSocketServer } = require('./patches/websocket-server-patch.js');
...
2. **Serve client patch** from your control-ui:
   ...
   <script src="/patches/websocket-client-patch.js"></script>
