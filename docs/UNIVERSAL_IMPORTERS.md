...
`V4-MIGR-003` adds a pluggable importer surface at `systems/migration/importers/` and a CLI bridge for switching from other agent stacks.
...
```bash
# Direct lane
node systems/migration/universal_importers.js run --from=openfang --path=./dump/openfang.json --apply=1 --strict=1
