...
`systems/ops/rust50_sprint1_batch.ts` can execute `V6-RUST50-CONF-004` by loading:

- `config/rust50_mega_sprint1_batch_policy.json`
...
## Commands
```bash
node systems/ops/rust50_sprint1_batch.js run \
  --policy=config/rust50_mega_sprint1_batch_policy.json \
  --enforcer-active=1 \
  --preamble-text="ENFORCER RULES ACTIVE — READ codex_enforcer.md FIRST." \
  --strict=1 --apply=1

node systems/ops/rust50_sprint1_batch.js status \
  --policy=config/rust50_mega_sprint1_batch_policy.json
```
