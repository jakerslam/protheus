...
```bash
node systems/ops/wasi2_execution_completeness_gate.js run --strict=1 --apply=1
node systems/ops/wasi2_execution_completeness_gate.js status
```
...
The gate uses `systems/ops/wasi2_lane_adapter.js` to normalize probe envelopes and routes WASI2 probes through `systems/wasm/component_runtime.js`.
