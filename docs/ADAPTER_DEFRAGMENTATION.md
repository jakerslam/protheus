...
`systems/actuation/adapter_defragmentation.js` analyzes adapter usage and profile coverage to identify bespoke adapters that should migrate to the universal primitive lane.

## Commands

```bash
# Generate daily snapshot
node systems/actuation/adapter_defragmentation.js run

# Read latest snapshot
node systems/actuation/adapter_defragmentation.js status latest
```
...
Policy file: `config/adapter_defragmentation_policy.json`
