...
- `systems/<domain>/my_lane.ts`
- `systems/<domain>/my_lane.js` (ts bootstrap wrapper)
- `config/my_lane_policy.json`
- `memory/tools/tests/my_lane.test.js`
...
- `docs/README.md` for discoverability
- `CHANGELOG.md` for user-visible behavior

Run:

```bash
node memory/tools/tests/my_lane.test.js
...
```bash
node systems/<domain>/my_lane.js run --force-fallback=1 --strict=1
```
