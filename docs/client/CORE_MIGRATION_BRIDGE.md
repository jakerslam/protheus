...
- `client/cognition/habits/` (if present)
...
`client/runtime/systems/migration/bridge.rs` defines deterministic target normalization and receipt-signing primitives that mirror the operator lane semantics for future native cutover.
