...
7. Run strict supply-chain gate:
   - `cargo run --quiet --manifest-path core/layer0/ops/Cargo.toml --bin infring-ops -- supply-chain-provenance-v2 run --strict=1 --policy=client/runtime/config/supply_chain_provenance_v2_policy.json`
...
- Run `infring-ops benchmark-matrix run --refresh-runtime=1` and attach report snapshot.
