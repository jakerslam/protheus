...
`client/runtime/systems/security/supply_chain_trust_plane.ts` provides deterministic build trust artifacts and release-gate verification.
...
Policy file: `client/runtime/config/supply_chain_trust_policy.json`
...
`client/runtime/systems/security/merge_guard.ts` now runs the supply-chain trust plane in strict verification mode.
