...
## Failure Domains
- Release gate failure is isolated via strict fail-closed checks in `infring-ops` lanes.
...
## Benchmark Claims
- Benchmark matrix receipts are emitted by `infring-ops benchmark-matrix run --refresh-runtime=1`.
