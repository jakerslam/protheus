...
- `client/runtime/systems/conduit/conduit-client.ts` typed TS client
...
- `client/runtime/systems/ops/protheusd.ts` supports conduit-first routing for:
  - `start`
  - `stop`
  - `status`
...
- `client/runtime/systems/ops/protheusd.ts` now defaults lifecycle commands (`start`, `stop`, `status`) to strict conduit routing.
...
- `client/runtime/config/rust_source_of_truth_policy.json` defines source-of-truth contract checks for:
  ...
  - strict conduit enforcement tokens in `client/runtime/systems/ops/protheusd.ts`
...
- TS dashboard surface (`client/runtime/systems/ops/protheus_status_dashboard.ts`) is a Rust bridge wrapper only.
