...
- Rust core crate: `systems/hybrid/rust`
- Ops executor: `systems/ops/rust_hybrid_migration_program.ts`
- Policy: `config/rust_hybrid_migration_program_policy.json`
...
```bash
node systems/ops/rust_hybrid_migration_program.js list
node systems/ops/rust_hybrid_migration_program.js run --id=V5-RUST-HYB-001 --apply=1 --strict=1
node systems/ops/rust_hybrid_migration_program.js run-all --apply=1 --strict=1
node systems/ops/rust_hybrid_migration_program.js status
```

## Guardrails

- Fail-closed lane receipts on missing docs/policy contracts.
