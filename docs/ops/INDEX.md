# Ops Documentation Index

Single-entry operator index for deployment, security, incident response, and maintenance runbooks.

## Core Operator Surfaces

- Deployment: [RUNBOOK-002-deployment-procedures.md](./RUNBOOK-002-deployment-procedures.md)
- Incident response: [RUNBOOK-001-incident-response.md](./RUNBOOK-001-incident-response.md)
- Command registry: [COMMAND_REGISTRY.md](./COMMAND_REGISTRY.md)
- Cross-component architecture: [INTEGRATION-001-cross-component-architecture.md](./INTEGRATION-001-cross-component-architecture.md)

## Reliability + Maintenance

- Log aggregation: [RUNBOOK-004-log-aggregation.md](./RUNBOOK-004-log-aggregation.md)
- Database maintenance: [RUNBOOK-003-database-maintenance.md](./RUNBOOK-003-database-maintenance.md)
- Backup and restore: [RUNBOOK-010-backup-and-restore.md](./RUNBOOK-010-backup-and-restore.md)
- Service discovery troubleshooting: [RUNBOOK-009-service-discovery-troubleshooting.md](./RUNBOOK-009-service-discovery-troubleshooting.md)
- System health checks: [RUNBOOK-006-system-health-checks.md](./RUNBOOK-006-system-health-checks.md)

## Security + Governance

- Security policy: [SECURITY.md](../../SECURITY.md)
- ADR registry: [docs/client/adr/INDEX.md](../client/adr/INDEX.md)
- Documentation governance: [docs/client/DOCUMENTATION_PROGRAM_GOVERNANCE.md](../client/DOCUMENTATION_PROGRAM_GOVERNANCE.md)

## Specialized Runbooks

- MCU proof sprint: [RUNBOOK-005-mcu-proof-sprint.md](./RUNBOOK-005-mcu-proof-sprint.md)
- Pure/tiny capability restore: [RUNBOOK-007-pure-tiny-capability-restore.md](./RUNBOOK-007-pure-tiny-capability-restore.md)
- SSL certificate renewal: [RUNBOOK-008-ssl-certificate-renewal.md](./RUNBOOK-008-ssl-certificate-renewal.md)
