...
| `client/runtime/config/kernel_naming_policy.json` | `core/layer0/ops/config/kernel_naming_policy.json` | Core/Kernel | loader prefers canonical | Kernel naming constraints belong with core ownership; package scripts now pass the canonical copy. |
| `client/runtime/config/ownership_drift_policy.json` | `validation/release_gates/contracts/ownership_drift_policy.json` | Validation release gates | loader prefers canonical | Cross-domain ownership drift checks are validation authority; the guard now defaults to the release-gate contract. |
...
| `client/runtime/config/orchestration_workflow_contract_policy.json` | `orchestration/config/orchestration_workflow_contract_policy.json` | Orchestration | loader prefers canonical | Workflow lifecycle/template contract is Orchestration authority; the guard now defaults to the orchestration copy. |
| `client/runtime/config/orchestration_quality_policy.json` | `orchestration/config/orchestration_quality_policy.json` | Orchestration | loader prefers canonical | Planner/runtime/gateway quality thresholds are orchestration-control authority; quality guards now default to the orchestration copy. |
| `client/runtime/config/orchestration_ownership_policy.json` | `orchestration/config/orchestration_ownership_policy.json` | Orchestration | loader prefers canonical | Orchestration placement/ownership checks are orchestration authority; the guard now defaults to the orchestration copy. |
| `client/runtime/config/orchestration_ts_boundary_policy.json` | `orchestration/config/orchestration_ts_boundary_policy.json` | Orchestration | loader prefers canonical | Orchestration TS/Rust boundary constraints are orchestration authority; the audit now defaults to the orchestration copy. |
| `client/runtime/config/orchestration_naming_policy.json` | `orchestration/config/orchestration_naming_policy.json` | Orchestration | loader prefers canonical | Orchestration naming rules are orchestration authority; the orchestration naming scripts now pass the canonical policy. |
...
| `client/runtime/config/gateway_ingress_egress_contract.json` | `validation/conformance/contracts/gateway_ingress_egress_contract.json` | Validation conformance | guard defaults to canonical | Gateway route-class conformance is validation authority; client copy is now a compatibility mirror. |
| `client/runtime/config/interface_payload_budget_contract.json` | `validation/conformance/contracts/interface_payload_budget_contract.json` | Validation conformance | guard defaults to canonical | Interface payload ceilings are conformance contracts for Shell/Gateway boundaries; client copy is now a compatibility mirror. |
| `client/runtime/config/conduit_scrambler_posture_contract.json` | `validation/conformance/contracts/conduit_scrambler_posture_contract.json` | Validation conformance | guard defaults to canonical | Conduit/Scrambler posture declarations are cross-domain conformance contracts; client copy is now a compatibility mirror. |
| `client/runtime/config/cross_domain_nexus_route_inventory.json` | `validation/conformance/contracts/cross_domain_nexus_route_inventory.json` | Validation conformance | guard defaults to canonical | Nexus route inventory is a conformance contract spanning Core, Orchestration, Gateway, Shell, and external boundaries; client copy is now a compatibility mirror. |
| `client/runtime/config/shell_ui_message_detail_contract.json` | `validation/conformance/contracts/shell_ui_message_detail_contract.json` | Validation conformance | guard defaults to canonical | Shell projection/detail-route shape is validated as a conformance contract; client copy is now a compatibility mirror. |
| `client/runtime/config/shell_truth_leak_policy.json` | `validation/conformance/contracts/shell_truth_leak_policy.json` | Validation conformance | guard defaults to canonical | Shell truth-leak scanning is a conformance gate, not client authority; client copy is now a compatibility mirror. |
| `client/runtime/config/shell_backend_state_contract.json` | `validation/conformance/contracts/shell_backend_state_contract.json` | Validation conformance | guard defaults to canonical | Backend state/freshness expectations are validation contracts for Shell projection behavior; client copy is now a compatibility mirror. |
| `client/runtime/config/shell_naming_policy.json` | `validation/conformance/contracts/shell_naming_policy.json` | Validation conformance | guard defaults to canonical | Shell path naming rules are enforced by validation tooling; client copy is now a compatibility mirror. |
| `client/runtime/config/root_surface_contract.json` | `validation/conformance/contracts/root_surface_contract.json` | Validation conformance | guard defaults to canonical | Root workspace surface constraints are conformance guard inputs; client copy is now a compatibility mirror. |
| `client/runtime/config/repo_surface_policy.json` | `validation/conformance/contracts/repo_surface_policy.json` | Validation conformance | guard defaults to canonical | Repo surface constraints are conformance guard inputs; client copy is now a compatibility mirror. |
| `client/runtime/config/docs_surface_contract_policy.json` | `validation/conformance/contracts/docs_surface_contract_policy.json` | Validation conformance | guard defaults to canonical | Docs surface constraints are validation contracts; client copy is now a compatibility mirror. |
| `client/runtime/config/public_platform_contract_policy.json` | `validation/conformance/contracts/public_platform_contract_policy.json` | Validation conformance | guard defaults to canonical | Public platform surface constraints are validation contracts; client copy is now a compatibility mirror. |
...
| Kernel naming and ownership drift guard scripts | `core/layer0/ops/config/kernel_naming_policy.json`, `validation/release_gates/contracts/ownership_drift_policy.json` | matching `client/runtime/config/*.json` mirrors | flipped |
...
| `tests/tooling/scripts/ci/orchestration_workflow_contract_guard.ts` | `orchestration/config/orchestration_workflow_contract_policy.json` | `client/runtime/config/orchestration_workflow_contract_policy.json` mirror kept for legacy callers | flipped |
| Orchestration quality guard scripts | `orchestration/config/orchestration_quality_policy.json` | `client/runtime/config/orchestration_quality_policy.json` mirror kept for legacy callers | flipped |
| Orchestration ownership/naming/boundary guard scripts | `orchestration/config/orchestration_ownership_policy.json`, `orchestration/config/orchestration_ts_boundary_policy.json`, `orchestration/config/orchestration_naming_policy.json` | matching `client/runtime/config/*.json` mirrors | flipped |
...
| Gateway/Shell/Nexus conformance guard scripts | `validation/conformance/contracts/{gateway_ingress_egress_contract,interface_payload_budget_contract,conduit_scrambler_posture_contract,cross_domain_nexus_route_inventory,shell_ui_message_detail_contract}.json` | matching `client/runtime/config/*.json` mirrors | flipped |
| Shell truth-leak and naming guard scripts | `validation/conformance/contracts/{shell_truth_leak_policy,shell_backend_state_contract,shell_naming_policy}.json` | matching `client/runtime/config/*.json` mirrors | flipped |
| Root/repo/docs/public surface guard scripts | `validation/conformance/contracts/{root_surface_contract,repo_surface_policy,docs_surface_contract_policy,public_platform_contract_policy}.json` | matching `client/runtime/config/*.json` mirrors | flipped |
...
| `tests/tooling/scripts/ci/shell_authority_config_guard.ts` | mirror declarations for rehomed Core, Orchestration, Validation, and conformance contracts | branch-diff check rejects new authority-shaped client config JSON unless explicitly mirror-marked | guarded |
