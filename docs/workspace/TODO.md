...
Queued: 13 items. Intent: eliminate Alpine.js reactive proxy overhead from the chat dashboard, which accounts for the ~750MB "system" JS heap from Alpine wrapping every property of every message object and all chat state. Items are ordered by dependency — each step is independently shippable. The Svelte custom element shells (`chat_page_shell`, `chat_thread_shell`, `chat_input_footer_shell`, etc.) already exist as stubs and are the migration targets.
