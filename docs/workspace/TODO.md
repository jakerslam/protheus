...
- [ ] KS-AUDIT-021 Add consistency checks between upstream guards and sentinel findings.
- [ ] KS-AUDIT-022 Prevent sentinel from opening a critical finding when the upstream guard artifact says `pass: true` unless corroborating evidence exists.
- [ ] KS-AUDIT-023 Add contradiction reporting for `guard passed / sentinel failed` cases.
- [ ] KS-AUDIT-024 Require sentinel findings to cite the exact failing field or failing check from the upstream artifact.
- [ ] KS-AUDIT-025 Add a cross-artifact truth-consistency guard for sentinel ingestion.
- [ ] KS-AUDIT-026 Improve feedback quality ranking.
- [ ] KS-AUDIT-027 Prioritize findings by real operator value: correctness, security, release blocking, then optimization.
- [ ] KS-AUDIT-028 Down-rank repetitive empty-response variants once a parent issue already exists.
- [ ] KS-AUDIT-029 Add better summaries/titles for issue drafts so they are GitHub-issue-ready with less cleanup.
- [ ] KS-AUDIT-030 Surface top recurring failure families separately from raw finding count.
...
- [ ] WF-AUTH-015 Add a workflow-authority audit command that reports whether each transition came from `llm_submission`, `system_guard`, `telemetry_only`, or `forbidden_auto_decision`.
