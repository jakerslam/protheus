...
| --- | --- | --- | --- |
| Scrape pipeline internals | remaining browser/playwright service implementation details beyond the service-client/replay/agent surfaces | low | Core scrape orchestration, engine routing, metadata, links, cleanup, worker lifecycle, replay validation, browser-agent loop, and specialist media helpers are parsed. Remaining browser-service implementation is only needed if browser-enabled retrieval becomes the target. |
| Crawl and batch worker internals | full NuQ SQL implementation and reconciler details | low | Controller/status contracts, SDK waiters, v1/v2 tests, scrape worker lifecycle, queue scheduling, liveness, locks, concurrency, and terminal aggregation are parsed/surveyed. Remaining queue internals are mostly implementation detail unless async research execution is next. |
| Map/discovery internals | deeper crawler fixture/mocks and edge e2e line-by-line | low | Controller, map-utils, crawler, sitemap, v1/v2 map tests, and e2e map behavior are parsed. The portable discovery patterns are now captured. |
| Extract/structured data | remaining duplicate/legacy extract helpers and examples | low | Main lifecycle, schema/completion internals, document packaging, source tracking, merge/dedupe helpers, v1/v2 tests, and e2e extraction are parsed. No current freeform research synthesis change needed. |
| Tests and evals | scrape load/eval artifacts and full long e2e files line-by-line | low | Core v2, v1, map, extract, search, deep-research, and representative e2e surfaces are parsed/surveyed. Remaining full e2e pass is mostly parity/noise unless a specific regression points there. |
| MCP/CLI/skill surfaces | any dedicated MCP server package if added upstream later | low | Current inventory did not reveal a separate high-signal MCP implementation surface beyond examples/SDK taxonomy. Re-check only if provider integration becomes the target. |
