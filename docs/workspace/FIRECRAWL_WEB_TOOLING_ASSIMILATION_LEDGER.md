...
| --- | --- | --- | --- |
| Scrape pipeline internals | deeper browser/playwright service details and specialist media extraction helpers | medium | Core scrape orchestration, engine routing, metadata, links, cleanup, and worker lifecycle are parsed. Remaining specialist browser/media internals should wait until we work on browser-enabled retrieval. |
| Crawl and batch worker internals | deeper redis/queue scheduling, cancellation, and retry internals | medium | Controller/status contracts, SDK waiters, v1/v2 tests, and scrape worker lifecycle are parsed. Deeper queue internals matter for future async research, not current single-turn web quality. |
| Map/discovery internals | deeper crawler/sitemap utilities beyond map-utils | medium | Controller, map-utils, v1/v2 map tests, and e2e map behavior are parsed. Remaining sitemap/crawler internals matter only if site-discovery becomes the next primitive. |
