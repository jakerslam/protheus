...
| `adapter_graduation_manifest_gate` | Adapter graduation uses manifest + scenario/hook completion checks. | Update adapter manifest with scenario coverage and verify all required chaos scenarios pass before release. | Revert manifest change to prior known-good state; mark affected adapters as non-graduated. |
