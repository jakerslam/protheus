...
3. User-specific state stays in `memory/` + `adaptive/`.
4. Permanent runtime and policy logic stays in `systems/` + `config/` + `docs/`.
...
```bash
node systems/ops/pinnacle_integration_contract_check.js check --strict=1
```
