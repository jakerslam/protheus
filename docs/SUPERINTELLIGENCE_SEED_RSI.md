...
This upgrade adds an RSI wrapper at `adaptive/rsi/rsi_bootstrap.{ts,js}` that composes and governs existing live primitives:

- System 3 executive (`adaptive/executive/system3_executive_layer.js`)
- Existing executors (`systems/strategy/strategy_learner.js`, `systems/autonomy/model_catalog_loop.js`)
...
2. RSI orchestration latency:
   - `node adaptive/rsi/rsi_bootstrap.js step --owner=jay --mock=1`
   - Record p50/p95 step duration from `state/adaptive/rsi/receipts.jsonl`.
3. Contract-lane health:
   - `node adaptive/rsi/rsi_bootstrap.js contract-lane-status --owner=jay`
