...
- Start daemon and verify control plane:
  - `npm run start`
  - `protheus status`
- Read escalation surfaces in `docs/OPERATOR_RUNBOOK.md`.
...
- Run full local baseline:
  ...
  - `npm run lint`
  - `npm run test`
- Identify one lane in `systems/` and map:
  - entrypoint
  - tests
  - dependent config/state files
...
- No onboarding PR is merged without tests/docs/changelog coverage.
