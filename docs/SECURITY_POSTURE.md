...
- Rust owns validation and policy decisions (`crates/conduit`, `crates/conduit-security`).
...
Source: `config/guard_check_registry.json` (`merge_guard.checks`)
...
cargo run --quiet --manifest-path crates/ops/Cargo.toml --bin protheus-ops -- benchmark-matrix run --refresh-runtime=1
```
