...
- human-authority tasks (legal/commercial/compliance governance).
...
- Acceptance:
  - `config/f100_enterprise_baseline_contract.json` defines baseline checks.
  - `systems/ops/f100_enterprise_baseline_gate.js` emits deterministic receipts.
...
- Acceptance:
  - Helm chart exists under `deploy/helm/protheus/`.
  - Terraform module exists under `deploy/terraform/protheus_helm/`.

3. `REQ-24-003` Human-only requirements must be explicitly split from machine-executable backlog lanes.
- Acceptance:
  - `docs/HUMAN_ONLY_ACTIONS.md` includes rows for external certifications, support/SLA authority, and legal attestation ownership.
