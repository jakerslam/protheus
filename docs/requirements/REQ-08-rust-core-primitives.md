...
- Acceptance: `crates/task`, `crates/resource`, `crates/isolation`, `crates/ipc`, `crates/storage`, `crates/observability`, `crates/update` exist and compile in workspace.
...
- Migrated remaining legacy `systems/primitives/*` wrapper lanes to direct conduit (`canonical_event_log`, `cognitive_control_primitive`, `policy_vm`, `primitive_catalog`, `primitive_registry`, `replay_verify`).
