...
```bash
node systems/ops/post_launch_migration_readiness.js run
node systems/ops/post_launch_migration_readiness.js run --strict=1
node systems/ops/post_launch_migration_readiness.js final-review --decision=no-go --signed-by=jay --approval-note="pending full pass"
node systems/ops/post_launch_migration_readiness.js status
```
...
Rollback template: `docs/release/templates/rollback_plan.md`.
