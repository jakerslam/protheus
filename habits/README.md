...
node habits/scripts/run_habit.js --list

# Run a habit (active habits; candidates require --force to auto-promote tracking)
node habits/scripts/run_habit.js --id rebuild_validate_memory --json '{"notes":"optional"}'

# Propose a new habit (requires BOTH triggers A AND B)
node habits/scripts/propose_habit.js \
...
node habits/scripts/habit_gc.js --dry-run
node habits/scripts/habit_gc.js --apply

# Check capacity
node habits/scripts/habit_gc.js --check-cap

# List habits with trust status
node habits/scripts/habit_list.js --trusted
...
- `entrypoint` — must be habits/routines/<id>.js
...
node habits/scripts/trust_add_habit.js \
  habits/routines/<habit>.js \
...
- `habits/registry.json` — State machine + metrics
- `habits/routines/` — Habit code (routines)
- `habits/routines/_archived/` — Archived routines
- `habits/scripts/` — Infrastructure (runner, doctor, GC, proposer)
- `habits/logs/habit_runs.ndjson` — Execution log with outcome_score/delta_value
- `habits/logs/habit_errors.ndjson` — Error log

## Schema Reference

See `habits/registry.json` for full schema:
- `version`: 1.5
- `max_active`: 25
