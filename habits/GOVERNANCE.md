...
1. Move routine file to `habits/_archive/<id>.<date>.js`
2. Mark registry `status: archived`
3. Keep trusted hash record, add `"status": "archived"`
4. Log to `habits/logs/gc.ndjson`:
   ```json
...
| Write paths | Explicit only | No broad globs beyond `memory/*.md`, `habits/logs/*`, `habits/registry.json` |
...
### Per-Run Logging
Each habit run logs to `habits/logs/habit_runs.ndjson`:
```json
{
  "ts": "ISO-8601",
  "habit_id": "...",
