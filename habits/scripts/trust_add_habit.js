...
const TRUST_ADD_PATH = '/Users/jay/.openclaw/workspace/memory/tools/trust_add.js';
...
function main() {
  ...
  if (args.length < 2) {
    ...
    console.error('Example: node trust_add_habit.js habits/routines/rebuild_validate_memory.js "Core validation habit"');
    process.exit(1);
  }
  ...
  // Check file is in habits/routines
  if (!targetPath.includes('habits/routines')) {
    console.error('ERROR: Habit must be in habits/routines/ directory');
    process.exit(1);
  }
  ...
  const { computeHash } = require('/Users/jay/.openclaw/workspace/memory/tools/skill_gate');
