...
fn comparative_request_exposes_verifier_and_alternative_plan_provenance() {
    ...
    assert!(merged_memory_step
        .merged_capabilities
        .contains(&infring_orchestration_v1::contracts::Capability::ReadMemory));
    assert!(merged_memory_step
        .merged_capabilities
        .contains(&infring_orchestration_v1::contracts::Capability::VerifyClaim));
    assert!(
...
fn pure_read_plan_is_non_mutating_without_context_preparation() {
    ...
    assert!(!package.selected_plan.steps.iter().any(|step| {
        step.target_contract
            == infring_orchestration_v1::contracts::CoreContractCall::ContextAtomAppend
    }));
    assert!(package
        .decision_trace
        .receipt_metadata
...
fn observed_core_execution_is_projected_into_execution_state_correlation() {
    ...
    runtime.record_execution_observation(
        "core-running",
        infring_orchestration_v1::contracts::CoreExecutionObservation {
            plan_status: Some(infring_orchestration_v1::contracts::PlanStatus::Completed),
            receipt_ids: vec!["receipt-1".to_string(), "receipt-2".to_string()],
            outcome_refs: vec!["outcome-1".to_string()],
            step_statuses: vec![
                infring_orchestration_v1::contracts::CoreExecutionStepObservation {
                    step_id: "step_tool_route_capability_probe".to_string(),
                    status: infring_orchestration_v1::contracts::StepStatus::Succeeded,
                },
                infring_orchestration_v1::contracts::CoreExecutionStepObservation {
                    step_id: "step_claim_verifier_request".to_string(),
                    status: infring_orchestration_v1::contracts::StepStatus::Failed,
                },
            ],
        },
    );
    ...
    assert_eq!(
        ...
        infring_orchestration_v1::contracts::PlanStatus::Completed
    );
    ...
    assert!(package.execution_state.steps.iter().any(|row| {
        row.step_id == "step_claim_verifier_request"
            && row.status == infring_orchestration_v1::contracts::StepStatus::Failed
    }));
    assert_eq!(
        package
            .execution_state
...
fn typed_execution_observation_derives_plan_status_from_step_outcomes() {
    ...
    assert_eq!(
        package.execution_state.plan_status,
        infring_orchestration_v1::contracts::PlanStatus::Failed
    );
    assert_eq!(
        package
            .execution_state
...
fn invalid_target_is_reported_separately_from_missing_target() {
    ...
    assert_eq!(
        ...
        Some(infring_orchestration_v1::contracts::RecoveryReason::TargetInvalid)
    );
    assert_eq!(package.classification.request_class, RequestClass::Mutation);
    assert_eq!(
        package.selected_plan.blocked_on,
        vec![infring_orchestration_v1::contracts::Precondition::TargetSyntacticallyValid]
    );
}
...
fn missing_existing_target_is_reported_as_not_found() {
    ...
    assert_eq!(
        package
            ...
            .as_ref()
            .and_then(|row| row.reason.clone()),
        Some(infring_orchestration_v1::contracts::RecoveryReason::TargetNotFound)
    );
    assert!(package
        .selected_plan
        .blocked_on
        .contains(&infring_orchestration_v1::contracts::Precondition::TargetExists));
}
...
fn adapted_mutation_request_requires_explicit_target_probe_envelope_fields() {
    ...
    assert!(package
        .selected_plan
        .blocked_on
        .contains(&infring_orchestration_v1::contracts::Precondition::TargetSupplied));
    assert!(package.selected_plan.capability_probes.iter().any(|row| {
        row.capability == infring_orchestration_v1::contracts::Capability::MutateTask
...
fn forgecode_request_exposes_workflow_decomposition_metadata() {
    ...
    assert!(plans.iter().any(|plan| {
        plan.capability_graph
            .contains(&infring_orchestration_v1::contracts::Capability::PlanAssimilation)
    }));
