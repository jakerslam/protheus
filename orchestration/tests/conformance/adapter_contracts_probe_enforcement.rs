...
fn adapted_tool_request_requires_explicit_tool_probe() {
    ...
    assert_eq!(
        ...
        infring_orchestration_v1::contracts::PlanStatus::ClarificationRequired
    );
    assert_eq!(
        package
            .execution_state
        ...
        Some(infring_orchestration_v1::contracts::RecoveryReason::TransportFailure)
    );
    assert!(package
        .selected_plan
        .blocked_on
        .contains(&infring_orchestration_v1::contracts::Precondition::ToolAvailable));
...
fn adapted_workspace_request_requires_workspace_specific_probe_fields() {
    ...
    assert!(package.selected_plan.capability_probes.iter().any(|row| {
        matches!(
            row.capability,
            infring_orchestration_v1::contracts::Capability::WorkspaceRead
                | infring_orchestration_v1::contracts::Capability::WorkspaceSearch
        ) && row.probe_sources.iter().any(|source| {
...
fn adapted_workspace_request_requires_workspace_transport_probe_field() {
    ...
    assert!(package
        .selected_plan
        .blocked_on
        .contains(&infring_orchestration_v1::contracts::Precondition::TransportAvailable));
    assert!(package.selected_plan.capability_probes.iter().any(|row| {
        matches!(
            row.capability,
            infring_orchestration_v1::contracts::Capability::WorkspaceRead
...
fn adapted_tool_route_request_requires_transport_probe_field() {
    ...
    assert!(package
        ...
        .contains(&infring_orchestration_v1::contracts::Precondition::TransportAvailable));
    assert!(package.selected_plan.capability_probes.iter().any(|row| {
        row.capability == infring_orchestration_v1::contracts::Capability::ToolRoute
            && row.probe_sources.iter().any(|source| {
...
fn adapted_assimilation_request_requires_explicit_policy_probe() {
    ...
    assert_eq!(
        package.execution_state.plan_status,
        infring_orchestration_v1::contracts::PlanStatus::Blocked
    );
    assert_eq!(
        package
            .execution_state
            ...
            .as_ref()
            .and_then(|row| row.reason.clone()),
        Some(infring_orchestration_v1::contracts::RecoveryReason::PolicyDenied)
    );
    assert!(package
        .selected_plan
        .blocked_on
        .contains(&infring_orchestration_v1::contracts::Precondition::PolicyAllows));
    assert!(package.selected_plan.capability_probes.iter().any(|row| {
        row.capability == infring_orchestration_v1::contracts::Capability::PlanAssimilation
            && row.probe_sources.iter().any(|source| {
...
fn adapted_mutation_request_requires_explicit_authorization_probe() {
    ...
    assert_eq!(
        package
            ...
            .as_ref()
            .and_then(|row| row.reason.clone()),
        Some(infring_orchestration_v1::contracts::RecoveryReason::AuthorizationFailure)
    );
    assert!(package
        .selected_plan
        .blocked_on
        .contains(&infring_orchestration_v1::contracts::Precondition::AuthorizationValid));
    assert!(package.selected_plan.capability_probes.iter().any(|row| {
