...
fn build_report(chat_monitor_path: &str, issue_drafts_path: &str, router_path: &str) -> Value {
    ...
    let checks = vec![
        ...
        json!({
            ...
            "ok": issues.iter().all(|row| str_at(row, &["owner_component"], "").starts_with("orchestration")),
    ...
    for row in rows {
        ...
        out.push(json!({
            ...
            "verification": {
                ...
                "replay_command": str_at(&row, &["replay_command"], "cargo run --quiet --manifest-path orchestration/Cargo.toml --bin eval_runtime -- issue-authority --strict=1")
            },
