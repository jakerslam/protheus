...
const DEFAULT_CASES_PATH: &str = "validation/evals/fixtures/eval_grader_hacking_cases.json";
...
fn trust_issues(
    ...
    if pass_rate_divergence > max_pass_rate_divergence {
        issues.push(json!({
            ...
            "replay_command": "cargo run --quiet --manifest-path orchestration/Cargo.toml --bin eval_runtime -- grader-hacking-guard --strict=1"
        }));
    }
