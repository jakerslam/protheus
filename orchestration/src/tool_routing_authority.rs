// Layer ownership: orchestration (tool-routing authority evidence only).
...
pub fn build_tool_routing_authority_report(root: impl AsRef<Path>) -> ToolRoutingAuthorityReport {
    ...
    let request_surface_policy = read_text(
        root,
        "orchestration/config/request_surface_probe_authority_policy.json",
    );
    let result_packaging = read_text(root, "orchestration/src/result_packaging.rs");
    let continuous_eval = read_text(root, "orchestration/src/continuous_eval.rs");
    let eval_feedback_router = read_text(root, "orchestration/src/eval_feedback_router.rs");
    let transient_context = read_text(root, "orchestration/src/transient_context.rs");
    ...
    let self_maintenance_noise_policy_json = read_text(
        root,
        "orchestration/config/self_maintenance_noise_discipline_policy.json",
    );
    let legacy_ingress_budget_ratchet_schedule = read_text(
        root,
        "orchestration/config/legacy_ingress_budget_ratchet_schedule.json",
    );
    ...
    let eval_issue_candidates_source = read_text(
        root,
        "orchestration/src/eval_learning_loop_issue_candidates.rs",
    );
...
fn planner_payload_decision_audit(
    ...
    vec![
        legacy_payload_decision_row(
            "orchestration/src/planner/preconditions.rs",
            "legacy_probe_shortcuts",
            preconditions,
        ),
        non_legacy_payload_decision_row(
            "orchestration/src/planner/plan_candidates.rs",
        ...
        non_legacy_payload_decision_row(
            "orchestration/src/sequencing.rs",
...
fn legacy_ingress_budget_ratchet_monotonic_declared(
    ...
    if schedule_json.is_empty() {
        missing.push("legacy_ingress_budget_ratchet_schedule.json missing or empty".to_string());
...
mod tests {
    ...
    fn runtime_quality_schema_workflow_clean_rejects_forgecode_field() {
        ...
        let check = runtime_quality_schema_workflow_clean(contracts);
        assert!(!check.ok);
        assert!(check.missing.iter().any(|row| row.contains("forgecode")));
    }
    ...
    fn runtime_quality_schema_workflow_clean_does_not_inspect_workflow_struct() {
        ...
        assert!(
            check.ok,
            "guard must not flag workflow-tier fields: {:?}",
            check.missing
        );
    }
