...
fn issue(agent: &str, id: &str) -> EvalIssue {
    EvalIssue {
        ...
        owner_component: "orchestration/tool-routing".to_string(),
...
fn repeated_learning_candidates_emit_agent_trend_feedback() {
    ...
    for idx in 0..4 {
        ...
        row.owner_component = "orchestration".to_string();
        row.replay_command = "cargo run --quiet --manifest-path orchestration/Cargo.toml --bin eval_runtime -- learning-loop-issues --strict=1".to_string();
        issues.push(row);
    }
