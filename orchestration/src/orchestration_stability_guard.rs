// Layer ownership: orchestration (remediation-scope stability evidence only).
...
pub fn build_orchestration_stability_report(
    ...
    let lib_rs = read_text(root, "orchestration/src/lib.rs");
...
fn architecture_split_preserved(root: &Path, lib_rs: &str) -> OrchestrationStabilityCheck {
    let required_paths = [
        "orchestration/src/ingress",
        "orchestration/src/planner",
        "orchestration/src/control_plane",
        "orchestration/src/recovery.rs",
        "orchestration/src/sequencing.rs",
        "orchestration/src/result_packaging.rs",
        "orchestration/src/telemetry.rs",
    ];
