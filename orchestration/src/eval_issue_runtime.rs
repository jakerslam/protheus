...
const DEFAULT_ISSUE_SOURCE_PATH: &str = "validation/evals/fixtures/eval_gold_dataset_v1.jsonl";
...
fn issue_owner(issue_class: &str) -> &'static str {
    match issue_class {
        ...
        "no_response" | "response_loop" | "policy_block_confusion" => {
            "orchestration/workflow-finalization"
        }
        "hallucination" | "tool_output_misdirection" => "orchestration/synthesis",
        "non_actionable_feedback" => "orchestration/eval",
        _ => "orchestration",
    }
}
...
fn make_issue_draft(row: &Value, fixture_dir: &str) -> Option<Value> {
    ...
    let replay_command = format!(
        "cargo run --quiet --manifest-path orchestration/Cargo.toml --bin eval_runtime -- replay --fixture={}",
        replay_fixture_path
    );
    Some(json!({
        "id": id,
