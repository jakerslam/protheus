...
const DEFAULT_WORKFLOW_CASES: &str = "validation/evals/fixtures/eval_workflow_selection_cases.json";
...
pub fn run_runtime_ownership(args: &[String]) -> i32 {
    ...
    let runtime_paths = [
        ...
        "orchestration/src/eval_issue_runtime.rs",
        "orchestration/src/eval_lifecycle_runtime.rs",
        "orchestration/src/eval_final_runtime.rs",
    ];
    let all_present = runtime_paths.iter().all(|path| Path::new(path).exists());
    let all_orchestration = runtime_paths
        .iter()
        .all(|path| path.starts_with("orchestration/src/"));
    ...
    let report = json!({
        ...
        "checks": [
            ...
            {"id": "eval_runtime_orchestration_owned", "ok": all_orchestration, "detail": "orchestration/src"},
