...
fn issue_candidate_from_inbox_row(row: &Value) -> Value {
    ...
    let signal = primary_signal(row);
    let suspected_layer = str_at(row, &["suspected_layer"]).unwrap_or("orchestration");
    let confidence = row
        .get("confidence")
    ...
    json!({
        ...
        "repro_path": format!("cargo run --quiet --manifest-path orchestration/Cargo.toml --bin eval_runtime -- learning-loop-issues --source={} --strict=1", DEFAULT_ISSUE_SOURCE_PATH),
        "acceptance_criteria": [
...
mod tests {
    ...
    fn grounded_row(trace_id: &str, severity: &str) -> Value {
        json!({
            ...
            "turn_id": "web_001",
            "component": "orchestration.tool_routing",
            "severity": severity,
            "suspected_layer": "orchestration/tool-routing",
            "confidence": 0.75,
            "receipt_ids": [],
